...
class action_overview
{
	...
	function run()
	{
		...
		{
			$this->install->template->append( $this->install->template->login_page( "Необходимо указать имя пользователя" ) );
		...
		{
			$this->install->template->append( $this->install->template->login_page( "Необходимо указать пароль" ) );
			$this->install->template->next_action = '?p=overview';
			return;
		}
        ...
        if ( empty($mem['id']) )
        {
            $this->install->template->append( $this->install->template->login_page( "Пользователь с указанными именем не существует в системе" ) );
            $this->install->template->next_action = '?p=overview';
            return;
        }
            ...
            ( isset($this->install->saved_data['vid']) AND $this->install->saved_data['vid'] < 10003 ) )
        {
            ...
            {
                $this->install->template->append( $this->install->template->login_page( "Введенный пароль неверен" ) );
                $this->install->template->next_action = '?p=overview';
                return;
            }
            else
            {
                ...
                {
                    $this->install->template->append( $this->install->template->login_page( "Вы не обладаете правами администратора" ) );
                    $this->install->template->next_action = '?p=overview';
                    return;
                }
                else
        ...
        else
        {
            ...
            {
                $this->install->template->append( $this->install->template->login_page( "Пользователь с указанными email не существует в системе" ) );
                $this->install->template->next_action = '?p=overview';
                return;
            }
		...
		{
			$error[] = "Не удалось определить последнюю доступную версию продукта.";
		}
		if ( count($this->install->versions) == 0 )
		{
			$error[] = "Не удалось найти скрипт для обновления: 'upg_{$this->install->last_poss_id}/version_history.php'";
		}
		if ( ! $this->install->current_version )
		{
			$error[] = "Не удалось определить версию установленного продукта";
		}
		if ( $this->install->last_poss_id AND $this->install->last_poss_id <= $this->install->current_version )
		{
			$error[] = "У вас установлена последняя доступная версия продукта. Обновление не требуется.";
		}
		...
		foreach ( $checkfiles as $cf )
		{
			if ( ! file_exists($cf) )
			{
				$error[] = "Мастер не смог найти необходимый для работы файл '$cf'.";
			}
		}
		...
		foreach ( $writeable as $cf )
		{
			if ( ! is_writeable($cf) )
			{
				$error[] = "Ресурс (файл или директория) '$cf' не доступен для записи. Пожалуйста измените CHMOD на 0777.";
			}
		}
		...
		$current_version = $this->install->current_version == '00000' ? 'не установлен' : $this->install->versions[ $this->install->current_version ];
		...
		{
			$summary .= "обновление ".$this->install->versions[ $this->install->current_version ]." до версии ".$this->install->versions[ $this->install->last_poss_id ];
		}
		...
		$summary .= "<br /><br /><strong>Проверка валидности ресурс-файлов</strong>";
		$_flag    = 0;
		...
		foreach ( $timecheck as $cf )
		{
			...
			{
				$_flag    = 1;
				$summary .= "<div style='color:red'>&middot;" . str_replace( ROOT_PATH, '', $cf ) . ' может быть устаревшим</div>';
			}
		}
		
		if ( $_flag )
		{
			$summary .= "<br /><div>Пожалуйста убедитесь, что файлы из списка закачаны из дистрибутива ".$this->install->versions[ $this->install->last_poss_id ]." версии";
		}
		else
		{
			$summary .= "<div style='color:green'>Все ресурс-файлы валидны</div>";
		}

		if ( $cp = $this->install->get_db_codepage() ) 
		{
			$summary .= "
                  <br />
 				  <strong>Кодировка таблиц в MySQL</strong>
				  <table width='100%' cellspacing='0' cellpadding='0' align='left' border='0'>
				  <tr>
					<td class='title'><div style='color:gray; font-weight:normal;'>Не изменяйте, если не уверены.</div></td>
					<td class='content'><input type='text' name='mysql_codepage' class='sql_form' value='" . $cp . "'></td>
				  </tr>
                  </table>";
			
		} else {
			$summary .= "<input type='hidden' name='mysql_codepage' value='none'>";
		}
