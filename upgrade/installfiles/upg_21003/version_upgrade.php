...
class version_upgrade
{
	...
	function upgrade_sql( $id=1 )
	{
		...
		if ( file_exists( ROOT_PATH . 'upgrade/installfiles/upg_21003/' . strtolower($this->install->ipsclass->vars['sql_driver']) . $file ) )
		{
			...
			$this->install->message = "Выполнено $this->sqlcount запросов...";
		}
		...
		{
			$this->install->message .= "<br /><br /><h3><b>Для продолжения обновления необходимо выполнить следующие MySQL запросы:</b></h3><br />".nl2br(htmlspecialchars($output));
	...
	function convert_polls()
	{
		...
		if ( ! $done )
		{
			$this->install->message = "Опросы: $start из $end обновлены...";
			$this->install->saved_data['workact'] 	= 'polls';
		...
		{
			$this->install->message = "Опросы обновлены, переходим к обновлению календарных событий...";
	...
	function convert_calevents()
	{
		...
		if ( $this->install->ipsclass->DB->get_num_rows($o) )
		{
			...
			$this->install->message = "Календарные события: $start из $end обновлены...";
			$this->install->saved_data['workact'] 	= 'calevents';
			$this->install->saved_data['st'] 		= $end;
			return FALSE;
		}
		...
		{
			$this->install->message = "Календарные события обновленя,  переходим к созданию главноего стиля IPB 2.1 ...";
	...
	function add_skin()
	{
		$this->install->message = "Пропускаем создание стиля для версии 2.1 (он будет создан позже)...";
		...
		$this->install->message = "Главный стиль IPB 2.1 создан...";
		unset($this->install->saved_data['workact']);
		unset($this->install->saved_data['vid']);
