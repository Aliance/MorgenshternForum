	...
	function check_requirements()
	{
		...
		if( count( $this->writeable_dirs ) )
		{
			foreach( $this->writeable_dirs as $_d )
			{
				if( ! is_writeable( $_d ) )
				{
					$errors[] = 'Директория &laquo;' . str_replace( INS_DOC_ROOT_PATH, '', $_d ) . '&raquo; не доступна для записи.<br />Пожалуйста измените права доступа (CHMOD) на 777.';
				}
			}
			...
			foreach( $this->writeable_files as $_f )
			{
				if( ! is_writeable( $_f ) )
				{
					$errors[] = 'Файл &laquo;' . str_replace( INS_DOC_ROOT_PATH, '', $_f ) . '&raquo; не доступен для записи.<br />Пожалуйста измените права доступа (CHMOD) на 777.';
				}
			}
		}
