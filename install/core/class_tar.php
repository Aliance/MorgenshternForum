...
class tar {
	...
	function add_directory( $dir ) {
		...
		if (! is_dir($dir) )
		{
			$this->error = "Ошибка распаковки файлов: Директории ($to_dir) для распаковки не существует";
			return FALSE;
		}
	...
	function get_dir_contents( $dir )
	{
		...
		{
			$this->error = "Не возможно найти директорию $dir";
			return;
		}
	}
		...
		foreach ($in_files as $file)
		{
			...
			if (count($path_info) > 0)
			{
				foreach($path_info as $dir_component)
				{
					...
					{
						$this->warnings[] = "ВНИМАНИЕ: $dir_component существует, но не является директорией";
						continue;
					}
					...
					{
						$this->warnings[] = "ОШИБКА: не возможно перейти в директорию $dir_component!";
					}
				}
			}
			...
			if (($file['typeflag'] == 0) or (!$file['typeflag']) or ($file['typeflag'] == ""))
			{
				...
				{
					$this->warnings[] = "Не возможно записать данные в файл $file_name";
				}
			}
			else if ($file['typeflag'] == 5)
			{
				...
				{
					$this->warnings[] = "$file_name существует, но не является директорией";
					continue;
				}
				if (! is_dir($file_name))
				{
			...
			{
				$this->warnings[] = "Не возможно обработать именованные pipes";
				continue;
			}
			else if ($file['typeflag'] == 1)
			{
				$this->warnings[] = "Не возможно обработать ссылки";
			}
			else if ($file['typeflag'] == 4)
			{
				$this->warnings[] = "Не возможно обработать файлы устройств";
			}	
			...
			{
				$this->warnings[] = "Не известный тип файла";
			}
			
			if (! @chmod( $file_name, $file['mode'] ) )
			{
				$this->warnings[] = "ОШИБКА: не возможно выставить CHMOD $mode на $file_name";
			}
	...
	function add_files( $files, $root_path="" )
	{
		...
		foreach ($files as $file)
		{
			...
			if (! is_array($stat) )
			{
				$this->warnings[] = "ОШИБКА: не возможно получить информацию о $file";
				continue;
			}
			...
			if (is_file($file))
			{
				...
				{
					$this->warnings[] = "ОШИБКА: Не возможно открыть файл $file";
					continue;
				}
			}
	...
	function write_tar() {
		...
		if ($this->tarfile_path_name == "") {
			$this->error = 'Не указано имя или путь для создания нового tar архива';
			return;
		}
		
		if ( count($this->tar_in_mem) < 1 ) {
			$this->error = 'Нет данных для записи в tar архив';
			return;
		}
		...
		foreach ($this->tar_in_mem as $file) {
			...
			if (strlen($file['name']) > 99)
			{
				...
				if (is_string($pos) && !$pos)
				{
					...
					$this->error[] = "Имя файла {$file['name']} превышает допустимую стандартом GNU Tape ARchives длину";
					continue;
				}
				...
				{
					$this->error[] = "Путь файла, помещаемого в архив, превышает допустимую стандартом GNU Tape ARchives длину";
					continue;
				}
			}
	...
	function read_tar() {
		...
		if ($filename == "") {
			$this->error = 'Ну указано имя архива для чтения';
			return array();
		}
		
		if (! file_exists($filename) ) {
			$this->error = 'Не возможно найти файл '.$filename;
			return array();
		}
		...
		if (! $FH = fopen( $filename , 'rb' ) ) {
			$this->error = "Файл $filename не доступен для чтения";
			return array();
		}
		...
		while (!feof($FH)) {
			...
			if (strlen($data) != $size) {
				$this->error = "Ошибка чтения tar-архива";
				fclose( $FH );
				return array();
			}
