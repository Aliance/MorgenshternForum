	...
	function run_task()
	{
		...
		if ( intval($this->ipsclass->vars['validate_day_prune']) > 0 )
		{
			...
			$this->ipsclass->DB->build_query( array( 'select' => 'v.vid, v.member_id',
													 ...
													 'add_join' => array( 0 => array( 'select' 	=> 'm.posts, m.mgroup',
				...
				if( $i['mgroup'] != $this->ipsclass->vars['auth_group'] )
			{
					...
					$this->ipsclass->DB->do_delete( 'validating', "v.vid='{$i['vid']}'" );
					continue;
				}
			...
			if ( count($mids) > 0 )
			{
				$this->ipsclass->DB->simple_exec_query( array( 'delete' => 'members_converge', 'where' => "converge_id IN(".implode(",",$mids).")" ) );
			...
			$this->class->append_task_log( $this->task, 'Удалено '.count($mids).' не подтвердивших регистрацию пользователей' );
		}
