...
class ad_logs_login
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Попытки авторизации в АЦ' );
	...
	function login_logs_view_detail()
	{
		...
		if ( ! $log['admin_id'] )
		{
			$this->ipsclass->main_msg = "По указанной записи деталей не найдено.";
			$this->login_logs_view();
			return;
		}
	...
	function login_logs_view()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->skin_acp_global->information_box( "Попытки авторизации в АЦ", "На этой странице отображены все попытки авторизации в админцентр вашего форума.<br />Красным индикатором отмечены неудачные попытки авторизации, в то время как зеленым — удачные." ) . "<br />";
