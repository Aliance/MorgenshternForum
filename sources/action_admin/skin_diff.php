...
+---------------------------------------------------------------------------
|   > $Date: 2006-12-08 16:41:30 -0500 (Fri, 08 Dec 2006) $
|   > $Revision: 781 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_skin_diff
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->page_detail = "Сравнить ваш стиль.";
		$this->ipsclass->admin->page_title  = "HTML Сравнение стилей";
		$this->ipsclass->admin->nav[] 		= array( $this->ipsclass->form_code, 'Сравнение' );
	...
	*/
	function skin_differences_remove()
	{
		...
		{
			$this->ipsclass->admin->error("Невозможно получить текущую сессию шаблона.");
		}
		...
		$this->ipsclass->main_msg = "Отчет о различиях удален";
	...
	*/
	function skin_differences_view_diff()
	{
		...
		{
			$this->ipsclass->admin->error( "Ключ не найден" );
		}
	...
	*/
	function skin_differences_view()
	{
		...
		while( $row = $this->ipsclass->DB->fetch_row() )
		{
			...
			{
				$diff_is = '<span style="color:red">Новое</span>';
				$missing++;
			}
			else
			{
				$diff_is = '<span style="color:green">Изменено</span>';
				$changed++;
			}
	...
	*/
	function skin_differences_start_from_skin()
	{
		...
		{
			$this->ipsclass->admin->error( "Не выбран ID стиля (заново!)" );
		}
		...
		$this->ipsclass->DB->do_insert( 'template_diff_session', array( 'diff_session_togo'    		  => intval( $total_bits['count'] ),
																		'diff_session_done'    		  => 0,
																		'diff_session_title'   		  => "Сравнение со стилем: ".$skin_set['set_name'],
	...
	*/
	function skin_differences_start()
	{
		...
		{
			$this->ipsclass->admin->error( "Вы должны ввести название" );
		}
		...
		if ( $_FILES['FILE_UPLOAD']['name'] == "" or ! $_FILES['FILE_UPLOAD']['name'] or ($_FILES['FILE_UPLOAD']['name'] == "none") )
		{
			...
			$this->ipsclass->admin->error( "Файл не был загружен" );
		}
		else
		{
			//-----------------------------------------
		...
		{
			$this->ipsclass->admin->error("Ошибка с ipb_templates.xml - продолжить невозможно");
		}
	...
	function skin_differences_process()
	{
		...
		if ( ! $done )
		{
			$this->ipsclass->admin->output_multiple_redirect_hit( $this->ipsclass->base_url.'&'.$this->ipsclass->form_code_js."&code=skin_diff_process&diff_session_id={$diff_session_id}&pergo=".$pergo,
															  	  $img.' '.$current_session['diff_session_done'].' из '. $current_session['diff_session_togo'] .' шаблонов обработано...' );
		}
		else
		{
			$this->ipsclass->admin->output_multiple_redirect_done( "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=skin_diff_view&diff_session_id={$diff_session_id}' target='_top'>Результаты</a>");
		}
	}
