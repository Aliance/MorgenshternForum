...
class ad_skin_sets
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Список стилей' );
	...
	function export_bits_complete()
	{
		...
		if ( ! count( $ids ) )
		{
			$this->ipsclass->main_msg = "Вы должны выбрать какую-нибудь часть шаблона для экспорта!";
			$this->export_bits_choose();
		}
	...
	function export_bits_choose()
	{
		...
		$this->ipsclass->DB->simple_construct( array( 'select' => 'group_name,set_id,suid,func_name', 'from' => 'skin_templates', 'where' => 'set_id=1' ) );
		$this->ipsclass->DB->simple_exec();
		...
		$this->ipsclass->admin->page_title  = "Экспорт выбранных частей шаблона";
		$this->ipsclass->admin->page_detail = "Выберите, какие части вы хотели бы экспортировать.";
		...
		$this->ipsclass->html .= "<div class='tablesubheader' align='center'><input type='submit' class='realbutton' value='Экспортировать выбранное' /></form></div></div>";
	...
	function rebuild_all_templates()
	{
		...
		if ( $r['set_skin_set_id'] )
		{
			...
			$this->ipsclass->admin->redirect( "{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&code=rebuildalltemplates&justdone={$r['set_skin_set_id']}", "Обновление кеша для стиля «{$r['set_name']}».<br />Переход к следующему стилю..." );
		}
		else
		{
			$this->ipsclass->main_msg = "Все шаблоны кешированы!";
			$this->list_sets();
		}
	}
	...
	function export_master()
	{
		...
		$this->ipsclass->DB->simple_construct( array( 'select' => 'group_name, section_content, func_name, func_data', 'from' => 'skin_templates', 'where' => 'set_id=1' ) );
		$this->ipsclass->DB->simple_exec();
	...
	function add_set()
	{
		...
		{
			$this->ipsclass->admin->error("Вы должны выбрать ID существующего стиля, вернитесь и попытайтесь заново.");
		}
		
		if ( $this->ipsclass->input['id'] == 1 )
		{
			$this->ipsclass->main_msg = "Вы не можете изменять стиль";
			$this->list_sets();
		}
		
		if ( ! $this->ipsclass->input['set_name'] )
		{
			$this->ipsclass->main_msg = "Вы должны ввести имя стилю.";
			$this->list_sets();
		}
		...
		$this->ipsclass->main_msg = '<b>Стиль добавлен</b>';
	...
	function do_revert_all()
	{
		...
		if ( isset($this->ipsclass->input['html']) AND $this->ipsclass->input['html'] )
		{
			...
			$message[] = 'Все части HTML шаблоны были удалены...';
		}
		...
		if ( isset($this->ipsclass->input['macro']) AND $this->ipsclass->input['macro'] )
		{
			...
			$message[] = 'Все макросы были удалены...';
		}
		...
		$this->ipsclass->main_msg = 'Настройки стиля удалены';
	...
	function revert_all_form()
	{
		...
		if ($this->ipsclass->input['id'] == "")
		{
			$this->ipsclass->admin->error("Вы должны выбрать какую-нибудь часть шаблона для экспорта");
		}
		...
		$this->ipsclass->admin->page_detail = "<strong>Пожалуйста, имейте ввиду, что эти изменения не могут быть приняты!</strong>";
		$this->ipsclass->admin->page_title  = "Возвращение настроек стиля";
		...
		$none = "<em>Нет настроек для удаления</em>";
		...
		$this->ipsclass->html .= "<div class='tableborder'>
							<div class='tableheaderalt'>Возвращение настроек стиля {$this_set['set_name']}</div>
							<div class='tablepad'>
							<fieldset class='tdfset'>
							 <legend><strong>Настройки HTML шаблон</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Вернуть все настройки HTML шаблона?<br /><span style='color:gray'>Вы сделали {$templates} изменений в шаблонах</span></td>
							...
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Настройки макросов </strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Вернуть все настройки макросов?<br /><span style='color:gray'>You have {$macros} macro customizations</span></td>
							...
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Настройка шапки и низа шаблона</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Возвращение шапки и низа шаблона?</td>
							...
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Настройка таблицы стилей (CSS)</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Обратить CSS к стандартным?</td>
							   <td width='60%' class='tablerow1'>{$css}</td>
							 </tr>
							 </table>
							</fieldset>
							<div style='color:red;text-align:center;font-size:11px;padding:6px'>Имейте ввиду, что все настройки будут потеряны, если вы поставите 'да'<br /><b>Это не может быть завершено</b></div>
							</div>
							</div>";
												 
		$this->ipsclass->html .= $this->ipsclass->adskin->end_form_standalone("Начать");
		...
		$this->ipsclass->admin->nav[] = array( '' ,'Возвращение всех настроек для '.$this_set['set_name'] );
	...
	{
		$this->ipsclass->admin->page_detail = "Пожалуйста, ознакомьтесь с этой страницей внимательно.";
		$this->ipsclass->admin->page_title  = "Удаление стиля";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Удаление {$this_set['set_name']}" );
		
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
																"<div style='color:red;font-weight:bold;font-size:11px'>
																ВНИМАНИЕ: Это действие нельзя будет вернуть или отменить</div><br />
																Продолжение этого действия навсегда удалит все настройки этого стиля, включающие HTML шаблоны, CSS и макросы .
																<br /><br />
																После удаления этого стиля другой автоматически станет основным для форума.
																",
													  )      );
		
		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Окончательное удаление {$this_set['set_name']}");
	...
	function set_toggle_default()
	{
		...
		$this->ipsclass->main_msg = 'Стиль установлен, как по умолчанию';
		$this->list_sets();
	}
	...
	function set_toggle_visible()
	{
		...
		if ( $hidden == 1 )
		{
			...
			{
				$this->ipsclass->main_msg = 'Вы не можете сделать единственный видимый стиль невидимым. ';
				$this->list_sets();
				return;
			}
		}
		...
		$this->ipsclass->main_msg = 'Видимость стиля изменена';
		$this->list_sets();
	}
	...
	function do_remove()
	{
		...
		{
			$this->ipsclass->main_msg = 'IPB не может удалить этот стиль, так как он установлен, как по умолчанию, вам нужно поставить другой стиль, как основной и попробовать удалить этот заново.';
			$this->list_sets();
		}
		...
		{
			$this->ipsclass->main_msg = 'IPB не может удалить этот стиль, так как он единственный стиль, который у вас есть.';
			$this->list_sets();
		}
		...
		$message[] = 'Очистка: удаление кеша CSS...';
		...
		$message[] = 'Очистка: удаление кеша HTML шаблонов...';
		...
		$this->ipsclass->main_msg = 'Стиль удален';
	...
	function save_skin( $type='add' )
	{
		...
		if ($this->ipsclass->input['set_name'] == "")
		{
			$this->ipsclass->admin->error("Вы должны ввести имя для этого пакета");
		}
		...
		else
		{
			...
			$this->ipsclass->main_msg = 'Настройки стиля обновлены';
			
			$this->ipsclass->main_msg .= "<br />".implode("<br />", $this->ipsclass->cache_func->messages);
			
			//$this->ipsclass->admin->redirect("{$this->ipsclass->form_code}", "Настройки стиля обновлены" );
			$this->list_sets();
		}
	}
	...
	function do_form( $type='add' )
	{
		...
		$sets     = array();
		$parents  = array( 0=> array( '-1', 'Нет родителя' ) );
		$row      = array();
		...
		{
			$cssextra = "<em>IPB не может произвести запись в директорию /style_images/.</em>";
		}
		
		
		//-----------------------------------------
	
		$this->ipsclass->admin->page_detail = "Изменить настройки вы можете ниже.";
		$this->ipsclass->admin->page_title  = "Управление стилями";
		...
		$this->ipsclass->html .= "<div class='tableborder'>
							...
							<div class='tablerow2'>
							<fieldset class='tdfset'>
							 <legend><strong>Основной</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Название</td>
							   <td width='60%' class='tablerow1'>".$this->ipsclass->adskin->form_input('set_name', $row['set_name'])."</td>
							 </tr>
							 <tr>
							   <td width='40%' class='tablerow1'>Скрыть от пользователей?</td>
		...
		if ( $row['set_default'] )
		{
			$this->ipsclass->html .= "<tr>
							    <td width='40%' class='tablerow1'>Сделать его стилем по умолчанию?</td>
							    <td width='60%' class='tablerow1'><i>Стиль уже используется по умолчанию.</i></td>";
		}
		else
		{
			$this->ipsclass->html .= "<tr>
		...
		$this->ipsclass->html .= "</tr>
							  <tr>
							   <td width='40%' class='tablerow1'>Стиль имеет родителя?</td>
							...
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Hастройки CSS</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Кешировать CSS стили в файл?<br /><span style='color:gray'>Это уменьшит размер HTML.</span>
							   								 </td>
							   <td width='60%' class='tablerow1'>".$cssextra."<br /><span style='color:red'>Внимание: Изменение значения этой опции перекеширует все кеши CSS стилей. Убедитесь, что все ваши кеши CSS синхронизированы с базой данных.</span></td>
							 </tr>
							 </table>
							</fieldset>
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Настройки изображений</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Директория с изображениями<br /><span style='color:gray'>Директория изображений будет заменяться <#IMG_DIR#> в CSS и макросах.</span></td>
							   <td width='60%' class='tablerow1'>style_images/ ".$this->ipsclass->adskin->form_dropdown('set_image_dir', $dirs, $row['set_image_dir'])."</td>
							 </tr>
							  <tr>
							   <td width='40%' class='tablerow1'>Используемые смайлики<br /><span style='color:gray'>Выберите, какие смайлики будут использоваться в этом стиле.</span></td>
							...
							<br />
							<fieldset class='tdfset'>
							 <legend><strong>Сведения об авторе</strong></legend>
							 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
							 <tr>
							   <td width='40%' class='tablerow1'>Имя<br /><span style='color:gray'>*Необязательно.</span></td>
							   <td width='60%' class='tablerow1'>".$this->ipsclass->adskin->form_input('set_author_name', $row['set_author_name'])."</td>
							 </tr>
							 <tr>
							   <td width='40%' class='tablerow1'>E-mail адрес<br /><span style='color:gray'>*Необязательно.</span></td>
							   <td width='60%' class='tablerow1'>".$this->ipsclass->adskin->form_input('set_author_email', $row['set_author_email'])."</td>
							 </tr>
							 <tr>
							   <td width='40%' class='tablerow1'>Домашняя страница<br /><span style='color:gray'>*Необязательно.</span></td>
	...
	function list_sets()
	{
		...
		$this->ipsclass->admin->page_detail = "Для изменения стиля просто нажмите на кнопку справа от названия и выберите необходимый пункт в появившемся меню.";
		$this->ipsclass->admin->page_title  = "Список стилей";
		...
		{	
			...
			$extra  = "";
			$forums = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_notforums.gif' border='0' alt='Не используется в форумах' title='Не используется в форумах' />";
			...
			if ( isset($forum_skins[ $r['set_skin_set_id'] ]) AND is_array($forum_skins[ $r['set_skin_set_id'] ]) )
			{
				...
				{
					$extra  = "Используется в форумах: ".implode( ",", $forum_skins[ $r['set_skin_set_id'] ] );
					
					$forums = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_forums.gif' border='0' alt='' title='$extra' />";
				}
			}
			
			$this->unaltered    = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_item_unaltered.gif' border='0' alt='-' title='Независимый  от родительского стиля' />&nbsp;";
			$this->altered      = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_item_altered.gif' border='0' alt='+' title='Зависимый от родительского стиля' />&nbsp;";
			$this->inherited    = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_item_inherited.gif' border='0' alt='|' title='Унаследованный от родительского стиля' />&nbsp;";
			
			//-----------------------------------------
			// Default / Hidden?
			//-----------------------------------------
			
			$default      = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=toggledefault&id={$r['set_skin_set_id']}' title='Сделать по умолчанию'><img src='{$this->ipsclass->skin_acp_url}/images/skin_notdefault.gif' border='0' alt='Не по умолчанию' /></a>";
			$hidden       = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=togglevisible&id={$r['set_skin_set_id']}' title='Изменение видимости'><img src='{$this->ipsclass->skin_acp_url}/images/skin_visible.gif' border='0' alt='visible' title='Шаблон не скрыт от пользователей' /></a>";
			...
			if ($r['set_hidden'] == 1)
			{ 
				$hidden      = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=togglevisible&id={$r['set_skin_set_id']}' title='Изменение видимости'><img src='{$this->ipsclass->skin_acp_url}/images/skin_invisible.gif' border='0' alt='Invisible' title='Шаблон скрыт от пользователей' /></a>";
			...
			if ($r['set_default'] == 1)
			{
				$default      = "<img src='{$this->ipsclass->skin_acp_url}/images/skin_default.gif' border='0' alt='Не по умолчанию' title='По умолчанию' />";
			...
			{
				$menulist = "\"Это базовый стиль, он не может быть удален или изменен.<br />"
						     ."Если вы хотите изменить стиль по умолчанию, нажмите"
							 ." на «<!DEFAULT>» и выберите соответствующую опцию.\"";
			}
			else
			{
		...
		if( IN_DEV )
		{
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->start_table( 'Режим разработчика: экспорт файлов стиля' );
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->end_form( 'Сгенерировать и скачать файл XML' );
	...
	function generate_mod_xml()
	{
		...
		//$content[] = $xml->xml_build_simple_tag( 'description', "Описание" );
