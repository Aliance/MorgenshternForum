...
+---------------------------------------------------------------------------
|   > $Date: 2006-10-30 06:40:23 -0600 (Mon, 30 Oct 2006) $
|   > $Revision: 684 $
|   > $Author: matt $
+---------------------------------------------------------------------------
|
...
class ad_bbcode {
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->nav[] 		= array( $this->ipsclass->form_code, 'Управление BB-кодами' );

		//-----------------------------------------
		// Require and RUN !! THERES A BOMB
		//-----------------------------------------

		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам добавлять новые BB-коды.";
		$this->ipsclass->admin->page_title  = "Менеджер настройки BB-кодов";
	...
	function bbcode_import()
	{
		...
		if ( ! $content )
		{
			$this->ipsclass->main_msg = "Невозможно выполнить импортирование, поискольку в файле «bbcode.xml» ошибка или он не имеет записей.";
			$this->bbcode_start();
		}
		...
		$this->ipsclass->main_msg = "Записи с BB-кодовами успешно импортирован.";
	...
	function bbcode_delete()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы не указали ID BB-кода для удаления.";
			$this->bbcode_start();
		}
	...
	function bbcode_test()
	{
		...
		$this->ipsclass->main_msg = "<b>Проверка BB-кода:</b><br /><br />".$t;
	...
	function bbcode_save($type='add')
	{
		if ( $type == 'edit' )
		{
			if ( ! $this->ipsclass->input['id'] )
			{
				$this->ipsclass->main_msg = "Вы не указали ID для изменения.";
		...
		{
			$this->ipsclass->main_msg = "Вы должны использовать {content} в тех местах, где происходит замена BB-кодов.";
			$this->bbcode_form($type);
		}

		if ( ! strstr( $this->ipsclass->input['bbcode_replace'], '{option}' ) AND $this->ipsclass->input['bbcode_useoption'] )
		{
			$this->ipsclass->main_msg = "Вы должны использовать {option}, в тех местах, где происходит замена BB-кодов или параметру «Использовать настройки в тегах» установить значение «Нет».";
			$this->bbcode_form($type);
		}
		...
		if ( $type == 'add' )
		{
			...
			$this->ipsclass->main_msg = 'Новый BB-код успешно добавлен.';
		}
		else
		{
			$this->ipsclass->DB->do_update( 'custom_bbcode', $array, 'bbcode_id='.intval($this->ipsclass->input['id']) );
			$this->ipsclass->main_msg = 'BB-код успешно изменен.';
		}
	...
	{
		$this->ipsclass->admin->page_detail = "В этой секции вы сможете просмотреть все созданные вами BB-коды, а так же изменить их или удалить.";
		$this->ipsclass->admin->page_title  = "Дополнительные BB-коды";
		$this->ipsclass->admin->nav[] 		= array( '', 'Добавление/изменение BB-кодов' );
		...
		if ( $type == 'edit' )
		{
			...
			$button = "Изменить";
			$code   = 'bbcode_doedit';
			$title  = "Редактирование BB-кода: ".$bbcode['bbcode_title'];
		}
		else
		{
			$bbcode = array( 'bbcode_title' 	=> '',
							 ...
							 'bbcode_replace'	=> '' );
			$code   = 'bbcode_doadd';
			$title  = "Добавление нового BB-кода";
			$button = "Добавить";
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Название BB-кода</b><div style='color:gray'>Используется во всплывающей подсказке BB-кода</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Полное описание этого BB-кода</b><div style='color:gray'>Используется во всплывающей подсказке BB-код</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Пример этого BB-кода</b><div style='color:gray'>Используется во всплывающей подсказке BB-код<br />Тег для примера: [tag]Здесь ваш пример![/tag]</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Тег этого BB-кода</b><div style='color:gray'>Пример: Для [tag] введите <b>tag</b></div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Использовать опции в теге?</b><div style='color:gray'>Используется для создания BB-кода с опциями: [tag=option]</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Сменить операнды?</b><div style='color:gray'>Если вы желаете заменять {content} на {option} (при использовании тегов IE «любит» [tag={content}]{option}[/tag]</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Замена BB-кода</b><div style='color:gray'>&lt;tag&gt;{content}&lt;/tag&gt;<br />&lt;tag thing='{option}'&gt;{content}&lt;/tag&gt;</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Добавить в меню «Спец. элементы» редактора?</b>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Введите текст для 'option' </b><div style='color:gray'>Текст для помощи в использовании поля 'Option'. Он будет отображаться, когда будет выбран тег в редакторе.</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Введите текст для 'content' </b><div style='color:gray'>Текст для помощи в использовании поля 'Content'. Он будет отображаться, когда будет выбран тег в редакторе.</div>",
		...
		$this->ipsclass->html .= "<br /><div class='tableborder'><div class='tablerow1' style='padding:6px'><b>Дополнительная информация </b><br />Во время добавления замены BB-кода, не забудьте добавить блок {content} там, где это требуется.  <br />
						    Если вы используете опцию BB-кода [tag=option][/tag], не забудьте добавить блок {option} там, где это требуется в замене BB-кода. </div></div>";
	...
	{
		$this->ipsclass->admin->page_detail = "В этой секции вы сможете создать новые BB-коды, которые в последсвие смогут использовать пользователи вашего форума.";
		$this->ipsclass->admin->page_title  = "Добавление нового дополнительного BB-кода";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Проверить BB-код" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "Введите в поле BB-код",
																$this->ipsclass->adskin->form_textarea( 'bbtest', isset($_POST['bbtest']) ? $_POST['bbtest'] : '' ),
														 )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Начать проверку");
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Импортирование списка BB-кодов" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													 		 "<b>Загрузка XML списка BB-кодов</b><div style='color:gray'>Выберите на вашем компьютере файл «ipb_bbcode.xml». Дублирование [tag] невозможно — импортировать не удастся.</div>",
