...
class ad_prefs {
	...
	function auto_run()
	{
		...
		if ( $this->ipsclass->input['set'] == 1 )
		{
			...
			{
				$this->msg = 'Пожалуйста, заполните форму';
...
<html>
 <head>
   <title>IPB-ваши предпочтения</title>
   <style type='text/css'>
	 BODY {
			font-size: 10px;
  ...
  <fieldset style='padding:10px'>
   <legend>Ваши предпочтения в админцентре</legend>
   <strong>Сохранить расположение меню админцентра</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select class='textinput' name='menu'><option value='1' {$this->s_yes}>Да</option><option value='0' {$this->s_no}>Нет</option></select>
   <br /><br />
   <strong>Размер шрифта в текстбоксах</strong>&nbsp;<input  class='textinput' type='text' size='3' name='tx' value='{$this->tx}' /> <strong>X</strong> <input  class='textinput' type='text' size='3' name='ty' value='{$this->ty}' />
   <br /><br />
   <strong>Показывать предпросмотрт макроса</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select class='textinput' name='preview'><option value='1' {$this->p_yes}>Да</option><option value='0' {$this->p_no}>Нет</option></select>
   <br /><br />
   <center><input type='submit'  class='textinput' value='Сохранить' /></center>
  </fieldset>
  </form>
  </body>
  </html>
