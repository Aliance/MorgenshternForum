...
class ad_member
{
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->nav[] = array( "{$this->ipsclass->form_code}&code=edit", 'Управление пользователями' );
		...
		while( $r = $this->ipsclass->DB->fetch_row( $outer ) )
		{
			...
			else if ( $delete_posts AND ! $r['new_topic'] )
			{
				if ( $this->trash_forum and $this->trash_forum != $topic['forum_id'] )
				{
					...
					$new_title   = "АЦ - сообщения, удаленные из темы &laquo;".$topic['title']."&raquo;";
					$new_desc    = "АЦ - сообщения, удаленные из темы с ID: ".$topic['tid'];
	...
	function member_delete_posts_start()
	{
		...
		$this->ipsclass->admin->nav[] 		= array( '', 'Удаление сообщений пользователя' );
		...
		if ( ! $posts['count'] )
		{
			$this->ipsclass->main_msg = "Нет сообщений для удаления.";
			$this->search_results();
		}
	...
	function member_password_complete()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы должны ввести пароль.";
			$this->member_password_start();
		}
		...
		if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
		{
			...
			{
				$this->ipsclass->main_msg = "Действие не выполнено, поскольку невозможно найти сведений о API IP.Converge.";
				$this->member_password_start();
			}
		}
		...
		if( !$converge_member )
		{
			$this->ipsclass->admin->error("Проблема с загрузкой пользовательского аккаунта");
		}
		...
		if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
		{
			...
			{
				$this->ipsclass->main_msg = "Пароль не может быть изменен, так как IP.Converge возвращает ошибку.";
				$this->member_password_start();
				return;
			}
		}
		...
		$this->ipsclass->admin->save_log("Изменен пароль пользователя (ID: {$this->ipsclass->input['id']} )");

		$this->ipsclass->admin->done_screen("Пароль изменен", "Поиск пользователей", "{$this->ipsclass->form_code}".$page_query, "redirect" );
	}
	...
	function member_password_start()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Изменение пароля пользователя: {$member['members_display_name']}" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<strong>Введите новый пароль</strong>" ,
												  			     $this->ipsclass->adskin->form_input('password' ),
									     			    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Создать новый кеш пароля?</b><div style='color:gray'>При выборе «Да», будет создан новый кеш пароля.<br />Помогает в тех случаях, когда пользователь имеет проблемы со входом в систему.</div>" ,
												  				 $this->ipsclass->adskin->form_yes_no( "newsalt", 1 )
									     				)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Создать новый ключ доступа?</b><div style='color:gray'>При выборе «Да», будут созданы новые Cookie для этого пользователя.<br />Помогает в тех случаях, когда пользователь имеет проблемы со входом в систему.<br />Текущие Cookie пользователя работать не будут.</div>" ,
		...
		$this->ipsclass->admin->nav[] 		= array( '', 'Изменение пароля пользователя' );
	...
	{
		$this->ipsclass->admin->page_title = "Блокировка пользователя";
		$this->ipsclass->admin->page_detail = "Временная блокировка пользователя. Просто укажите продолжительность блокировки и нажмите «Заблокировать».";
		$this->ipsclass->admin->nav[] 		= array( '', 'Временная блокировка пользователя' );

		$contents = "{membername},\nАдминистратор форума {$this->ipsclass->vars['board_name']} временно заблокировал вашу учетную запись\n\nВаша учетная запись будет активна с {date_end} (в зависимости от вашего часового пояса). Это полностью автоматический процесс блокировки и от вас не требуется ничего делать для его ускорения или прекращения.\n\nФорум: {$this->ipsclass->vars['board_url']}/index.php";

		if ($this->ipsclass->input['mid'] == "")
		{
			$this->ipsclass->admin->error("Вы не указали ID пользователя.");
		}
		...
		{
			$this->ipsclass->admin->error("Вы должны ввести правильный ID пользователя, вернитесь и попробуйте еще раз");
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Блокировка учетной записи пользователя", "Примечание: Если этот пользователь был уже заблокирован, то отсчет времени новой блокировки перекроет старую (отсчет времени начнется с текущего момента)." );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<strong>Заблокировать {$member['members_display_name']} на...</strong>" ,
												                 $this->ipsclass->adskin->form_input('timespan', $ban['timespan'], "text", "", '5' ) . '&nbsp;' . $this->ipsclass->adskin->form_dropdown('units', $units, $ban['units'] ),
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Предупредить пользователя по e-mail?</b><br />(Если что, то Вы можете отредактировать текст сообщения ниже)" ,
												                 $this->ipsclass->adskin->form_yes_no( "send_email", 0 )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Содержимое письма</b><br />(Доступные теги: {membername} = имя пользователя, {date_end} = дата и время окончания блокировки)" ,
												                 $this->ipsclass->adskin->form_textarea( "email_contents", $contents )
									                    ), "", 'top'       );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Заблокировать");
	...
	{
		$this->ipsclass->admin->page_title = "Блокировка учетной записи пользователя";

		$this->ipsclass->admin->page_detail = "Автоматическая блокировка учетной записи пользователя. Дополнительная информация";
		...
		if ( ! $member = $this->ipsclass->DB->fetch_row() )
		{
			$this->ipsclass->admin->error("Невозможно найти пользователя с таким ID");
		}
		...
		if ($this->ipsclass->input['send_email'] == 1)
		{
			...
			$this->email->subject = "Уведомление о блокировке учетной записи";
		...
		$this->ipsclass->admin->save_log("Заблокирован пользователь ( {$member['members_display_name']} )");

		$this->ipsclass->admin->done_screen("Пользователь заблокирован", "Поиск пользователей", "{$this->ipsclass->form_code}".$page_query, "redirect" );
	}
	...
	function member_unsuspend()
	{
		...
		if ($this->ipsclass->input['mid'] == 'all')
		{
			...
			$this->ipsclass->admin->save_log("Все пользователи разблокированы");

			$msg = "Все пользователи разблокированы";
		}
		else
		{
			...
			$this->ipsclass->admin->save_log("Пользователь {$member['members_display_name']} разблокирован");

			$msg = "{$member['members_display_name']} разблокирован";
		}
		...
		$this->ipsclass->admin->done_screen($msg, "Поиск пользователей", "{$this->ipsclass->form_code}".$page_query, "redirect" );
	}
	...
	function member_change_display_name_do()
	{
		...
		{
			$this->member_change_name_start("Вы должны ввести новое имя для этого пользователя");
			exit();
		}
		...
		{
			$this->member_change_display_name("Новое имя пользователя аналогично старому, вернитесь назад и введите другое");
			exit();
		}
		...
		if ( ! $display_name )
		{
			$this->member_change_display_name("Вы должны ввести имя");
			exit();
		}
		...
		{
			$this->member_change_display_name("В новом имени есть запрещенные символы");
			exit();
		}
    	...
    	if ( $found_name )
    	{
    		$this->member_change_display_name("Это имя уже используется другим пользователем");
			exit();
    	}
		...
		$this->ipsclass->admin->save_log("Изменено имя пользователя: с '{$member['members_display_name']}' на '$display_name'");
		...
		$this->ipsclass->admin->done_screen("Имя пользователя изменено", "Поиск пользователей", "{$this->ipsclass->form_code}".$this->_generate_page_string_url(), "redirect" );
	}
	...
	function member_change_display_name($message="")
	{
		...
		$this->ipsclass->admin->page_title  = "Изменение отображаемого имени";
		$this->ipsclass->admin->page_detail = "В этой секции вы можете изменить текущее имя пользователя.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Изменение отображаемого имени' );
		...
		$contents = "Уважаемый(ая) {old_name},\nАдминистратор форума {$this->ipsclass->vars['board_name']} изменил ваше имя пользователя.\n\nВаше новое имя пользователя: {new_name}\n\nПожалуйста, запомните его, при следующем входе в систему вам потребуется ввести именно это имя пользователя.\nФорум: {$this->ipsclass->vars['board_url']}/index.php";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Изменение имени пользователя" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Текущее имя пользователя</b>" ,
																			 $member['members_display_name'],
																	)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Новое отображаемое имя</b><div class='desctext'>Запрещенные символы: [ ] | ; &#036;<br />Максимальная длина: {$this->ipsclass->vars['max_user_name_length']} символов.</div>" ,
												                 			$this->ipsclass->adskin->form_input( "new_name", $this->ipsclass->input['new_name'] )
									                    			)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Изменить имя пользователя");
	...
	function member_change_name_complete()
	{
		...
		if ($this->ipsclass->input['mid'] == "")
		{
			$this->ipsclass->admin->error("Вы не выбрали пользователя");
		}

		if ($this->ipsclass->input['new_name'] == "")
		{
			$this->member_change_name_start("Вы должны ввести имя для этого пользователя");
			exit();
		}
		...
		{
			$this->member_change_name_start("Новое имя, точно такое же, как и старое, так не должно быть");
			exit();
		}
		...
		if ( $this->ipsclass->DB->get_num_rows() )
		{
			...
			if( $check['id'] != $member['id'] )
			{
				$this->member_change_name_start("Пользователь с именем '$new_name' уже существует, пожалуйста, выберите другое имя");
				exit();
			}
		}
		...
		if ( $this->ipsclass->vars['auth_allow_dnames'] )
		{
			if ( $this->ipsclass->vars['auth_dnames_nologinname'] )
			{
				...
				if ( $this->ipsclass->DB->get_num_rows() )
				{
					$this->member_change_name_start("Пользователь с отображаемым именем '$new_name' уже существует, пожалуйста, выберите другое отображаемое имя");
					exit();
				}
			}
		}		
		...
		if ($this->ipsclass->input['send_email'] == 1)
		{
			...
			$this->email->subject = "Уведомление пользователю о смене его имени";
			$this->email->to      = $member['email'];
			$this->email->send_mail();
		}

		$this->ipsclass->admin->save_log("Изменить имя: с '{$member['name']}' на '$new_name'");
		...
		$this->ipsclass->admin->done_screen("Имя пользователя изменено", "Поиск пользователей", "{$this->ipsclass->form_code}".$page_query, "redirect" );
	}
	...
	{
		$this->ipsclass->admin->page_title = "Изменение имени пользователя";
		$this->ipsclass->admin->page_detail = "Здесь вы можете изменить имя этому пользователю.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Изменение имени пользователя' );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Новое имя пользователя</b>" ,
												                 $this->ipsclass->adskin->form_input( "new_name", $this->ipsclass->input['new_name'] )
									                    )      );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Содержимое письма</b><br />(Доступные теги: {old_name} = текущее имя пользователя, {new_name} = новое имя пользователя)" ,
												                 $this->ipsclass->adskin->form_textarea( "email_contents", $contents )
									                    )      );
	...
	{
		$this->ipsclass->admin->page_title = "Управление званиями";
		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам изменять, удалять и добавлять новые звания.<br />Если вы хотите, чтобы форум показывал звездочки под именем пользователя, введите их количество.<br />Если вы хотите, чтобы форум показывал специальное изображение под именем пользователя, достаточно ввести ссылку с названием изображения.<br />Примечание: эти изображения должны находиться в директории «style_images/{img_dir}/folder_team_icons».";
		$this->ipsclass->admin->nav[] = array( '', 'Звания пользователей' );

		//-----------------------------------------

		$this->ipsclass->adskin->td_header[] = array( "Название"      , "30%" );
		$this->ipsclass->adskin->td_header[] = array( "Требуется сообщений"  , "10%" );
		$this->ipsclass->adskin->td_header[] = array( "Звездочки"       , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Изменение"     , "20%" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Звания пользователей" );
		...
		while ( $r = $this->ipsclass->DB->fetch_row() )
		{
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>".$r['title']."</b>" ,
																	 $r['posts'],
																	 $img,
																	 "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=rank_edit&id={$r['id']}'>Изменить</a>",
																	 "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=rank_delete&id={$r['id']}'>Удалить</a>",
															)      );
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Добавление звания" );


		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Звание</b>" ,
												  $this->ipsclass->adskin->form_input( "title" )
									     )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Минимальное количество сообщений для получения звания</b>" ,
												  $this->ipsclass->adskin->form_input( "posts" )
									     )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Количество звездочек</b><div class='graytext'>Или название - изображение должно быть загружено в директорию 'style_images/{img_dir}/folder_team_icons'</div>" ,
	...
	function titles_add_rank()
	{
		...
		foreach( array( 'posts', 'title', 'pips' ) as $field )
		{
			...
			{
				$this->ipsclass->admin->error("Вы должностью полностью заполнить форму");
			}
		}
		...
		$this->ipsclass->admin->done_screen("Звание добавлено", "Управление званиями", "{$this->ipsclass->form_code}&code=title", 'redirect' );
	...
	function titles_delete_rank()
	{
		...
		{
			$this->ipsclass->admin->error("Невозможно найти звание с таким ID");
		}
		...
		$this->ipsclass->admin->save_log("Удалено звание");

		$this->ipsclass->admin->done_screen("Звание удалено", "Управление званиями", "{$this->ipsclass->form_code}&code=title", 'redirect' );
	...
	function titles_edit_rank()
	{
		...
		if ( intval( $this->ipsclass->input['pips'] ) == $this->ipsclass->input['pips'] )
		{
			...
			{
				$this->ipsclass->admin->error( "Максимальное количество звездочек — 100." );
			}
		}
		...
		$this->ipsclass->admin->save_log("Изменено звание");

		$this->ipsclass->admin->done_screen("Звание изменено", "Управление званиями", "{$this->ipsclass->form_code}&code=title", 'redirect' );
	}
	...
	{
		$this->ipsclass->admin->page_title = "Добавление/изменение звания";
		$this->ipsclass->admin->page_detail = "Если вы хотите, чтобы форум показывал звездочки под именем пользователя, введите их количество.<br />Если вы хотите, чтобы форум показывал специальное изображение под именем пользователя, достаточно ввести ссылку с названием изображения.<br />Примечание: эти изображения должны находиться в директории 'style_images/{img_dir}/folder_team_icons'.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Добавление/изменение звания' );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Количество звездочек (максимум: 100)</b><br />(Или изображение звания)" ,
	...
	{
		$this->ipsclass->admin->nav[] = array( '', 'Удаление' );
		...
		$this->ipsclass->admin->page_detail = "Пожалуйста, подтвердите ваше действие.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Количество удаляемых пользователей</b>" ,
												  				 count($ids)
									     				)      );

		if ( count($member_arr) > 0 )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Имена пользователей</b>" ,
												    		  implode( '<br />', $member_arr )
											                )      );
		}

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Удалить этих пользователей");
	...
	function member_doprune()
	{
		...
		if ($query == "")
		{
			$this->ipsclass->admin->error("Ошибка удаления");
		}
		...
		{
			$this->ipsclass->admin->error("Невозможно найти пользователей для удаления");
		}

		$this->member_delete_do($ids);

		$this->ipsclass->admin->done_screen("Пользователи удалены", "Управление пользователями", "{$this->ipsclass->form_code}" );
	...
	function member_delete_do($id)
	{
		...
		if ( ! count( $tmp_mids ) )
		{
			$this->ipsclass->admin->error("Нет пользователей для удаления");
		}
	...
	function member_add_form()
	{
		...
		$this->ipsclass->admin->page_title  = "Регистрация пользователя";
		$this->ipsclass->admin->page_detail = "Вы можете зарегистрировать пользователя, пользуясь формой ниже.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Создание пользователя' );
		...
		{
			$this->ipsclass->html .= "<div class='input-warn-content' style='color:black'><strong>Помните: Вы имеете ограниченный доступ в АЦ.</strong><br />Любой пользователь, которого вы
									  создадите с доступом в админцентр, унаследует ваши ограничения.</div><br />";
		}
    	...
    	{
    		$custom_out .= $this->ipsclass->adskin->add_td_basic( "Дополнительные поля в профиле", "left", "tablesubheader", 2 );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Создание пользователя" );


		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Имя пользователя</b>" ,
																			 $this->ipsclass->adskin->form_input( "name", isset($_POST['name']) ? $this->ipsclass->txt_stripslashes($_POST['name']) : '' )
																	)      );

		if( $this->ipsclass->vars['auth_allow_dnames'] )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Отображаемое имя</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Пароль</b>" ,
																			  $this->ipsclass->adskin->form_input( "password", isset($_POST['password']) ? $_POST['password'] : '', 'password' )
									     							)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Адрес e-mail</b>" ,
												  							$this->ipsclass->adskin->form_input( "email", isset($_POST['email']) ? $_POST['email'] : '' )
									     							)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Группа</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Создать");
	...
	function member_do_add()
	{
		...
		foreach( array('name', 'password', 'email', 'mgroup') as $field )
		{
			if ( ! $_POST[ $field ] )
			{
				$this->ipsclass->admin->error("Вы должны заполнить форму полностью!");
			}
		}
		...
		if ( $this->ipsclass->vars['admin_group'] == $this->ipsclass->input['mgroup'] )
		{
			if ($this->ipsclass->member['mgroup'] != $this->ipsclass->vars['admin_group'])
			{
				$this->ipsclass->admin->error("Только главные администраторы могут создать пользователя с такой группой");
			}
		}

		if( preg_match( "#[\;\#\n\r\*\'\"<>&\%\!\(\)\{\}\[\]\?\\/\s]#", $in_email) )
		{
			$this->ipsclass->main_msg = "E-mail адрес не может содержать следующие символы: [ ] ; # & ! * ' &quot; &lt; &gt; % ( ) { } ? &#092;";
			$this->member_add_form();
			return;
		}
		else
		{
			...
			{
				$this->ipsclass->main_msg = "Вы ввели некорректный e-mail адрес.";
				$this->member_add_form();
				return;
			}
		}
		...
		{
			$this->ipsclass->main_msg = "Отображаемое имя пользователя не может состоять из следующих символов: [ ] ; , |";
			$this->member_add_form();
			return;
		}
		...
		if( $this->ipsclass->vars['username_characters'] )
		{
			...
			{
				$this->ipsclass->main_msg = 'Имя пользователя может содержать только следующие символы:' . ' ' . $this->ipsclass->vars['username_characters'];
				$this->member_add_form();
				return;
			}
			...
			{
				$this->ipsclass->main_msg = 'Отображаемое имя может содержать только следующие символы:' . ' ' . $this->ipsclass->vars['username_characters'];
				$this->member_add_form();
				return;
			}
		}
		...
		if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
		{
			...
			{
				$this->ipsclass->main_msg = "Введеный e-mail адрес уже используется.";
				$this->member_add_form();
				return;
			}
		}
		...
		if ( $this->ipsclass->DB->get_num_rows() )
		{
			$this->ipsclass->main_msg = "Уже есть пользователь с таким именем, введите другое имя пользователя";
			$this->member_add_form();
			return;
		}
		...
		if( $members_display_name )
		{
			if ( $this->ipsclass->vars['auth_dnames_nologinname'] )
			{
				...
				if ( $this->ipsclass->DB->get_num_rows() )
				{
					$this->ipsclass->main_msg = "Мы уже имеем пользователя, который использует это имя, как отображаемое имя, пожалуйста выберите другое";
					$this->member_add_form();
					return;
				}
			}
		...
		if( $members_display_name )
		{
			...
			if ( $this->ipsclass->vars['auth_dnames_nologinname'] )
			{
				...
				if ( $this->ipsclass->DB->get_num_rows() )
				{
					$this->ipsclass->main_msg = "В базе данных уже есть пользователь, использующий это имя как отображаемое.";
					$this->member_add_form();
					return;
				}
			}
		...
		{
			$this->ipsclass->main_msg = "Имя пользователя не может быть больше 32 символов.";
			$this->member_add_form();
			return;
		}
		...
		if ($email_check['id'])
		{
			$this->ipsclass->main_msg = "Уже есть пользователь с таким e-mail адресом, введите другой e-mail";
			$this->member_add_form();
			return;
		}
		...
		if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
		{
			...
			if ( count( $this->api_server->errors ) OR $this->api_server->params['response'] != 'SUCCESS' )
			{
				$this->ipsclass->main_msg = "Пользователь не добавлен.<br />" . implode( "<br />", $this->api_server->errors );
				$this->member_add_form();
				return;
			}
		}
		...
		$this->ipsclass->admin->save_log("Добавлен новый пользователь '{$this->ipsclass->input['name']}'");
	...
	function search_form()
	{
		...
		$this->ipsclass->admin->page_title  = "Поиск пользователей";
		$this->ipsclass->admin->page_detail = "Воспользуйтесь формой ниже, чтобы найти пользователя или пользователей с конкретными параметрами.<br />Если вы не заполните ни одно поле, то результатом поиска будет вывод всех пользователей вашего форума.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Поиск пользователей" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Имя пользователя</b><div class='graytext'>Вы можете оставить это поле пустым, если используете опции, представленные ниже.</div>",
																			 $this->ipsclass->adskin->form_dropdown( 'namewhere', array( 0 => array( 'begin'   , 'Начинается с' ),
																																		 1 => array( 'is'      , 'Совпадает с'          ),
																																		 2 => array( 'contains', 'Содержит'    ),
																																		 3 => array( 'ends'    , 'Заканчивается на'   )
		...
		if ( $this->ipsclass->vars['auth_allow_dnames'] )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b><u>ИЛИ</u> Отображаемое имя</b><div class='graytext'>Вы можете оставить это поле пустым, если используете опции, представленные ниже</div>",
																			 $this->ipsclass->adskin->form_dropdown( 'dnamewhere', array(   0 => array( 'begin'   , 'Начинается с' ),
																			                                                                1 => array( 'is'      , 'Совпадает с'          ),
																																		    2 => array( 'contains', 'Содержит'    ),
																																		    3 => array( 'ends'    , 'Заканчивается на'   )
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b><u>ИЛИ</u> ID пользователя</b>" ,
												                 $this->ipsclass->adskin->form_input( "memberid", isset($_POST['mid']) ? $_POST['mid'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Тип поиска:</b>" ,
												                 $this->ipsclass->adskin->form_dropdown( "searchtype", array( 0 => array( 'normal', 'Найти пользователей для изменения или удаления' ),
												                 													    1 => array( 'prune' , 'Поиск пользователей для массового удаления' )
												                 													  ), isset($_POST['searchtype']) ? $_POST['searchtype'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic( "Дополнительные опции для поиска", "left", "tablesubheader" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Адрес e-mail содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "email", isset($_POST['email']) ? $_POST['email'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Пользователь заблокирован?</b>" ,
												                 $this->ipsclass->adskin->form_dropdown( "suspended", array( 0=>array('0','Не важно'),1=>array('yes', 'Да'),2=>array('no', 'Нет') ), isset($_POST['suspended']) ? $_POST['suspended'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>IP-адрес содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "ip_address", isset($_POST['ip_address']) ? $_POST['ip_address'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Идентификатор AIM содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "aim_name", isset($_POST['aim_name']) ? $_POST['aim_name'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Номер ICQ содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "icq_number", isset($_POST['icq_number']) ? $_POST['icq_number'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Идентификатор Yahoo! содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "yahoo", isset($_POST['yahoo']) ? $_POST['yahoo'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Подпись содержит</b>" ,
												                 $this->ipsclass->adskin->form_input( "signature", isset($_POST['signature']) ? $_POST['signature'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Количество сообщений меньше, чем:</b>" ,
												                 $this->ipsclass->adskin->form_input( "posts", isset($_POST['posts']) ? $_POST['posts'] : '' )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Зарегистрирован между</b><div class='graytext'>Формат дат: ДД.ММ.ГГГГ<br />Пример: 31.12.1983 (31 декабря 1983 г.)<br />Оставьте первое поле пустым для поиска с самой ранней регистрации, или последнее - до самой последней регистрации.</div>",
												                 $this->ipsclass->adskin->form_simple_input( "registered_first", isset($_POST['registered_first']) ? $_POST['registered_first'] : '', 10 ). ' до ' .$this->ipsclass->adskin->form_simple_input( "registered_last", isset($_POST['registered_last']) ? $_POST['registered_last'] : '', 10 )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Последнее сообщение между</b><div class='graytext'>Формат дат: ДД.ММ.ГГГГ<br />Пример: 31.12.1983 (31 декабря 1983 г.)<br />Оставьте первое поле пустым для поиска с самого первого сообщения, или последнее - до самого последнего сообщения.</div>" ,
												                 $this->ipsclass->adskin->form_simple_input( "last_post_first", isset($_POST['last_post_first']) ? $_POST['last_post_first'] : '', 10 ). ' до ' . $this->ipsclass->adskin->form_simple_input( "last_post_last", isset($_POST['last_post_last']) ? $_POST['last_post_last'] : '', 10 )
									                    )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Последнее посещение между</b><div class='graytext'>Формат дат: ДД.ММ.ГГГГ<br />Пример: 31.12.1983 (31 декабря 1983 г.)<br />Оставьте первое поле пустым для поиска с самого раннего посещения, или последнее - до самого последнего.</div>" ,
												                 $this->ipsclass->adskin->form_simple_input( "last_activity_first", isset($_POST['last_activity_first']) ? $_POST['last_activity_first'] : '', 10 ). ' до ' . $this->ipsclass->adskin->form_simple_input( "last_activity_last", isset($_POST['last_activity_last']) ? $_POST['last_activity_last'] : '', 10 )
									                    )      );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Вторичная группа</b>" ,
    	...
    	if ( count( $fields->out_fields ) )
    	{
    		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic( "Дополнительные поля", "left", "tablesubheader" );
			...
			foreach( $fields->out_fields as $id => $data )
			{
				if ( $fields->cache_data[ $id ]['pf_type'] == 'drop' )
				{
					$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>".$fields->field_names[ $id ]."</b>" ,
												                			 "<select class='dropdown' name='cm_field_{$id}'><option value=''>Не важно...</option>{$data}</select>"
									                    			)      );
				}
				else
				{
	...
	{
		$this->ipsclass->admin->nav[] = array( '', 'Результаты поиска' );
		...
		foreach( array('name', 'members_display_name', 'memberid', 'email','ip_address','aim_name','icq_number','yahoo','signature','posts','suspended', 'registered_first', 'registered_last','last_post_first', 'last_post_last', 'last_activity_first', 'last_activity_last','mgroup','mgroup_others') as $bit )
		{
			...
			if ( $this->ipsclass->input[ $bit ] )
			{
				...
				if ( in_array( $bit, $date_keys ) )
				{
					...
					{
						$this->ipsclass->main_msg = "Неверная дата (День: $day, Месяц: $month, Год: $year). Формат даты должен иметь вид: ДД.ММ.ГГГГ. Например: 31.12.1983 (31 декабря 1983 г.)";
						$this->search_form();
					}
		...
		if ($count['count'] < 1)
		{
			$this->ipsclass->main_msg = "Не найдено ни одной записи, соответствующей вашему запросу.";
			$this->search_form();
		}
		...
		$this->ipsclass->admin->page_title = "Управление пользователями";

		$this->ipsclass->admin->page_detail = "Результаты поиска с заданным запросом.";
		...
		$pages = $this->ipsclass->adskin->build_pagelinks( array( 'TOTAL_POSS'  => $count['count'],
														  ...
														  'L_SINGLE'    => $un_all."Одна страница",
														  'L_MULTI'     => $un_all."Множество страниц",
		...
		$this->ipsclass->html .= "
							<div class='tableborder'>
							 <div class='tableheaderalt'>Результаты поиска — пользователей найдено: {$count['count']}</div>
		...
		while ( $r = $this->ipsclass->DB->fetch_row() )
		{
			...
			if ( ! $r['temp_ban'] )
			{
 				$suspend_html = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=banmember&mid={$r['id']}{$page_query}' title='Временная блокировка учетной записи пользователя'>Временная блокировка</a>";
			}
			else
			{
				$s_ban        = $this->ipsclass->hdl_ban_line( $r['temp_ban'] );
				$suspend_html = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=unsuspend&mid={$r['id']}{$page_query}'>Разблокировать пользователя (".$this->ipsclass->admin->get_date( $s_ban['date_end'], 'LONG') .$sus_link.")...</a>";
			}
						...
						<td width='{$td_width}%' align='left' style='background-color:#F1F1F1;padding:6px;'>
						  <fieldset>
						  	...
						  	<tr>
						  	 ...
						  	 <td width='99%'>
						  	  <a style='font-size:12px;font-weight:bold' title='Просмотр профиля пользователя' href='{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?showuser={$r['id']}' target='blank'>{$r['members_display_name']}</a>
						  	...
						  	<tr>
						  	 ...
						  	 <td width='99%'><strong>{$this->ipsclass->cache['group_cache'][$r['mgroup']]['g_title']}</strong> <span style='font-size:10px' class='graytext'>(сообщений: {$r['posts']})</span></td>
						  	</tr>
						  	<tr>
						  	 <td width='1%' align='center'><img src='{$this->ipsclass->skin_acp_url}/images/memsearch_posts.gif' border='0' /><td>
						  	 <td width='99%'><strong>Зарегистрировался: {$joined}</strong></td>
						  	</tr>
						  	</table>
						  	</div>
						  </fieldset>
...
EOF;
			...
			{
				$people .= "new Array( 'Вы не можете редактировать администраторов' ) );</script></td>";
			}
			else
			{
				$people .= <<<EOF
							new Array( img_edit + " <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=doform&mid={$r['id']}{$page_query}'>Профиль</a>",
						  			  img_edit + " <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=changename&mid={$r['id']}{$page_query}'>Имя пользователя</a>",
EOF;
				if ( $this->ipsclass->vars['auth_allow_dnames'] )
				{
					$people .= <<<EOF
										 img_edit + " <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=change_display_name&mid={$r['id']}{$page_query}'>Отображаемое имя</a>",
EOF;
				}

				$people .= <<<EOF
						  				 img_password + " <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=changepassword&id={$r['id']}{$page_query}'>Пароль</a>",
						  			     img_action + " <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=deleteposts&mid={$r['id']}{$page_query}'>Удалить все сообщения и темы</a>",
...
EOF;
	...
	function member_do_edit_form()
	{
		...
		if ( ! $mem['id'] )
		{
			$this->ipsclass->admin->error("Невозможно найти пользователя с указанным ID.");
		}
		...
		$this->ipsclass->admin->nav[] = array( '', 'Изменение пользователя' );
		...
		{
			$form['_custom_fields'] = $this->ipsclass->adskin->add_td_row( array( array( "<b>Дополнительные поля не найдены.", 2 ) ) );
		}
		...
		$this->ipsclass->admin->page_title = "Изменение пользователя «".$mem['members_display_name']."» (ID: ".$mem['id'].")";

		$this->ipsclass->admin->page_detail = "Здесь вы сможете изменить основную информацию и настройки пользователя.";
		...
		elseif ($mem['mod_posts'] > 0)
		{
			...
			$form['_mod_extra'] = "<br /><span style='color:red'>Используется ограничение — оставшееся время было пересчитано</span>";
		}
		...
		else if( $mem['restrict_post'] > 0 )
		{
			...
			$form['_post_extra'] = "<br /><span style='color:red'>Используется ограничение — оставшееся время было пересчитано</span>";
		}
		...
		$form['pp_gender']      = $this->ipsclass->adskin->form_dropdown( "pp_gender", array( 0 => array('none', 'Не указан'), 1 => array( 'female', 'Женщина'), 2 => array( 'male', 'Мужчина' ) ), $mem['pp_gender'] );
		...
		$form['avatar_type'] = $this->ipsclass->adskin->form_dropdown("avatar_type", array( 0 => array( 'local'  , 'из галереи'  ),
																	  						1 => array( 'url'    , 'внешняя ссылка'      ),
																							2 => array( 'upload' , 'загружен на сервер' ),
																						  ), $mem['avatar_type'] );
	...
	function member_do_edit()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы должны ввести настоящий e-mail адрес.";
			$this->member_do_edit_form();
		}
		...
		if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
		{
			...
			if ( ! $converge['converge_active'] )
			{
				$this->ipsclass->main_msg = "Действие не выполнено, поскольку невозможно найти сведений о API IP.Convege.";
				$this->member_do_edit_form();
			}
		}
		...
		if ( $this->ipsclass->input['email'] != $this->ipsclass->input['curemail'] )
		{
			...
			if ($email_check['id'])
			{
				$this->ipsclass->main_msg = "Уже есть пользователь с таким E-mail, введите другой E-mail";
				$this->member_do_edit_form();
			}
			...
			if ( $this->ipsclass->vars['ipbli_key'] == 'converge' )
			{
				...
				if ( count( $this->api_server->errors ) OR $this->api_server->params['response'] != 'EMAIL_NOT_IN_USE' )
				{
					$this->ipsclass->main_msg = "Указанный e-mail адрес уже используется.";
					$this->member_do_edit_form();
					return;
				}
		...
		$this->ipsclass->admin->save_log("Изменение учетной записи пользователя «{$memb['members_display_name']}»");

		$this->ipsclass->main_msg = "Пользователь изменен";
