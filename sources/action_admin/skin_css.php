	...
	*/
	...
	function auto_run()
	{
		...
		switch($this->ipsclass->input['code'])
		{
			...
			default:
				print "Действия не выбраны"; exit();
				break;
	...
	function do_resynch()
	{
		...
		{
			$this->ipsclass->admin->error("Нельзя получить CSS из базы данных");
		}
		...
		if ( $this->ipsclass->input['favour'] == 'cache' )
		{
			...
			{
				$this->ipsclass->admin->error("Нельзя найти Кеш CSS файла @ $cache_file");
			}
	...
	function resync_splash($db_length, $cache_length, $cache_mtime, $db_mtime, $id, $return="")
	{
		...
		$this->ipsclass->admin->page_detail = "Найдено несоответствие между кеш стилем страницы и таблицой стилей сохраненной в базе данных.";
		$this->ipsclass->admin->page_title  = " Синхронизация таблицы стилей";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Синхранизировать CSS перед его редактированием..." );
		
		if ( intval($cache_mtime) > intval($db_mtime) )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
														"<b>CSS в базе данных, последнее обновление:</b> ".$this->ipsclass->admin->get_date($db_mtime, 'LONG'),
														"<b>CSS в базе данных, кол-во символов:</b> $db_length",
											 )      );
											 
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
														"<span style='color:red'><b>CSS в кеше, последнее обновление:</b> ".$this->ipsclass->admin->get_date($cache_mtime, 'LONG')."</span>",
														"<span style='color:red'><b>CSS в кеше, кол-во символов:</b> $cache_length</span>",
											 )      );
			$favour = 'cache';
		...
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
														"<span style='color:red'><b>CSS в базе данных, последнее обновление:</b> ".$this->ipsclass->admin->get_date($db_mtime, 'LONG')."</span>",
														"<span style='color:red'><b>CSS в базе данных, кол-во символов:</b> $db_length</span>",
											 )      );
											 
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
														"<b>CSS в кеше, последнее обновление:</b> ".$this->ipsclass->admin->get_date($cache_mtime, 'LONG'),
														"<b>CSS в кеше, кол-во символов:</b> $cache_length",
											 )      );
		}
		
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
														"<b>Использовать синхронизация....</b>",
														$this->ipsclass->adskin->form_dropdown( 'favour', array(
																							    0 => array( 'cache', 'Переписать версию базы данных с кеш версией'),
																							    1 => array( 'db'   ,  'Обновить версию кеша из базы данных' ),
																							 ), $favour ),
											 )      );
		
		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Синхронизировать");
	...
	function optimize()
	{
		if ($this->ipsclass->input['id'] == "")
		{
			$this->ipsclass->admin->error("Вы должны уточнить текущий CSS ID, вернитесь и повторите заново");
		}
		...
		{
			$this->ipsclass->admin->error("Невозможно запросить информацию из базы данных");
		}
		...
		if ( count($parsed) < 1)
		{
			$this->ipsclass->admin->error("Таблицы стиля находится в формате который не понимает Invision Power Board, оптимизация не произведена.");
		}
		...
		$this->ipsclass->admin->done_screen("Таблица стилей обновлена: Изменения сохранены: $saved ($pc_saved %)", "Настройка таблицы стилей", "{$this->ipsclass->form_code}" );
	}
	...
	function save_css( $type='add' )
	{
		...
		{
			$this->ipsclass->admin->error("Ваша таблица стилей не должна быть пустой");
		}
		...
		$extra = "<b>Файл кеша таблицы стилей обновлен</b>";
		...
		{
			$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code ,'Управление стилем' );
			$this->ipsclass->main_msg = "Таблица стилей обновлена: $extra";
			$this->ipsclass->admin->redirect( $this->ipsclass->base_url.'&section='.$this->ipsclass->section_code.'&act=sets', "Таблица стилей обновлена, вернитесь к управлению скином" );
		}
		else
		{
			//-----------------------------------------
	...
	function do_form( $type='add' )
	{
		...
		if ( $this->ipsclass->input['id'] == "")
		{
			$this->ipsclass->admin->error("Вы должны уточнить ID текущего шаблона, вернитесь и повторите заново");
		}
		...
		$this->ipsclass->admin->page_detail = "Вы можете полностью использовать CSS когда добавляете или редактируете таблицу стилей.";
		$this->ipsclass->admin->page_title  = " Настройка таблицы стилей";
		...
		$formbuttons = "<div align='center' class='tablesubheader'>
						...
						<input type='submit' name='savereload' value='Сохранить и перезагрузить' class='realdarkbutton'>
						</div></form>\n";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Информация об использовании CSS классов" );
		
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( 
																			   "Показать, где в шаблонах используется: ",
																			   $this->ipsclass->adskin->form_dropdown('csschoice', $css_elements).'&nbsp;'
																			  .'<input type="button" value="Показать!" onClick="cssSearch(\''.$this->ipsclass->input['id'].'\');" id="editbutton">'
																			  .'&nbsp;<input type="button" value="Предпросмотр стиля" onClick="cssPreview(\''.$this->ipsclass->input['id'].'\');" id="editbutton">'
																	)      );
		...
		$this->ipsclass->admin->nav[] = array( 'section='.$this->ipsclass->section_code.'&act=sets' ,'Настройки стиля' );
		$this->ipsclass->admin->nav[] = array( '' ,'Редактирование таблицей стилей в '.$this_set['set_name'] );
	...
	function colouredit()
	{
		...
		if ($this->ipsclass->input['id'] == "")
		{
			$this->ipsclass->admin->error("Вы должны уточнить ID текущего шаблона, вернитесь и повторите попытку снова");
		}
		...
		if ( count($colours) < 1 )
		{
			$this->ipsclass->admin->error("CSS работает не надежно! Цвета для редактирования не выбраны");
		}
		...
		$this->ipsclass->admin->page_title  = " Настройка таблицы стилей [Цвета]";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Цвета CSS" );
								 ...
								 <fieldset>
								  ...
								  <tr>
								   <td width='40%' valign='top'>
								    <fieldset>
								     <legend><strong>Цвет шрифта</strong></legend>
										{$font_box}&nbsp;&nbsp;<input type='text' id='{$md5}color' onclick=\"updatecolor('{$md5}color')\" size='6' style='border:1px solid black;background-color:{$val['color']}' readonly='readonly'>&nbsp;<a href='#' title='настройка цвета' onclick=\"poppicker('{$val['color']}', '{$md5}color'); return false;\"><img src='{$this->ipsclass->skin_acp_url}/images/colorselect.png' border='0' /></a>
									</fieldset>
									<br />
									<fieldset>
									 <legend><strong>Цвет заднего фона</strong></legend>
			 						    {$bgcol_box}&nbsp;&nbsp;<input type='text' id='{$md5}backgroundcolor'  onclick=\"updatecolor('{$md5}backgroundcolor')\" size='6' style='border:1px solid black;background-color:{$val['background-color']}' readonly='readonly'>&nbsp;<a href='#' title='настройка цвета' onclick=\"poppicker('{$val['background-color']}', '{$md5}backgroundcolor'); return false;\"><img src='{$this->ipsclass->skin_acp_url}/images/colorselect.png' border='0' /></a>
			 						</fieldset>
			 					   </td>
			 					   <td width='60%' valign='top'>
			 					   <fieldset>
									 <legend><strong>Другие атрибуты CSS</strong></legend>
	...
	function do_colouredit()
	{
		...
		if ( count($colours) < 1 )
		{
			$this->ipsclass->admin->error("Нет цветов для редактирования");
		}
