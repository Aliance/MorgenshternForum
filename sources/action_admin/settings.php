...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-01 16:46:24 -0600 (Wed, 01 Nov 2006) $
|   > $Revision: 695 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_settings {
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[]       = array( "{$this->ipsclass->form_code}", "Просмотр всех основных настроек" );
	...
	function settings_do_import( $noreturn=0, $use_lite_xml=1 )
	{
		//-----------------------------------------
		// INIT
		//-----------------------------------------
		...
		if ( $_FILES['FILE_UPLOAD']['name'] == "" or ! $_FILES['FILE_UPLOAD']['name'] or ($_FILES['FILE_UPLOAD']['name'] == "none") )
		{
			...
			{
				$this->ipsclass->main_msg = "Не выбрано ни одного файла для загрузки или неправильное название файла.";
				$this->setting_start();
			}

			if ( ! file_exists( ROOT_PATH . $this->ipsclass->input['file_location'] ) )
			{
				$this->ipsclass->main_msg = "Невозможно найти файл, чтобы открыть: " . ROOT_PATH . $this->ipsclass->input['file_location'];
				$this->setting_start();
			}
		...
		if ( ! $content )
		{
			$this->ipsclass->admin->error( "Импортируемый файл имеет неправильный формат или испорчен." );
		}
		...
		$this->ipsclass->main_msg = "$updated настройки обновлены и $inserted добавлены";
	...
	function setting_someexport_complete()
	{
		...
		if ( ! count( $ids ) )
		{
			$this->ipsclass->main_msg = "Вы должны выделить хоть КАКИЕ-НИБУДЬ настройки для экспорта!";
			$this->setting_someexport_start();
		}
	...
	{
		$this->ipsclass->admin->page_title  = "Экспорт выделенных настроек системы";
		$this->ipsclass->admin->page_detail = "Проверьте все поля, которые вы выделили для экспорта настроек";
		$this->ipsclass->admin->nav[]       = array( '', "Настройки экспорта" );
		...
		$this->ipsclass->html .= "<div class='tableborder'>
								  <div class='tableheaderalt'>Системные настройки</div>
		...
		$this->ipsclass->html .= "</table>
						    <div class='tablesubheader' align='center'><input type='submit' class='realbutton' value='Экспортировать' /></form></div></div>";
	...
	function settinggroup_delete()
	{
		if ( $this->ipsclass->input['id'] )
		{
			...
			if ( $count > 0 )
			{
				$this->ipsclass->main_msg = "Нельзя удалить эту группу настроек, так как в ней содержатся активные настройки";
			}
			else
			{
				$this->ipsclass->DB->simple_exec_query( array( 'delete' => 'conf_settings_titles', 'where' => 'conf_title_id='.intval($this->ipsclass->input['id']) ) );

				$this->ipsclass->main_msg = "Группа настроек удалена";
			}
	...
	{
		$this->ipsclass->admin->page_title  = "Настройка конфигурации системы";
		$this->ipsclass->admin->page_detail = "Эта секция содержит все конфигурационные опции для вашего IPB.";
		$this->ipsclass->admin->nav[]       = array( '', "Добавление/изменение настройки" );

		if ( $type == 'add' )
		{
			$formcode = 'settinggroup_add';
			$title    = "Создание новой группы системных настроек";
			$button   = "Создать";
		...
		else
		{
			...
			{
				$this->ipsclass->main_msg = "Нет пригодного ID, попробуйте заново .";
				$this->setting_start();
			}

			$formcode = 'settinggroup_edit';
			$title    = "Редактирование настройки ".$conf['conf_title'];
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Название</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Описание</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Ключевое слово</b><div class='desctext'>Используется для того, чтобы получить из базы данных без ссылки на ID." ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>PHP-модуль</b><div class='desctext'>Введите название PHP-модуля (например, example.php). Указанный файл должен быть загружен в директорию /sources/components_acp/setting_plugins." ,
		...
		if ( IN_DEV )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Скрыть из списка системных настроек?</b>" ,
	...
	function settinggroup_save($type='add')
	{
		...
		{
			$this->ipsclass->main_msg = "Вы должны ввести уникальное ключевое слово для: Ключевые слова для группы";
		...
		if ( $type == 'edit' )
		{
			...
			{
				$this->ipsclass->main_msg = "Вы уже используете это ключевое слово для уже существующих групп";
				$this->settinggroup_form();
				return;
			}
	...
	function setting_form( $type='add' )
	{
		...
		$types = array( 0 => array( 'input'   , 'Ввод текста (одна строка)' ),
						1 => array( 'dropdown', 'Выпадающее меню'  ),
						2 => array( 'yes_no'  , 'Да/нет (единичный выбор ответа)'),
						3 => array( 'textarea', 'Текстовое поле'   ),
						4 => array( 'editor'  , 'Редактор'   ),
						5 => array( 'multi'   , 'Мульти-выбор' ),
					 );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Расположение</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Тип</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Ключ</b>" ,
												  			   $this->ipsclass->adskin->form_input( 'conf_key', ( isset($_POST['conf_key']) AND $_POST['conf_key'] ) ? $_POST['conf_key'] : $conf['conf_key'] )
										 		    	)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Текущее значение</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Значение по умолчанию?</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Дополнительно</b><div style='color:gray'>Используетя для ввода дополнительной информации.<br/>Пример использования для выпадающего меню: <b>key=value</b> (по одному на строку).</div>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Выполнение PHP-кода до отображения или сохранения настройки:</b><div style='color:gray'><b>\$show = 1;</b> до отображения настройки.<br /><b>\$save = 1;</b> до сохранения настройки.<br />Используйте <b>\$key</b> и <b>\$value</b> во время написания PHP-кода.</div>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Начать группу настроек:</b><div style='color:gray'>Введите название или оставьте поле пустым, если вы не хотите начинать новую группу.</div>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Закончить группу настроек?</b><div style='color:gray'>Вы можете закрыть открытую группу.</div>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Ключ помощи (опционально)</b><div style='color:gray'>«Ключ» используется для ссылок к помощи админцентра.</div>" ,
												  			   $this->ipsclass->adskin->form_input( 'conf_help_key', ( isset($_POST['conf_help_key']) AND $_POST['conf_help_key'] ) ? $_POST['conf_help_key'] : $conf['conf_help_key'] )
										 		    	)      );

		if ( IN_DEV )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Сделать настройкой по умолчанию?</b><div style='color:gray'>Пользователи не смогут удалить эту настройку.</div>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Добавить в кеш настроек?</b>" ,
	...
	*/
	function setting_view( $conf_fields=array() )
	{
		...
		$this->ipsclass->admin->page_title  = $this->ipsclass->admin->page_title  ? $this->ipsclass->admin->page_title  : "Настройки системной конфигурации";
		$this->ipsclass->admin->page_detail = $this->ipsclass->admin->page_detail ? $this->ipsclass->admin->page_detail : "Эта секция содержит все конфигурационные настройки системы IP.Board.<br />Если вы желаете оставить запись пустой, пожалуйста используйте ключевую фразу: <b>{blank}</b> или значение: <b>0</b>.";
		...
		else
		{
			...
			{
			$this->ipsclass->main_msg = "Ни одна группа не выбрана, попробуйте снова.";
				$this->setting_start();
			}
			...
			if ( $search_key )
			{
				...
				if ( ! count( $conf_entry ) )
				{
					$this->ipsclass->main_msg = "Не найдено ни одной настройки, соответствующей запросу «{$keywords}».";
					$this->setting_start();
				}
			...
			else
			{
				...
				$this->ipsclass->DB->simple_construct( array( 'select'   => 'c.*',
															  ...
															  'add_join' => array( 0 => array(    'select' => 'cc.conf_title_title, cc.conf_title_module',
				...
				$title = "Список настроек группы «{$group_title}»";
			}
		}
		...
		{
			$html .=  $this->ipsclass->adskin->js_make_button("Добавить настройку"  , $this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=settingnew&conf_group=".$conf_group).'&nbsp;';
			$html .=  "<input type='submit' name='reorder' value='Отсортировать' class='realdarkbutton' />";
		}
		...
		if ( $_show_error )
		{
			$this->ipsclass->html .= $this->ipsclass->skin_acp_global->warning_box( "Ошибка настроек", "Одна или более настроек возвращают ошибку.<br /><strong>Настройки не обновлены.</strong>" ) . "<br />";
		}
		...
		$this->ipsclass->html .= "<div class='tablesubheader' align='center'><input type='submit' value='Обновить настройки' class='realdarkbutton' /></div></div></form>";

		$this->ipsclass->html .= "<br /><br /><div align='right'><b><em>Системные настройки</em></b> ".$this->setting_make_dropdown()."</div>";
		$this->ipsclass->admin->output();
	}
	...
	function _setting_process_entry($r)
	{
		...
		if ( $r['conf_value'] != "" and ( $r['conf_value'] != $r['conf_default'] ) )
		{
			...
			$revert_button = "<div style='width:auto;float:right;padding-top:2px;padding-bottom:3px;'><a href='{$this->ipsclass->base_url}&section=tools&code=op&code=setting_revert&id={$r['conf_id']}&conf_group={$r['conf_group']}&search={$this->ipsclass->input['search']}' title='Вернуть значение по умолчанию'><img src='{$this->ipsclass->skin_acp_url}/images/te_revert.gif' alt='X' border='0' /></a></div>";
		...
		$delete  = "&#0124; <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_delete&id={$r['conf_id']}' title='ключ: {$r['conf_key']}'>Удалить</a>";
		$edit    = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_showedit&id={$r['conf_id']}' title='id: {$r['conf_id']}'>Изменить</a>";
		$reorder = 1;
		...
		if ( $this->ipsclass->input['search'] )
		{
			...
			$r['conf_description'] .= "<br /><br /><i><b>Группа настроек:</b> {$r['conf_title_title']}</i>";
		}

		$html = "$start
							<table cellpadding='5' cellspacing='0' border='0' width='100%'>
							 <tr>
							 <td width='30%' class='$tablerow1'><b>{$r['conf_title']}</b><div style='color:gray'>{$r['conf_description']}</div></td>
	...
	{
		$this->ipsclass->admin->page_title  = "Системные настройки";
		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам изменять системные настройки вашего IPB.";
		...
		if ( IN_DEV )
		{
			...
			if ( $last_settings_save > $last_update['cs_value'] )
			{
				...
				$_html = $this->ipsclass->skin_acp_global->warning_box( "Файл «settings.xml» обновлен",
																		"Файл «resources/settings.xml» был обновлен. Пожалуйста, посетите <a href='{$this->ipsclass->base_url}&amp;section=tools'>эту страницу</a> для повторного импортирования.
																		<br />Дата последнего изменения файла «settings.xml»: $_mtime.
																		<br />Последний запуск импортирования: $_dbtime" ) . "<br />";
		...
		$basic_title = "<table cellpadding='0' cellspacing='0' border='0' width='100%'>
						<tr>
						 <td align='left' width='40%' style='font-size:12px; vertical-align:middle;font-weight:bold; color:#FFF;'>Системные настройки</td>
						 <td align='right' width='60%'><form method='post' action='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_view'><input type='hidden' name='_admin_auth_key' value='{$this->ipsclass->_admin_auth_key}' /><input type='text' size='25' onclick='this.value=\"\"' value='Поиск настроек...' name='search' class='realbutton' />&nbsp;<input type='submit' class='realdarkbutton' value='Найти' /></form>"
						 ."&nbsp;&nbsp;</td>
						</tr>
						</table>";
		...
		foreach( $this->setting_groups as $r )
		{
			...
			if( IN_DEV )
			{
				$in_dev_extra = "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=MOD_export_setting&conf_group={$r['conf_title_id']}' title='Экспортировать группу настроек'><img src='{$this->ipsclass->adskin->img_url}/images/icons_menu/export.png' border='0' alt='Экспортировать' /></a>";
			}
			else
			{
				$in_dev_extra = '';
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<div align='center'><img src='{$this->ipsclass->adskin->img_url}/images/settings_folder.gif' border='0' alt='Директория' /></div>",
																   "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_view&conf_group={$r['conf_title_id']}'><b>{$r['conf_title_title']}</b></a>$hidden <span style='color:gray'>(настроек: ".intval($r['conf_title_count']).")</span><div style='color:gray'>{$r['conf_title_desc']}</div>" ,
												  				   array("<div align='center' style='white-space:nowrap'>
												  				          <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=settinggroup_showedit&id={$r['conf_title_id']}' title='Изменить группу настроек'><img src='{$this->ipsclass->adskin->img_url}/images/icons_menu/edit.gif' border='0' alt='Изменить'  /></a>
																          <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=settinggroup_delete&id={$r['conf_title_id']}' title='Удалить группу настроек'><img src='{$this->ipsclass->adskin->img_url}/images/icons_menu/delete.gif' border='0' alt='Удалить'  /></a>
																          <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=settinggroup_resync&id={$r['conf_title_id']}' title='Пересчитать кол-во настроек в этой группе'><img src='{$this->ipsclass->adskin->img_url}/images/acp_resync.gif' border='0' alt='Пересчитать'  /></a>
																          {$in_dev_extra}
																          </div>", 1, 'tablerow3' )
										 					  )      );
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( array("<div align='center' style='white-space:nowrap'>".
																	  $this->ipsclass->adskin->js_make_button("Добавить группу настроек", $this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=settinggroup_new")."</div>", 3, 'tablesubheader' )
										 					  )      );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Импортирование настроек" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Загрузка файла XML с настройками</b><div style='color:gray'>Этот файл должен начинаться с «ipb_» и иметь расширение «.xml» или «.xml.gz».<br />Существующие настройки будут обновлены, другие — добавлены.</div>" ,
										  				        			 $this->ipsclass->adskin->form_upload(  )
											                        )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b><u>ИЛИ</u> введите название файла</b><div style='color:gray'>Этот файл должен быть загружен в корневую директорию вашего форума.</div>" ,
										  				         			$this->ipsclass->adskin->form_input( 'file_location', 'resources/settings.xml'  )
											                        )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Запуск инструмента</b><div style='color:gray'>Пожалуйста, выберите инструмент, который необходимо запустить.</div>" ,
										  				         			  $this->ipsclass->adskin->form_dropdown( 'tool', array( 0 => array( 'import', 'Импортирование' ), 1 => array( 'check', 'Сравнение' ) )  )
																	)      );
		...
		$this->ipsclass->html .= "<br /><br/ ><div align='center'><a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_someexport_start'>Экспорт настроек</a></div>";

		if ( IN_DEV )
		{
			$this->ipsclass->html .= "<br /><div align='center'>Режим разработчика: <a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_allexport'>Экспорт всех настроек в XML-файл</a></div>";
		}
	...
	function setting_update( $donothing="" )
	{
		...
		{
			$this->ipsclass->main_msg = "Вы должны ввести ID системной настройки";
			$this->setting_start();
		}
		...
		if ( isset($this->ipsclass->input['reorder']) AND $this->ipsclass->input['reorder'] )
		{
			...
			$this->ipsclass->main_msg = "Системные настройки пересортированы";
		...
		if ( ! count($fields ) )
		{
			$this->ipsclass->main_msg = "Нет полей для сохранения";
		...
		$this->ipsclass->main_msg = "Системные настройки обновлены";
	...
	function setting_save($type='add')
	{
        ...
        //-----------------------------------------
		// Added by Artur (ibresource.ru)
		//-----------------------------------------

        //$_POST['conf_description'] = nl2br($_POST['conf_description']);
		...
		$array = array( 'conf_title'       => $this->ipsclass->input['conf_title'],
						...
						'conf_help_key'    => $this->ipsclass->input['conf_help_key'],
		...
		if ( $type == 'add' )
		{
			...
			$this->ipsclass->main_msg = 'Новая системная настройка добавлена';
		...
		else
		{
			...
			$this->ipsclass->main_msg = 'Системная настройка изменена';
		}
	...
	function setting_revert()
	{
		...
		$this->ipsclass->main_msg = "Системная настройка возвращена к первоначальному виду.";
	...
	function setting_delete()
	{
		...
		$this->ipsclass->main_msg = "Системная настройка удалена";
	...
	function setting_make_dropdown()
	{
		...
		$ret = "<form method='post' action='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=setting_view'>
		...
		return $ret."\n</select><input type='submit' id='button' value='Перейти' /></form>";
	}
	...
	function do_fulltext()
	{
		...
		if ( $this->ipsclass->DB->sql_can_fulltext() )
		{
			...
			if ( $result['cnt'] > 15000 )
			{
				...
				$this->ipsclass->admin->page_detail = "";
				$this->ipsclass->admin->page_title  = "Продолжение невозможно";
		...
		{
			$this->ipsclass->admin->error("Извините, ваша версия MySQL не поддерживает поиск по полному тексту");
		}

		$this->ipsclass->admin->save_log("Обновлены настройки поиска по полному тексту");

		$this->ipsclass->admin->done_screen("Индексы поиска по полному тексты обновлены", "Настройки поиска по полному тексту", "{$this->ipsclass->form_code}&code=setting_view&conf_group=19", "redirect" );
	}
	...
	*/
	function save_config( $new )
	{
		...
		$this->ipsclass->admin->save_log("Обновлены настройки форума, Back Up записан");

		$this->ipsclass->admin->done_screen("Конфигурация обновлена", "Начало Админцентра", "act=index" );
	}
...
In mysql type:
<br />
<pre>alter table ibf_topics add fulltext(title);</pre>
<br />
...
In mysql type:
<br />
<pre>alter table ibf_posts add fulltext(post);</pre>
<br />
If you are not using 'ibf_' as your table extension, adjust that query to suit. This query can take a while
