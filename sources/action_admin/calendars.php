...
+---------------------------------------------------------------------------
|   > $Date: 2006-10-06 17:00:51 -0500 (Fri, 06 Oct 2006) $
|   > $Revision: 617 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_calendars
{
	...
	function auto_run() 
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Менеджер календарей' );
	...
	function calendar_delete()
	{
		...
		if ( ! $cal_id )
		{
			$this->ipsclass->main_msg = "Вы не указали ID календаря для удаления.";
		...
		$this->ipsclass->main_msg = "Календарь успешно удален.";
		$this->calendar_list();
	}
	...
	function calendar_move()
	{
		...
		$this->ipsclass->main_msg = "Календарь обновлен.";
		$this->calendar_list();
	}
	...
	function calendar_rss_cache( $calendar_id='all', $return=0 )
	{
					...
					{ 
						if ( ! in_array( $event['event_id'], $seenids ) )
						{
							...
							if ( $calendar->get_info_events( $event, $month, $day, $year, 0 ) )
							{
								...
								$event['event_content'] = $this->parser->pre_db_parse( $event['event_content'] );
								$event['event_content'] = $this->parser->pre_display_parse( $event['event_content'] );
		...
		if ( $return )
		{
			$this->ipsclass->main_msg = "RSS события календаря были перекешованы";
			$this->calendar_list();
		}
		else
		{
	...
	function calendar_rebuildcache( $return=0 )
	{
		...
		if ( $return )
		{
			$this->ipsclass->main_msg = "Кеш событий календаря был успешно обновлен";
			$this->calendar_list();
		}
	}
	...
	function calendar_save($type='add')
	{
		...
		if ( $type == 'edit' )
		{
			...
			{
				$this->ipsclass->main_msg = "Вы не указали ID календаря или его название.";
				$this->calendar_list();
				return;
			}
		}
		else
		{
			if ( ! $cal_title )
			{
				$this->ipsclass->main_msg = "Вы не указали название календаря.";
		...
		if ( $type == 'add' )
		{
			...
			$this->ipsclass->main_msg = 'Новый календарь добавлен.';
		}
		else
		{
			
			$this->ipsclass->DB->do_update( 'cal_calendars', $array, 'cal_id='.$cal_id );
			$this->ipsclass->main_msg = 'Календарь изменен.';
		}
	...
	function calendar_form( $type='add' )
	{
		...
		if ( $type == 'add' )
		{
			...
			$title    = "Добавление нового календаря";
			$button   = "Добавить";
		...
		else
		{
			...
			if ( ! $calendar['cal_id'] )
			{
				$this->ipsclass->main_msg = "Вы не указали ID календаря.";
				$this->components_list();
				return;
			}
			
			$formcode = 'calendar_edit_do';
			$title    = "Редактирование: ".$calendar['cal_title'];
			$button   = "Изменить";
		}
		...
		$this->ipsclass->html_help_title = "Управление календарем";
		$this->ipsclass->html_help_msg   = "Эта секция позволит вам добавить или изменить календарь.";
		
		$this->ipsclass->admin->nav[]    = array( '', "Добавление/изменение календаря" );
	...
	function calendar_list()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление календарями";
		$this->ipsclass->admin->page_detail = "Эта секция позволит вам управлять всеми созданными календарями.";
		$this->ipsclass->admin->output();
	}
