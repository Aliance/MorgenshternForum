...
class ad_task_manager
{
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->page_detail = "Эта секция содержит все ваши установленные задачи.<br />Пожалуйста, учтите, что все эти задачи запускаются только тогда, когда ваш форум доступен посетителям, следующий запуск происходит только при вызове форума.";
		$this->ipsclass->admin->page_title  = "Управление задачами";

		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Список задач' );
	...
	function task_rebuild_xml()
	{
		...
		if ( ! file_exists( $file ) )
		{
			$this->ipsclass->main_msg = "Файл $file не найден.";
			$this->show_intro();
		}
		...
		$this->ipsclass->main_msg = "$inserted задач добалено и $updated обновлено.";
		$this->task_show_tasks();
	}
	...
	function task_log_delete()
	{
		...
		$this->ipsclass->main_msg = 'Выбранные записи выполненных задач удалены';
		$this->task_log_setup();
	}
	...
	function task_log_show()
	{
		...
		$this->ipsclass->adskin->td_header[] = array( "Выполненная задача" , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Дата" , "35%" );
		$this->ipsclass->adskin->td_header[] = array( "Информация", "45%" );


		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Выбранные записи журнала задач" );
	...
	{
		$this->ipsclass->admin->nav[] = array( '', 'Просмотр выполненных задач' );

		//-----------------------------------------
		// Some set up
		//-----------------------------------------

		$tasks = array( 0 => array( -1, 'Все задачи' ) );
		...
		$this->ipsclass->adskin->td_header[] = array( "Выполненная задача" , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Дата Время" , "35%" );
		$this->ipsclass->adskin->td_header[] = array( "Информация" , "45%" );


		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "5 последних запущенных задач" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Просмотр выполненных задач" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Задача</b>",
															  $this->ipsclass->adskin->form_dropdown( 'task_id', $tasks )
													 )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Сколько вывести записей?</b>",
															  $this->ipsclass->adskin->form_input( 'task_count', '30' )
													 )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form('Просмотр');
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Удаление записей" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Задача</b>",
															  $this->ipsclass->adskin->form_dropdown( 'task_id', $tasks )
													 )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Удалить выполненные задачи старше, чем (в днях):</b>",
	...
	function task_unlock()
	{
		...
		$this->ipsclass->main_msg = 'Закрытые задачи удалены';
		$this->task_show_tasks();
	}
	...
	function task_run_task()
	{
		...
		if ( ! $task_id )
		{
			$this->ipsclass->main_msg = 'Не указан ID задачи, действие не выполнено';
			$this->task_show_tasks();
		}
		...
		{
			$this->ipsclass->main_msg = 'Нет задач для выполнения.';
			$this->show_tasks();
		}

		if ( ! $this_task['task_enabled'] )
		{
			$this->ipsclass->main_msg = "Эта задача отключена. Пожалуйста, включите ее до запуска.";
			$this->task_show_tasks();
		}
		...
		{
			$this->ipsclass->main_msg = "Эта задача была закрыта в ". gmdate( 'j M Y - G:i', $this_task['task_locked'] ) ." и не может быть запущена пока Вы ее не откроете снова.";
			$this->task_show_tasks();
		}
		...
		if ( file_exists( $func->root_path.'sources/tasks/'.$this_task['task_file'] ) )
		{
			...
			$this->ipsclass->main_msg = 'Задача успешно запущена';
			$this->task_show_tasks();
		}
		else
		{
			$this->ipsclass->main_msg = 'Невозможно найти: '.$func->root_path.'sources/tasks/'.$this_task['task_file'];
			$this->task_show_tasks();
		}
	}
	...
	function task_delete_task()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы не можете удалить эту задачу.";
			$this->task_show_tasks();
			return;
		}
		...
		$this->ipsclass->main_msg = 'Задача удалена!';
	...
	function task_do_save($type='edit')
	{
		...
		{
			$this->ipsclass->main_msg = 'Вы должны ввести название задачи.';
			$this->task_form();
		}

		if ( ! $this->ipsclass->input['task_file'] )
		{
			$this->ipsclass->main_msg = 'Вы должны ввести имя файла для исполнения';
			$this->task_form();
		}
		...
		if ( $type == 'edit' )
		{
			...
			$this->ipsclass->main_msg = 'Задача успешно изменена';
		}
		else
		{
			$this->ipsclass->DB->do_insert( 'task_manager', $save );
			$this->ipsclass->main_msg = 'Задача успешно сохранена';
		}
	...
	{
		$this->ipsclass->admin->nav[] = array( '', 'Добавление/изменение задачи' );
		...
		if ( $type == 'edit' )
		{
			...
			if ( $task['task_safemode'] and ! IN_DEV )
			{
				$this->ipsclass->main_msg = "Вы не можете изменить эту задачу.";
				$this->task_show_tasks();
				return;
			}

			$title = "Изменение задачи: ".$group['cb_group_name'];
		}
		else
		{
			$button  = "Добавить эту задачу";
			$formbit = "task_add_do";
			$this->ipsclass->html_help_title = "";
			$this->ipsclass->html_help_msg   = "";
			$task   = array();
		...
		$dropdown['_minute'] = array( 0 => array( '-1', 'Каждую минуту'   ) );
		$dropdown['_hour']   = array( 0 => array( '-1', 'Каждый час'     ), 1 => array( '0', '0 - Полночь' ) );
		$dropdown['_wday']   = array( 0 => array( '-1', 'Каждый день недели' ) );
		$dropdown['_mday']   = array( 0 => array( '-1', 'Каждый день месяца' ) );
		...
		$dropdown['_wday'][]  = array( '0', 'Воскресенье'     );
		$dropdown['_wday'][]  = array( '1', 'Понедельник'     );
		$dropdown['_wday'][]  = array( '2', 'Вторник'    );
		$dropdown['_wday'][]  = array( '3', 'Среда'  );
		$dropdown['_wday'][]  = array( '4', 'Четверг'   );
		$dropdown['_wday'][]  = array( '5', 'Пятница'     );
		$dropdown['_wday'][]  = array( '6', 'Суббота'   );
	...
	function task_show_tasks()
	{
		...
		while ( $row = $this->ipsclass->DB->fetch_row() )
		{
			...
			$row['_title']    = $row['task_enabled'] != 1 ? " (Отключено)" : '';
