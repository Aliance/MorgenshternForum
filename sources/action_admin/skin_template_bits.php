...
class ad_skin_template_bits
{
	...
	*/
	...
	function template_bits_list( $type='' )
	{
		...
		if ( ! $id )
		{
			$this->ipsclass->admin->error("Вы должны указать ID шаблона, вернитесь назад и попробуйте еще раз");
		}
		...
		$add_button = "<div class='realbutton' style='padding:4px;width:105px'>
						<a style='text-decoration:none' href='#' onclick=\"parent.template_add_bit('{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=addbit&id={$this->ipsclass->input['id']}&p={$this->ipsclass->input['p']}&expand={$group['group_name']}', event)\">Добавить шаблон</a>
					   </div>";
	...
	function template_sections_list()
	{
		...
		$this->ipsclass->admin->page_detail = "В этой секции вы можете добавить новый шаблон.";
		$this->ipsclass->admin->page_title  = "Изменение шаблона";
		...
		foreach( $group_titles as $g )
		{
			...
			{
				$g['easy_name'] = "<b>".$g['group_name']."</b> (нестандартная)";
				$g['easy_desc'] = "Эта группа не является частью стандартной инсталляции IPB и не имеет доступного описания";
			}
			
			if ( isset($skin_names[ $g['group_name'] ][2]) )
			{
				$g['easy_preview'] = "<a title='Новое окно: Показать важную IPB-страницу' href='{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?{$skin_names[ $g['group_name'] ][2]}' target='_blank'><img src='{$this->ipsclass->skin_acp_url}/images/te_previewon.gif' alt='Предпросмотр' border='0' /></a>";
			}
			else
			{
				$g['easy_preview'] = "<img src='{$this->ipsclass->skin_acp_url}/images/te_previewoff.gif' alt='Нет доступных предпросмотров' border='0' title='Нет доступных предпросмотров' />";
			}
		...
		$this->ipsclass->admin->nav[] = array( 'section='.$this->ipsclass->section_code.'&act=sets' ,'Список стилей' );
		$this->ipsclass->admin->nav[] = array( '' ,'Изменение HTML шаблонов стиля «'.$this_set['set_name'].'»' );
	...
	function add_bit()
	{
		...
		$options .= "<div class='tableheaderalt'>Настройки нового шаблона</div>
					 <div class='tablerow1'>
					 <table width='100%' cellpadding='5' cellspacing='0' border='0'>
					 <tr>
					   <td width='40%' class='tablerow1'>Название шаблона<br /><span style='color:gray'>Допускаются только латинские символы, цифры и знаки подчеркивания, без пробелов.</span></td>
					   <td width='60%' class='tablerow1'>".$this->ipsclass->adskin->form_input('func_name', $this->ipsclass->txt_stripslashes($_POST['func_name']))."</td>
					 </tr>
					 <tr>
					   <td width='40%' class='tablerow1'>Входящие переменные<br /><span style='color:gray'>Определение переменных обрабатываемых в этом шаблоне.</span></td>
					  ...
					  <tr>
					   <td width='40%' class='tablerow1'>Группа для шаблона...</td>
					 ...
					 <tr>
					   <td width='40%' class='tablerow1'>Или создайте новую группу...<br /><span style='color:gray'>Оставьте пустым для использования существующей группы.<br />Допускаются только латинские символы, цифры и знаки подчеркивания, без пробелов</span></td>
					...
					<div class='tablesubheader' align='center' style='padding:4px'><input type='submit' name='submit' value='Продолжить...' class='realdarkbutton'></div>
					</form>";
	...
	function do_add_bit()
	{
		...
		if ( $_POST['new_group_name'] )
		{
			...
			{
				$this->ipsclass->main_msg = 'Название группы шаблонов может состоять только из латинских символов, цифр и знаков подчеркивания.';
				$this->add_bit();
			}
		}
		...
		if ( ! $_POST['func_name'] )
		{
			$this->ipsclass->main_msg = 'Название шаблона не может быть пустым.';
			$this->add_bit();
		}
		else
		{
			...
			{
				$this->ipsclass->main_msg = 'Название шаблона может состоять только из латинских символов, цифр и знаков подчеркивания.';
				$this->add_bit();
			}
		}
		...
		{
			$this->ipsclass->main_msg = "Шаблон '$func_name' уже существует в группе '$group_name'.";
			$this->add_bit();
		}
		...
		{
			$this->ipsclass->main_msg = "Имя шаблона не может быть 'end'";
			$this->add_bit();
		}
	...
	function template_edit_bit( $reload=0 )
	{
		...
		$bitname    = isset($_REQUEST['bitname'])			? $this->ipsclass->parse_clean_value( urldecode( $_REQUEST['bitname'] ) ) : '';
		...
		$this->ipsclass->admin->page_detail = "Вы можете редактировать HTML код этого шаблона.";
		...
		if ( $type == 'single' )
		{
			if ( $suid == "" )
			{
				...
				if ( $bitname == "" )
				{
					$this->ipsclass->admin->error("Не выбраны ID шаблонов, пожалуйста, вернитесь назад и выберите что-нибудь");
				}
		...
		else
		{
 		...
 		if ( count($ids) < 1 )
 		{
 			$this->ipsclass->admin->error("Ни одного ID не выбрано, пожалуйста, вернитесь назад и заполните все, что необходимо перед подтверждением формы");
 		}
		...
		if ( $type != 'single' AND $type != 'multiple' )
		{
			$formbuttons = "<div align='center' class='tablesubheader'>
							<input type='submit' name='submit' value='Сохранить шаблон(ы)' class='realdarkbutton'>
							<input type='submit' name='savereload' value='Сохранить и перезагрузить шаблон(ы)' class='realdarkbutton'>
							</div>\n";
	...
	{ 
		...
		$ids = $this->ipsclass->clean_int_array( $ids );
 		...
 		if ( count($ids) < 1 )
 		{
 			$this->ipsclass->main_msg = "Нечего сохранять";
		...
		$this->ipsclass->cache_func->_recache_templates( $this->ipsclass->input['id'], $this->ipsclass->input['p'], $real_name );
		...
		if ( count( $error_bits ) )
		{
			foreach( $error_bits as $id )
			{
				...
				$this->ipsclass->main_msg = "Эти шаблоны не могут быть сохранены, так как найдена ошибка при парсинге. Проверьте введенный HTML-код и все входящие переменные.";
		...
		else
		{
			...
			{
				$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code ,'Менеджер стилей' );
				$this->ipsclass->admin->redirect( "{$this->ipsclass->form_code}&code=template-edit-bit&id={$this->ipsclass->input['id']}&group_name={$real_name}", "Шаблон(ы) обновлены, возвращение к окну выбора шаблона" );
			}
			else
			{
				//-----------------------------------------
				...
				if( count($this->ipsclass->cache_func->messages) )
				{
					...
					$this->ipsclass->cache_func->messages[] = "Шаблон(ы) в базе данных обновлен(ы)";
					
					$this->ipsclass->main_msg = implode( "<br />", $this->ipsclass->cache_func->messages );
				}
				else
				{
					$this->ipsclass->main_msg = "Шаблон(ы) обновлен(ы)";
				}
	...
	function template_revert_bit()
	{
		...
		$bitname    = $this->ipsclass->parse_clean_value( urldecode( $_REQUEST['bitname'] ) );
		$group_name = trim( $this->ipsclass->input['group_name'] );
		$id         = intval( $this->ipsclass->input['id'] );
		$p          = intval( $this->ipsclass->input['p'] );
		...
		if ( ! $suid )
		{
			if ( $bitname == "" )
			{
				$this->ipsclass->admin->error("Вы должны ввести правильный ID шаблона");
			}
		...
		if ( $row['set_id'] == 1 )
		{
			$this->ipsclass->admin->error("Вы не можете удалить шаблона из главного стиля.");
		}
		...
		$this->ipsclass->cache_func->_recache_templates( $this->ipsclass->input['id'], $this->ipsclass->input['p'], $row['group_name'] );
		...
		{
			$this->ipsclass->admin->redirect( "{$this->ipsclass->form_code}&code=edit&id={$row['set_id']}&p={$this->ipsclass->input['p']}&group_name={$row['group_name']}&#{$row['group_name']}", "Шаблон(ы) возвращены к &laquo;оригинальному&raquo; состоянию, возвращение к окну выбора шаблона" );
		}
	}
