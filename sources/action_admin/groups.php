	...
	*/
	...
	function do_delete()
	{
		...
		if ( $this->ipsclass->vars['admin_group'] == $this->ipsclass->input['id'] )
		{
			if ( $this->ipsclass->member['mgroup'] != $this->ipsclass->vars['admin_group'] )
			{
				$this->ipsclass->admin->error("Извините, но у вас нет прав, чтобы переместить в эту группу");
			}
		}
		...
		{
			$this->ipsclass->admin->error("Нельзя изменить ID удаленной группы");
		}
		...
		$this->ipsclass->admin->save_log("Пользовательская группа'{$this->ipsclass->input['name']}' удалена");

		$this->ipsclass->main_msg = "Группа удалена";
		$this->main_screen();
	}
	...
	function save_group($type='edit')
	{
		...
		{
			$this->ipsclass->admin->error("Вы должны ввести название группы.");
		}
		...
		if ($type == 'edit')
		{
			...
			{
				$this->ipsclass->admin->error("Вы не можете удалить права на админцентр у этой группы");
			}
		}
		...
		{
			$this->ipsclass->admin->error("Ни одной маски доступа не выбрано");
		}
		...
		if ( $this->ipsclass->input['g_attach_per_post'] and $this->ipsclass->input['g_attach_max'] > 0 )
		{
			...
			{
				$this->ipsclass->main_msg = "Вы не можете указывать лимит на сообщения более, чем общий глобальный лимит.";
				$this->group_form('edit');
			}
		}
		...
		if ($type == 'edit')
		{
			...
			$this->ipsclass->admin->save_log("Отредактирована группа '{$this->ipsclass->input['g_title']}'");

			$this->rebuild_group_cache();

			$this->ipsclass->main_msg = "Группа отредактирована";
			$this->main_screen();
		...
		else
		{
			...
			$this->ipsclass->admin->save_log("Добавлена группа '{$this->ipsclass->input['g_title']}'");

			$this->rebuild_group_cache();

			$this->ipsclass->main_msg = "Группа добавлена";
			$this->main_screen();
		}
	}
