...
class ad_adminlogs
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Журнал администрирования' );
	...
	function view()
	{
		...
		$this->ipsclass->admin->page_detail = "Просмотр всех действий, произведенных администраторами";
		$this->ipsclass->admin->page_title  = "Журнал администрирования";
		...
		{
			$this->ipsclass->main_msg = "Вы не указали действие, которое необходимо найти.";
			$this->list_current();
			return;
		}
		...
		$this->ipsclass->admin->page_detail = "В этой секции вы можете просматривать и управлять журналом администрирования";
		$this->ipsclass->admin->page_title  = "Журнал администрирования";

		//-----------------------------------------

		$this->ipsclass->adskin->td_header[] = array( "Администратор"            , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Действие"        , "40%" );
	...
	function remove()
	{
		...
		{
			$this->ipsclass->admin->error("Вы не указали ID администратора");
		}
	...
	function list_current()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "5 последних действий" );
		...
		$this->ipsclass->adskin->td_header[] = array( "Администратор"            , "30%" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Журнал администрирования" );
		...
		while ( $r = $this->ipsclass->DB->fetch_row() )
		{
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>{$r['name']}</b>",
													  ...
													  "<center><a href='".$this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=view&mid={$r['member_id']}'>Посмотреть</a></center>",
													  "<center><a href='".$this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=remove&mid={$r['member_id']}'>Удалить</a></center>",
											 )      );
		}
		...
		$form_array = array(
							  0 => array( 'note'        , 'Действие' ),
							  1 => array( 'ip_address'  , 'IP-адрес'  ),
							  2 => array( 'member_id'   , 'ID администратора' ),
							  3 => array( 'act'         , 'Настройки ACT'  ),
							  4 => array( 'code'        , 'Настройки CODE'  ),
						   );
