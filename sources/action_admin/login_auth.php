...
+---------------------------------------------------------------------------
|   > $Date: 2006-12-15 04:58:55 -0500 (Fri, 15 Dec 2006) $
|   > $Revision: 794 $
|   > $Author: matt $
+---------------------------------------------------------------------------
|
...
class ad_login_auth
{
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->nav[]    = array( "{$this->ipsclass->form_code}", "Управление авторизациями" );
	...
	function components_import()
	{
		...
		if ( $_FILES['FILE_UPLOAD']['name'] == "" or ! $_FILES['FILE_UPLOAD']['name'] or ($_FILES['FILE_UPLOAD']['name'] == "none") )
		{
			...
			{
				$this->ipsclass->main_msg = "Загружаемый файл не найден!";
			...
			{
				$this->ipsclass->main_msg = "Нельзя найти файл для открытия: " . ROOT_PATH . $this->ipsclass->input['file_location'];
				$this->components_list();
				return;
			}
		...
		if( !$content )
		{
			$this->ipsclass->admin->error( "Импортирование не осуществлено, так как файл имеет неверный формат или в нем нет записей." );
		}
		...
		$this->ipsclass->main_msg = "$updated компонентов обновлено и $inserted вставлено.";
	...
	function login_diagnostics()
	{
		...
		$this->ipsclass->html_help_title = "Управление авторизацией";
		$this->ipsclass->html_help_msg   = "Эта секция позволит вам управлять идентификационными методами для входа.";
	...
	function login_save($type='add')
	{
		...
		if ( $type == 'add' )
		{
			...
			$this->ipsclass->main_msg = 'Новый метод входа был добавлен';
		}
		else
		{

			$this->ipsclass->DB->do_update( 'login_methods', $array, 'login_id='.$login_id );
			$this->ipsclass->main_msg = 'Новый метод входа был изменен';
		}
	...
	function login_form( $type='add' )
	{
		...
		$login_id  = intval($this->ipsclass->input['login_id']);
		$login_dd  = array( 0 => array( 'passthrough', 'Удаленный' ), 1 => array( 'onfail', 'Совместный' ) );
		$login_unt = array( 0 => array( 'username', 'Имя пользователя' ), 1 => array( 'email', 'E-mail адрес' ) );
		...
		if ( $type == 'add' )
		{
			...
			$title    = "Создания нового метода входа";
			$button   = "Создать";
		}
		else
		{
			...
			$title    = "Редактирование: ".$login['login_title'];
		...
		$this->ipsclass->html_help_title = "Управление авторизацией";
		$this->ipsclass->html_help_msg   = "Эта секция позволит вам управлять идентификационными методами для авторизации.";

		$this->ipsclass->admin->nav[]    = array( "", "Добавление/изменение метода авторизации" );
		$this->ipsclass->admin->output();
	}
	...
	function login_list()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление авторизациями";
		$this->ipsclass->admin->page_detail = "Эта секция позволит вам управлять способами авторизации.";
		$this->ipsclass->admin->output();
	}
