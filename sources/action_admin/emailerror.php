...
+---------------------------------------------------------------------------
|   > $Date: 2006-12-01 15:07:16 -0600 (Fri, 01 Dec 2006) $
|   > $Revision: 752 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_emailerror
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Журнал ошибок при работе с почтой' );
	...
	function view_email()
	{
		...
		{
			$this->ipsclass->admin->error("Вы не указали ID e-mail адреса.");
		}
		...
		{
			$this->ipsclass->admin->error("Указанный вами ID ($id) не найден в базе данных.");
		}
		...
		$row['mlog_msg']        = $row['mlog_msg']        ? $row['mlog_msg']        : '<em>Нет информации</em>';
		$row['mlog_code']       = $row['mlog_code']       ? $row['mlog_code']       : '<em>Нет информации</em>';
		$row['mlog_smtp_error'] = $row['mlog_smtp_error'] ? $row['mlog_smtp_error'] : '<em>Нет информации</em>';

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
																			  "<strong>От:</strong> &lt;{$row['mlog_from']}&gt;
																			  <br /><strong>Кому:</strong> &lt;{$row['mlog_to']}&gt;
																			  <br /><strong>Отправлено:</strong> {$row['mlog_date']}
																			  <br /><strong>Тема:</strong> {$row['mlog_subject']}
																			  <hr />
																			  <br />{$row['mlog_content']}....
																			  <hr />
																			  <br />IPB ОШИБКА: {$row['mlog_msg']}
																			  <br />SMTP КОД: {$row['mlog_code']}
																			  <br />SMTP ОШИБКА: {$row['mlog_smtp_error']}
																			  "
																   )      );
	...
	function remove_entries()
	{
		...
		else
		{
			...
			if ( count($ids) < 1 )
			{
				$this->ipsclass->admin->error("Вы должны выбрать как минимум одну запись для удаления.");
			}

			$this->ipsclass->DB->simple_exec_query( array( 'delete' => 'mail_error_logs', 'where' => "mlog_id IN (".implode(',', $ids ).")" ) );
		}

		$this->ipsclass->admin->save_log("Журнал e-mail ошибок очищен");
	...
	function list_current()
	{
		...
		$this->ipsclass->admin->page_detail = "Журнал сохраненных e-mail ошибок";
		$this->ipsclass->admin->page_title  = "Управление журналом e-mail ошибок";
		...
		$this->ipsclass->adskin->td_header[] = array( "Тема"        , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Ошибка"      , "30%" );
		$this->ipsclass->adskin->td_header[] = array( "Вермя"           , "20%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Список неотправленных писем" );
		...
		if ( $this->ipsclass->DB->get_num_rows() )
		{
			while ( $row = $this->ipsclass->DB->fetch_row() )
			{
				...
				$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
																		...
																		'<b>'.$row['mlog_to'].'</b>',
																		"<a href='javascript:pop_win(\"&{$this->ipsclass->form_code_js}&code=viewemail&id={$row['mlog_id']}\", \"{$row['mlog_id']}\",400,350)' title='Читать письмо'>".$row['mlog_subject']."</a>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic('<div style="float:left;width:auto"><input type="submit" value="Удалить выбранные" class="realbutton" />&nbsp;<input type="checkbox" id="checkbox" name="type" value="all" />&nbsp;Удалить все?</div><div align="right">'.$links.'</div></form>', 'left', 'tablesubheader');
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Поиск записей ошибок" );

		$form_array = array(
							  0 => array( 'subject'    , 'Тема письма'      ),
							  2 => array( 'email_from' , 'От (E-mail)' ),
							  3 => array( 'email_to'   , 'Кому (E-mail)'   ),
							  4 => array( 'error'      , 'Сообщение об ошибке'      ),
						   );
