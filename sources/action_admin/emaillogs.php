...
class ad_emaillogs
{
	...
	*/
	...
	function auto_run()
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Журнал e-mail отправлений' );

		// Make sure we're a root admin, or else!

		if ($this->ipsclass->member['mgroup'] != $this->ipsclass->vars['admin_group'])
		{
			//$this->ipsclass->admin->error("Извините, эта функция доступна только главным администраторам.");
		}
	...
	function view_email()
	{
		...
		{
			$this->ipsclass->admin->error("Невозможно определить ID адреса email, вернитесь и попробуйте еще раз");
		}
		...
		{
			$this->ipsclass->admin->error("Невозможно определить ID адреса email, вернитесь и попробуйте еще раз ($id)");
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													...
													<br /><strong>Кому:</strong> {$row['to_name']} &lt;{$row['to_email_address']}&gt;
													<br /><strong>Отправлено:</strong> {$row['email_date']}
													<br /><strong>IP-адрес:</strong> {$row['from_ip_address']}
													<br /><strong>Тема:</strong> {$row['email_subject']}
													<hr>
	...
	function remove_entries()
	{
		...
		else
		{
			...
			if ( count($ids) < 1 )
			{
				$this->ipsclass->admin->error("Вы должны выбрать хоть одну запись из журнала для удаления");
			}

			$this->ipsclass->DB->simple_exec_query( array( 'delete' => 'email_logs', 'where' => " email_id IN (".implode(',', $ids ).")" ) );
		}

		$this->ipsclass->admin->save_log("Удалены записи из журнала e-mail отправлений");
	...
	function list_current()
	{
		...
		$this->ipsclass->admin->page_detail = "Сохраненные записи журнала e-mail отправлений";
		$this->ipsclass->admin->page_title  = "Управление журналом e-mail отправлений";
		...
		{
			$this->ipsclass->admin->page_title .= " (Результаты поиска)";
			...
			switch( $this->ipsclass->input['type'] )
			{
				...
				case 'subject':
					...
					if ( $string == "" )
					{
						$this->ipsclass->admin->error("Вы должны ввести хоть что-нибудь");
					}
				...
				case 'name_from':
					...
					if ( $this->ipsclass->input['match'] == 'loose' )
					{
						...
						{
							$this->ipsclass->admin->error("Не найдено никаких записей, соответствующих запросу в журнале e-mail отправлений");
						}
		...
		$this->ipsclass->adskin->td_header[] = array( "От"    , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Тема"        , "30%" );
		$this->ipsclass->adskin->td_header[] = array( "Кому"      , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Дата"      , "25%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Список отправленных писем" );
		...
		if ( $this->ipsclass->DB->get_num_rows() )
		{
			while ( $row = $this->ipsclass->DB->fetch_row() )
			{
				...
				$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
														  ...
														  "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=list&type=fromid&id={$row['id']}' title='Показать все отправленные письма'><img src='{$this->ipsclass->skin_acp_url}/images/acp_search.gif' border='0' alt='..by id'></a>&nbsp;<b><a href='{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?showuser={$row['id']}' title='Профиль пользователя (откроется в новом окне)' target='blank'>{$row['name']}</a></b>",
														  "<a href='javascript:pop_win(\"&{$this->ipsclass->form_code_js}&code=viewemail&id={$row['email_id']}\",\"Log\",400,400)' title='Прочитать письмо'>{$row['email_subject']}</a>",
														  "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=list&type=toid&id={$row['to_id']}' title='Показать все полученные письма'><img src='{$this->ipsclass->skin_acp_url}/images/acp_search.gif' border='0' alt='..by id'></a>&nbsp;<a href='{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?showuser={$row['to_id']}'  title='Профиль пользователя (откроется в новом окне)' target='blank'>{$row['to_name']}</a>",
														  "{$row['email_date']}",
												 )      );
		...
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic("<center>Записей не найдено.</center>");
		}

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic('<div style="float:left;width:auto"><input type="submit" value="Удалить отмеченные" class="realbutton" />&nbsp;<input type="checkbox" id="checkbox" name="type" value="all" />&nbsp;Удалить все?</div><div align="right">'.$links.'</div></form>', 'left', 'tablesubheader');
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Поиск записей" );

		$form_array = array(
							  0 => array( 'subject'    , 'Тема письма'    ),
							  1 => array( 'content'    , 'Содержимое' ),
							  2 => array( 'email_from' , 'От (E-mail)' ),
							  3 => array( 'email_to'   , 'Кому (E-mail)'   ),
							  4 => array( 'name_from'  , 'От (имя пользователя)'),
							  5 => array( 'name_to'    , 'Кому (имя пользователя)' ),
						   );

		$type_array = array(
							  0 => array( 'exact'      , 'совпадение' ),
							  1 => array( 'loose'      , 'содержит'   ),
						   );
