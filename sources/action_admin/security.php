...
class ad_security
{
	...
	*/
	...
	function do_confglobal()
	{
		...
		{
			$this->ipsclass->main_msg = "<strong>Невозможно продолжить операцию.</strong><br />Пожалуйста, установите файлу «conf_global.php» при помощи FTP-программы атрибуты доступа (CHMOD) 0444.";
		}
...
EOF;
	...
	function deep_scan()
	{
		...
		if ( is_array( $class_virus_checker->bad_files ) and count( $class_virus_checker->bad_files ) )
		{
			...
			if ( $bad_score )
			{
				$this->ipsclass->html .= $this->ipsclass->skin_acp_global->warning_box( 'Используемые системой файлы', 'Сканер обнаружил следующие файлы.<br /><strong>'.$bad_score.'</strong> из '.$file_count.' файлов имеют рейтинг 7/10 и выше.<br />Если вам неизвестно происхождение какого-либо из файлов, мы рекомендуем вам их очень внимательно изучить.' ) . "<br />";
			}
			else
			{
				$this->ipsclass->html .= $this->ipsclass->skin_acp_global->information_box( 'Используемые системой файлы', 'Сканер проверил '.$file_count.' файлов.<br />Если вам неизвестно происхождение какого-либо из файлов, мы рекомендуем вам их очень внимательно изучить.' ) . "<br />";
			}
	...
	function anti_virus_check()
	{
		...
		if ( is_array( $class_virus_checker->bad_files ) and count( $class_virus_checker->bad_files ) )
		{
			...
			$this->ipsclass->html .= $this->ipsclass->skin_acp_global->warning_box( 'Обнаружены подозрительные файлы', 'Ниже приведен список обнаруженных подозрительных файлов.<br />Если вы не уверены, что загружали их, то, пожалуйста, удалите их в целях безопасности.' ) . "<br />";

			$this->ipsclass->html .= $this->html->anti_virus_bad_files_wrapper( $content );
		}
		else
		{
			$this->ipsclass->html .= $this->ipsclass->skin_acp_global->information_box( 'Подозрительные файлы не обнаружены', 'Сканер не обнаружил подозрительных файлов.<br />Пожалуйста, старайтесь как можно чаще проводить эту проверку.' ) . "<br />";
		}
	...
	function security_overview()
	{
		...
		if ( intval($cache_array['last_virus_check']) < time() - 7 * 86400 )
		{
			$content['bad'] .= $this->html->security_item_bad(  'Сканер подозрительных файлов',
			 													'Сканер проверит наиболее часто используемые директории для загрузки файлов на подозрительные файлы.<br />Сканер на подозрительные файлы не запускали более недели.',
																'Запустить',
		...
		else
		{
			...
			$content['good'] .= $this->html->security_item_good( 'Сканер подозрительных файлов',
			 													 'Сканер проверит наиболее часто используемые директории для загрузки файлов на подозрительные файлы.<br />Последний запуск: '.$last_run,
																 'Запустить',
																 $this->ipsclass->form_code_js.'&code=virus_check',
																 'vchecker' );
		}
		...
		if ( intval($cache_array['last_deepscan_check']) < time() - 30 * 86400 )
		{
			$content['bad'] .= $this->html->security_item_bad(  'Глубокое сканирование',
			 													'Сканер проверит и выведет список всех файлов вашей системы.<br />Сканер не запускали более месяца.',
																'Запустить',
		...
		else
		{
			...
			$content['good'] .= $this->html->security_item_good(  'Глубокое сканирование',
			 													  'Сканер проверит и выведет список всех файлов вашей системы.<br />Последний запуск: '.$last_run,
																  'Запустить',
																   $this->ipsclass->form_code_js.'&code=deep_scan',
																  'deepscan' );
		}
		...
		if ( strtoupper( substr(PHP_OS, 0, 3) ) !== 'WIN' )
		{
			...
			{
				$_extra = "<div style='color:red;font-weight:bold'>IPB не может произвести запись файла «.htaccess» в директорию админцентра. Пожалуйста, установите атрибуты доступа (CHMOD) 0777 на эту директорию при помощи FTP-программы.</div>";
			}

			if ( ! file_exists( ROOT_PATH . IPB_ACP_DIRECTORY . '/.htaccess' ) )
			{
				$content['ok'] .= $this->html->security_item_ok(    'Защита файлом «.htaccess» АЦ',
				 													'Для повышения безопасности вы можете добавить HTTP-аутентификацию на директорию админцентра (по умолчанию: /admin/).<br />IPB не обнаружил «.htaccess» файл в директории админцентра.'. $_extra,
																	'Подробнее',
			...
			else
			{
				$content['good'] .= $this->html->security_item_good( 'Защита файлом «.htaccess» АЦ',
				 											 		 'Для повышения безопасности вы можете добавить HTTP-аутентификацию на директорию админцентра (по умолчанию: /admin/).<br />IPB обнаружил «.htaccess» файл в директории админцентра.'.$_extra,
																	 'Подробнее',
																	 $this->ipsclass->form_code_js.'&code=acphtaccess',
																	 'acphtaccess' );
			}
			...
			if ( ! file_exists( ROOT_PATH . 'style_emoticons/.htaccess' ) )
			{
				$content['ok'] .= $this->html->security_item_ok( 'Защита файлом «.htaccess»',
				 												 'IPB может записать файл «.htaccess» в директории, где не содержатся php-файлы, препятствуя тем самым запуску файлов PHP и CGI в этих директориях.<br />IPB не обнаружил какие-либо «.htaccess» файлы.',
																 'Запустить',
			...
			else
			{
				$content['good'] .= $this->html->security_item_good( 'Защита файлом «.htaccess»',
				 											 		 'IPB может записать файл «.htaccess» в директории, где не содержатся php-файлы, препятствуя тем самым запуску файлов PHP и CGI в этих директориях.<br />IPB обнаружил «.htaccess» файлы.',
																	 'Запустить',
																	 $this->ipsclass->form_code_js.'&code=htaccess',
																	 'htaccess' );
			}
			...
			if ( is_writeable( ROOT_PATH . 'conf_global.php' ) )
			{
				$content['bad'] .= $this->html->security_item_bad( 'Запись в файл «conf_global»',
				 												   'После установки IPB рекомендуется изменить атрибуты доступа к файлу «conf_global.php», чтобы другие пользователи не могли произвести чтение или запись.<br />«conf_global.php» доступен для записи.',
																   'Запустить',
			...
			else
			{
				$content['good'] .= $this->html->security_item_good(  'Запись в файл «conf_global»',
				 												 	  'После установки IPB рекомендуется изменить атрибуты доступа к файлу «conf_global.php», чтобы другие пользователи не могли произвести чтение или запись.<br />«conf_global.php» недоступен для записи.',
																	   'Запустить',
																	   $this->ipsclass->form_code_js.'&code=confglobal',
																	   'confglobal' );
			}
		...
		if ( ! $this->ipsclass->vars['allow_dynamic_img'] )
		{
			$content['good'] .= $this->html->security_item_good( 'Отключение динамических изображений',
			 												  	 'IP.Board может запретить использовать динамические изображения в сообщениях. Их использование в сообщениях подвергает большому риску безопасность вашего форума.<br />Динамические изображения в данный момент запрещены.',
																 'Исправить',
		...
		else
		{
			$content['bad'] .= $this->html->security_item_bad( 'Отключение динамических изображений',
			 											       'IP.Board может запретить использовать динамические изображения в сообщениях. Их использование в сообщениях подвергает большому риску безопасность вашего форума.<br />Динамические изображения РАЗРЕШЕНЫ.',
														       'Исправить',
														        $this->ipsclass->form_code_js.'&code=dynamic_images',
														       'dynamic_images' );
		}
		...
		if ( ! $this->ipsclass->vars['cookie_stronghold'] )
		{
			$content['bad'] .= $this->html->security_item_bad(  'Защищенные cookies',
			 													'IPB может записывать более защищенные cookies в браузер пользователя после его успешной авторизации, тем самым снижая вероятность их кражи злоумышленником.<br />Защищенные cookies отключены.',
																'Исправить',
		...
		else
		{
			$content['good'] .= $this->html->security_item_good( 'Защищенные cookies',
			 													 'IPB может записывать более защищенные cookies в браузер пользователя после его успешной авторизации, тем самым снижая вероятность их кражи злоумышленником.<br />Защищенные cookies включены.',
																 'Исправить',
																 $this->ipsclass->form_code_js.'&code=stronghold',
																 'stronghold' );
		}
		...
		if ( ! $this->ipsclass->vars['security_remove_acp_link'] )
		{
			$content['ok'] .= $this->html->security_item_ok( 'Удаление ссылки на админцентр',
			 												 'IPB может удалить ссылку «Админцентр» из пользовательской панели навигации на вашем форуме.<br />Данная ссылка отображается, когда пользователь авторизирован на форуме как администратор.',
															 'Исправить',
		...
		else
		{
			$content['good'] .= $this->html->security_item_good( 'Удаление ссылки на админцентр',
			 											 		'IPB может удалить ссылку «Админцентр» из пользовательской панели навигации на вашем форуме.<br />Данная ссылка отображается, когда пользователь авторизирован на форуме как администратор.<br />Ссылка на админцентр в данный момент удалена.',
																 'Исправить',
																 $this->ipsclass->form_code_js.'&code=acplink',
																 'acplink' );
		}
		...
		if ( IPB_ACP_DIRECTORY == 'admin' )
		{
			$content['ok'] .= $this->html->security_item_ok( 'Название директории АЦ',
			 												 'Стандартную директорию админцентра (/admin/) можно переименовать — тем самым злоумышленнику будет сложнее нарушить работу форума в случае взлома учетной записи какого-либо из администраторов.<br />Директория админцентра не переименована.',
															 'Подробнее',
		...
		else
		{
			$content['good'] .= $this->html->security_item_good( 'Название директории АЦ',
			 													 'Стандартную директорию админцентра (/admin/) можно переименовать — тем самым злоумышленнику будет сложнее нарушить работу форума в случае взлома учетной записи какого-либо из администраторов.<br />Директория админцентра переименована.',
																 'Подробнее',
																 $this->ipsclass->form_code_js.'&code=acprename',
																 'acprename' );
		}
