...
class ad_forums
{
	...
	*/
	...
	function skin_edit()
	{
		...
		{
			$this->ipsclass->admin->error("Невозможно определить ID форума, чтобы очистить его.");
		}
		...
		if ( ! $forum['id'] )
		{
			$this->ipsclass->admin->error("Нельзя изменить ID этого Форума");
		}
		...
		$this->ipsclass->admin->page_title  = "Установка стиля для форума или категории";
		$this->ipsclass->admin->page_detail = "В этой секции вы можете установить или удалить стиль для выбранного форума или категории.<br />Выбранный стиль будет автоматически заменять стиль, который используют пользователи по умолчанию.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Установка стиля для «{$forum['name']}»" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Какой стиль установить для этого форума?</b>" ,
																 "<select class='dropdown' name='fsid'><option value='-1'>--Нет / Удалить все--</option>{$skin_list}</select>"
														 )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Применить ко всем подфорумам?</b>" ,
	...
	function do_skin_edit()
	{
		if ($this->ipsclass->input['f'] == "")
		{
			$this->ipsclass->admin->error("Невозможно определить ID форума, чтобы принять этот стиль.");
		}
		...
		$this->ipsclass->main_msg = "Шаблон форума обновлен";
	...
	function show_rules()
	{
		...
		$this->ipsclass->admin->page_title  = "Правила форума";
		$this->ipsclass->admin->page_detail = "В этой секции вы можете добавлять, изменять и удалять отображаемые правила выбранного форума";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Настройки правил форума" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Способ отображения</b>" ,
																 $this->ipsclass->adskin->form_dropdown( "show_rules",
																					   array(
																							   0 => array( '0' , 'Не показывать' ),
																							   1 => array( '1' , 'Показывать только ссылку' ),
																							   2 => array( '2' , 'Показывать весь текст' )
																							),
																					   $forum['show_rules']
																					 )
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Заголовок правил</b>" ,
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Текст</b>" ,
																 $form_element
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Сохранить изменения");
	...
	function do_rules()
	{
		...
		$this->recache_forums();
		$this->ipsclass->main_msg = "Правила форума обновлены";
	...
	function recount($f_override="")
	{
		...
		$this->ipsclass->admin->save_log("Все сообщения на форуме пересчитаны '{$this->ipsclass->forums->forum_by_id[$this->ipsclass->input['f']]['name']}'");

		$this->ipsclass->main_msg = "Форум ресинхронизирован";
	...
	function empty_form()
	{
		...
		$this->ipsclass->admin->page_title = "Очистить форум '{$forum['name']}'";

		$this->ipsclass->admin->page_detail = "ЭТА ОПЦИЯ УДАЛЯЕТ ВСЕ ТЕМЫ, СООБЩЕНИЯ И ГОЛОСОВАНИЯ.<br />Сам же форум не будет удален - убедитесь в том, что это действительно необходимо.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Очистка форума '{$forum['name']}'" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Очистить форум: </b>" , $forum['name'] )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Очистить этот форум");
	...
	function do_empty()
	{
		...
		if ($this->ipsclass->input['f'] == "")
		{
			$this->ipsclass->admin->error("Невозможно определить ID форума");
		}
		...
		$this->ipsclass->admin->save_log("'{$this->ipsclass->input['name']}' очищен от всех сообщений");

		$this->ipsclass->input['f'] =  $this->ipsclass->forums->forum_by_id[ $this->ipsclass->input['f'] ]['parent_id'];
		$this->ipsclass->main_msg   = "Форум очищен ;(";
		$this->show_forums();
	}
	...
	function delete_form()
	{
		...
		if ( ! $this->ipsclass->input['f'] )
		{
			$this->ipsclass->admin->error("Невозможно определить ID форума, чтобы удалить.");
		}
		...
		{
			$this->ipsclass->admin->error("Вы не можете удалить этот форум, создайте хоть один форум кроме этого для удаления выбранного форума.");
		}
		...
		$this->ipsclass->admin->page_title = "Удаление форума '$name'";

		$this->ipsclass->admin->page_detail = "До того как вы удалите этот форум: если выбранный форум не пуст, выберите, что вы будете делать с сообщениями этого форума.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Дополнительная информация" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Удалить форум: </b>" , $name )      );

		if ( $posts['count'] )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Переместить все <i>имеющиеся темы и сообщения в этом форуме</i> в другой?</b>" ,
		...
		if ( $children['count'] )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Переместить все <i>существующие темы, сообщения и голосования из этого форума</i> в форум:</b>" ,
																	$this->ipsclass->adskin->form_dropdown( "new_parent_id", $form_array )
														  )      );
		}


		$this->ipsclass->html .= $this->ipsclass->adskin->end_form( "Удалить форум" );
	...
	function do_delete()
	{
		...
		if ( ! $this->ipsclass->input['f'] )
		{
			$this->ipsclass->admin->error("Невозможно определить источник ID форума.");
		}
		...
		else
		{
			...
			{
				$this->ipsclass->main_msg = "Вы не можете переместить подфорумы в удаляемый форум!";
				$this->delete_form();
			}
		}
		...
		if ( $this->ipsclass->input['MOVE_ID'] )
		{
			...
			{
				$this->ipsclass->main_msg = "Вы не можете переместить темы, сообщения и голосования в удаляемый форум!";
				$this->delete_form();
			}
		...
		$this->ipsclass->admin->save_log("Форум удален '{$forum['name']}'");

		$this->ipsclass->admin->done_screen("Форум был успешно удален", "Управление форумом", $this->ipsclass->form_code, 'redirect' );
	}
	...
	function forum_form( $type='edit' )
	{
		...
		$f_name      = isset($_REQUEST['name']) ? $this->ipsclass->parse_clean_value(urldecode($_REQUEST['name'])) : '';
		$subcanpost  = ( $cat_id == 1 ) ? 0 : 1;
		$perm_matrix = "";
		$dd_state    = array( 0 => array( 1, 'активный' ), 1 => array( 0, 'только для чтения архивов' ) );
		$dd_moderate = array(
							 0 => array( 0, 'нет' ),
							 1 => array( 1, 'все новые темы и сообщения' ),
							 2 => array( 2, 'новые темы' ),
							 3 => array( 3, 'только новые ответы' ),
							);
		$dd_prune    = array(
							 0 => array( 1, 'день' ),
							 1 => array( 5, '5 дней'  ),
							 ...
							 8 => array( 60, '60 дней' ),
							 9 => array( 90, '90 дней' ),
							 10=> array( 100,'все время'     ),
							);

		$dd_order    = array(
							 0 => array( 'last_post', 'по дате последнего сообщения' ),
							 1 => array( 'title'    , 'названиям тем' ),
							 2 => array( 'starter_name', 'автору тем' ),
							 3 => array( 'posts'    , 'сообщениям в темах' ),
							 4 => array( 'views'    , 'просмотру тем' ),
		...
		$dd_by       = array(
							 0 => array( 'Z-A', 'по убыванию (Я—А, 0—10)' ),
							 1 => array( 'A-Z', 'по возрастанию (А—Я, 10—0)'  )
							);

		$dd_filter	 = array(
							 0 => array( 'all', 	'все темы' ),
							 1 => array( 'open', 	'открытые темы' ),
							 2 => array( 'hot',		'горячие темы' ),
							 3 => array( 'poll',	'опросы' ),
							 4 => array( 'locked',	'закрытые темы' ),
							 5 => array( 'moved',	'открытые темы' ),
							 6 => array( 'istarted', 'темы, созданные пользователем' ),
							 7 => array( 'ireplied', 'темы, с ответами пользователя' ),
							);
		...
		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам добавлять новые и изменять уже существующие форумы или категории.<br />Если вы хотите установить маски доступа к существующему форуму (такие как возможность просмотра форума, создание тем, ответы и загрузки) нажмите на «Маски доступа».";
		...
		if ( $type == 'edit' )
		{
			...
			if ( ! $forum_id )
			{
				$this->ipsclass->admin->error("Вы не выбрали форум для редактирования!");
			}
			...
			if ($forum['id'] == "")
			{
				$this->ipsclass->admin->error("Невозможно найти форум с ID {$this->ipsclass->input['f']}");
			}
			...
			$lang_title  = ( $forum['parent_id'] == -1 ) ? 'категории' : 'форума';

			$title  		= "Изменение {$lang_title}: {$forum['name']}";
			$button 		= "Сохранить";
			$code   		= "doedit";
			...
			$basic_title = "<table cellpadding='0' cellspacing='0' border='0' width='100%'>
							<tr>
							 <td align='left' width='40%' style='font-size:12px; vertical-align:middle;font-weight:bold; color:#FFF;'>Основные настройки {$forum['name']}</td>
							 <td align='right' width='60%'>".
							 $this->ipsclass->adskin->js_make_button("Изменить {$addnew_type} правила"  , $this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=frules&f={$this->ipsclass->input['f']}")."&nbsp;".
						     $this->ipsclass->adskin->js_make_button("Шаблон форума", $this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=skinedit&f={$this->ipsclass->input['f']}")."&nbsp;".
						     $this->ipsclass->adskin->js_make_button("Пересчитать {$addnew_type}"     , $this->ipsclass->base_url."&{$this->ipsclass->form_code}&code=recount&f={$this->ipsclass->input['f']}")
							 ."&nbsp;&nbsp;</td>
							</tr>
							</table>";
		}
		...
		else
		{
			...
			$lang_title  = ( $addnew_type == 'category' ) ? 'категории' : 'форума';
			$name_value  = ( $addnew_type == 'category' ) ? 'Новая категория' : 'Новый форум';
						
			$forum = array(
							'sub_can_post'				=> $subcanpost,
							'name'						=> $f_name ? $f_name : $name_value,
							'parent_id'					=> $parentid,
							'use_ibc'					=> 1,
							'quick_reply'				=> 1,
							'allow_poll'				=> 1,
			...
			$title       = "Добавление " . $lang_title;
			$button      = "Создать";
			$code        = "donew";
			$basic_title = 'Основные настройки';
		...
		$form['sub_can_post'] = $this->ipsclass->adskin->form_yes_no(  'sub_can_post', ( isset($_POST['sub_can_post']) AND $_POST['sub_can_post'] )         ? $_POST['sub_can_post'] : $forum['sub_can_post'] );
		...
		$form['addnew_type_upper']	= $lang_title;
		...
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Управление форумами' );
		$this->ipsclass->admin->nav[] = array( '', 'Добавление/изменение '.$lang_title );
	...
	function forum_save($type='new')
	{
		...
		$forum_cat_lang_msg = intval($this->ipsclass->input['parent_id']) == -1 ? 'категории' : 'форума';
		$forum_cat_lang = intval($this->ipsclass->input['parent_id']) == -1 ? 'Категория' : 'Форум';
		$forum_cat_lang_log = intval($this->ipsclass->input['parent_id']) == -1 ? 'а' : '';
		...
		{
			$this->ipsclass->main_msg = "Вы должны ввести название " . $forum_cat_lang_msg;
		...
		if ( $this->ipsclass->input['parent_id'] != $this->ipsclass->forums->forum_by_id[ $this->ipsclass->input['f'] ]['parent_id'] )
		{
			...
			{
				$this->ipsclass->main_msg = "Извините, сделать это невозможно. Вы хотите переместить родительский форума в него же самого. Пожалуйста, вернитесь назад и попытайтесь выбрать другой форум.";
				$this->forum_form( $type );
				return;
			}
		}
		...
		$save = array (  'name'                    => $this->ipsclass->input['name'],
						 ...
						 'sub_can_post'            => intval($this->ipsclass->input['sub_can_post']),
		...
		if ( $type == 'new' )
		{
			...
			$this->ipsclass->main_msg = $forum_cat_lang . " создан" . $forum_cat_lang_log .".";

			$this->ipsclass->admin->save_log($forum_cat_lang." «{$this->ipsclass->input['name']}» создан" . $forum_cat_lang_log . ".");
		}
		else
		{
			 if( $this->ipsclass->input['parent_id'] == -1 )
			...
			$this->ipsclass->main_msg = $forum_cat_lang." изменен". $forum_cat_lang_log . ".";

			$this->ipsclass->admin->save_log($forum_cat_lang." «{$this->ipsclass->input['name']}» изменен" . $forum_cat_lang_log . ".");
		}
	...
	function perm_edit_form()
	{
		...
		if ( ! $forum['id'] )
		{
			$this->ipsclass->admin->error("Нельзя извлечь из базы данных ID форума {$this->ipsclass->input['f']}");
		}

		$this->ipsclass->admin->page_title = "Изменение прав доступа «{$forum['name']}»";
		...
		{
			$perm_matrix = $this->build_group_perms(  $forum['show_perms'], $forum['read_perms'], $forum['start_perms'], $forum['reply_perms'], $forum['upload_perms'], $forum['download_perms'],
												  $this->ipsclass->forums->forum_by_id[ $forum['parent_id'] ]['name'].' &gt; '.$forum['name'].' &gt; '."Права доступа" );

			$this->ipsclass->admin->page_detail = "<b>Маски доступа к форуму</b><br />Если вы запрещаете маске доступа просмотр форума, то группы, которые закреплены за этой маской,&nbsp;не смогут просматривать этот форум.";
		}
		else
		{
			$perm_matrix = $this->build_group_cat_perms( $forum['show_perms'], $forum['name'].' &gt; '."Права доступа" );

			$this->ipsclass->admin->page_detail = "<b>Маски доступа к категории</b><br />Если вы не разрешите маске доступа просмотра форума, то группы, которые закреплены за этой маской, не смогут просматривать все форумы, относящиеся к этой категории.";
		}

		$this->ipsclass->html .= $this->html->forum_permission_form( $forum, $relative, $perm_matrix );

		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Управление форумами' );
		$this->ipsclass->admin->nav[] = array( '', "Изменение масок доступа «{$forum['name']}»" );
	...
	function perm_do_edit()
	{
		...
		$this->ipsclass->admin->save_log("Изменены маски доступа к '{$this->ipsclass->input['name']}'");

		$this->recache_forums();

		if ( isset($this->ipsclass->input['doprevious']) AND $this->ipsclass->input['doprevious'] and $this->ipsclass->input['previd'] > 0 )
		{
			$this->ipsclass->main_msg = 'Маски доступа изменены';
		...
		{
			$this->ipsclass->admin->done_screen("Маски доступа изменены", "Управление форумами", $this->ipsclass->form_code, 'redirect' );
		}
	}
	...
	function reorder_form()
	{
		...
		if ( ! $this->ipsclass->input['f'] )
		{
			$this->ipsclass->admin->error("Вы не указали ID форума...");
		}

		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам изменить порядок отображения (сортировку) форумов и категорий, достаточно ввести лишь порядковый номер для каждого форума.";
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->end_form_standalone("Пересортировать");
	...
	function show_forums()
	{
		...
		$this->ipsclass->admin->page_title   = "Управление форумами";
		$this->ipsclass->admin->page_detail  = "В этой секции вы можете создавать новые, изменять и удалять существующие форумы. А также устанавливать правила и стили для каждого форума.";
		...
		if ( $this->ipsclass->input['f'] )
		{
			...
			if ( is_array($nav) and count($nav) > 1 )
			{
				...
				$this->ipsclass->html .= "<div class='navstrip'><a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}'>Форумы</a> &gt; ".implode( " &gt; ", $nav )."</div><br />";
			}
		}
		...
		$this->ipsclass->DB->simple_construct( array( 'select' => '*', 'from' => 'moderators' ) );
		$this->ipsclass->DB->simple_exec();
		...
		$this->ipsclass->admin->nav[] = array( '', 'Настройка форумов' );
	...
	function build_group_perms( $show='*', $read='*', $write='*', $reply='*', $upload='*', $download='*', $title="Маски доступа" )
	{
		//-----------------------------------------
	...
	function build_group_cat_perms( $show='*', $title='Маски доступа' )
	{
		//-----------------------------------------
		// Load skin if required
		//-----------------------------------------
