...
+---------------------------------------------------------------------------
|   > $Date: 2006-12-04 14:32:28 -0600 (Mon, 04 Dec 2006) $
|   > $Revision: 759 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_emoticons {
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление смайликами";
		$this->ipsclass->admin->nav[] 		= array( $this->ipsclass->form_code, 'Управление смайликами' );
	...
	function emoticon_setalter($type='add')
	{
		...
		if ($name == "")
		{
			$this->ipsclass->main_msg = "Вы указали недопустимое название директории. <br />В имени директории допустимы только символы: A-Z, a-z, 0-9.";
			$this->emoticon_start();
		}
		...
		if ( SAFE_MODE_ON )
		{
			$this->ipsclass->main_msg = "Безопасный режим: IPB не может создать или изменить директорию.<br />Пожалуйста, создайте или удалите директорию при помощи FTP-программы в /style_emoticons/.";
			$this->emoticon_start();
		}
		...
		{
			$this->ipsclass->main_msg = "Директория «style_emoticons/{$name}» уже существует.<br />Пожалуйста, выберете другие имя.";
			$this->emoticon_start();
		}
		...
		if ( $type == 'add' )
		{
			...
			if ( @mkdir( CACHE_PATH.'style_emoticons/'.$name, 0777 ) )
			{
				...
				$this->ipsclass->main_msg = "Новая директория добавлена";
				$this->emoticon_start();
			}
			else
			{
				$this->ipsclass->main_msg = "IPB не может создать или отредактировать директорию за вас, пожалуйста, создайте сами (или удалите) директорию при помощи FTP в 'style_emoticons'";
				$this->emoticon_start();
			}
		}
		else
		{
			...
			{
				$this->ipsclass->main_msg = "Директории с таким названием нет.";
				$this->emoticon_start();
				return;
			}
			...
			{
				$this->ipsclass->main_msg = "Вы не можете переименовать эту директорию.";
				$this->emoticon_start();
				return;
			}
			...
			if ( @rename( CACHE_PATH.'style_emoticons/'.$this->ipsclass->input['id'], CACHE_PATH.'style_emoticons/'.$name ) )
			{
				...
				$this->ipsclass->main_msg = "Директория переименована.";
				$this->emoticon_start();
			}
			else
			{
				$this->ipsclass->main_msg = "IPB не может переименовать эту директорию за вас.";
				$this->emoticon_start();
			}
		}
	}
	...
	function emoticon_remove()
	{
		...
		if ($this->ipsclass->input['id'] != "default" )
		{
			$this->ipsclass->main_msg = "Вы можете только добавлять,изменять или удалять смайлики в этой директории.";
			$this->emoticon_start();
		}

		if ($this->ipsclass->input['eid'] == "")
		{
			$this->ipsclass->main_msg = "Не выбран ID смайликов";
			$this->emoticon_manage();
		}
		...
		$this->ipsclass->main_msg = "Смайлик удален";
	...
	function emoticon_add()
	{
		if ($this->ipsclass->input['id'] == "")
		{
			$this->ipsclass->main_msg = "Не выбран ID группы иконок.";
			$this->emoticon_start();
		}
	...
	function emoticon_upload()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы не выбрали смайлик чтобы произвести загрузку!";
			$this->emoticon_start();
		}
		...
		foreach( array( 1,2,3,4 ) as $i )
		{
			...
			{
				$this->ipsclass->main_msg = "Вы можете загружать файлы только с расширением jpeg, jpg, gif и png.";
				$this->emoticon_start();
			}
			...
			{
				$this->ipsclass->main_msg = "Невозможно произвести запись в указанную директорию.";
				$this->emoticon_start();
			}
			else
			{
		...
		$this->ipsclass->main_msg = "Загрузка завершена!";
		$this->emoticon_start();
	}
	...
	function emoticon_start()
	{
		...
		{
			$this->ipsclass->admin->error("Нельзя найти директорию со смайликами — убедитесь, что путь к директории /style_emoticons/ указан верно.");
		...
		foreach( $emodirs as $dir )
		{
			...
			{
				$data['line_image'] = '';
				$data['link_text'] = "Управление смайликами";
			}
			else
			{
				$data['link_text'] = "Сделать нажимаемым";
			...
			if ( is_writeable( CACHE_PATH . '/style_emoticons/'.$dir ) )
			{
				...
				$data['title']    = 'В эту директорию можно произвести запись — вы можете добавить в нее новые смайлики';
				$data['checkbox'] = "<input type='checkbox' name='dir_{$dir}' {$checked_def}value='1' />";
			}
			else
			{
				$data['icon']     = 'icon_cannot_write.gif';
				$data['title']    = 'В эту директорию нельзя произвести запись — вам необходимо изменить атрибуты доступа (CHMOD)';
				$data['checkbox'] = "-";
			}
		...
		$this->ipsclass->admin->page_detail = "В этой секции вы можете управлять смайликами системы.";
		$this->ipsclass->admin->page_title   = "Управление смайликами";
	...
	{
		$this->ipsclass->admin->page_detail = "Удалить IPB пак смайликов.";
		$this->ipsclass->admin->page_title  = "Управление смайликами";

		if ( ! $this->ipsclass->input['id'] )
		{
			$this->ipsclass->main_msg = "Не выбран ни один смайлик.";
			$this->emoticon_start();
		}
		...
		$this->ipsclass->main_msg = "Директория со смайликами успешно удалена.";
		$this->emoticon_start();
	}
	...
	{
		$this->ipsclass->admin->page_detail = "В этой секции вы можете произвести импортирование или экспортирование смайликов.";
		$this->ipsclass->admin->page_title  = "Импорт и экспорт смайликов";
		$this->ipsclass->admin->nav[] 		= array( '', 'Импорт и экспорт смайликов' );

		if ( ! is_dir( CACHE_PATH. 'style_emoticons') )
		{
			$this->ipsclass->admin->error("Нельзя найти месторасположение директории со смайликами — убедитесь что путь к /style_emoticons/ указан верно.");
			$this->ipsclass->admin->output();
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Экспорт директории" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													            "<b>Какую директорию смайликов экспортировать?</b><div style='color:gray'>Содержимое директории будет упаковано в XML-архив с изображениями и марекарми вызова смайликов (например :smile:)</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Импортирование директории" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													  		"<b>В какую существующую директорию произвести импортирование?</b>",
													  		$this->ipsclass->adskin->form_dropdown( 'emo_set', $emodd )
													   )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													  		"<b><u>ИЛИ</u> Импортировать в новую директорию:</b><div style='color:gray'>Введите название новой директории смайликов.</div>",
													  		$this->ipsclass->adskin->form_input( 'new_emo_set' )
													   )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													 		 "<b>Перезаписать существующие изображение и маркеры вызова смайликов?</b><div style='color:gray'>Если выбрано «Да», новые изображения заменят старые.</div>",
													  		$this->ipsclass->adskin->form_yes_no( 'overwrite' )
													   )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													 		 "<b>Загрузка XML-архива смайликов</b><div style='color:gray'>Выберите файл для загрузки с вашего компьютера 'ipb_emoticons.xml' или 'ipb_emoticons.xml.gz'</div>",
	...
	function emoticon_pack_export()
	{
		...
		{
			$this->ipsclass->main_msg = "Вы должны указать какую директорию смайликов экспортировать.";
		}
	...
	function emoticon_pack_import()
	{
		...
		if ( ! $content )
		{
			$this->ipsclass->main_msg = "Импортирование не соверешно, поскольку в файле «ipb_emoticons.xml» ошибка или он не имеет записей.";
			$this->emoticon_pack_splash();
		}
		...
		{
			$this->ipsclass->main_msg = "Вы должны указать в какую директорию произвести импортирование новых смайликов.";
		}
		...
		if ( $this->ipsclass->input['new_emo_set'] )
		{
			...
			{
				$this->ipsclass->main_msg = "Директория «style_emoticons/{$emo_set_dir}» уже существует, выберите другие имя.";
				$this->emoticon_pack_splash();
			}
		...
		$this->ipsclass->main_msg = "Импортирование XML-архива смайликов завершено.";
	...
	function emoticon_manage()
	{
		...
		$this->ipsclass->admin->nav[] = array( '', "Управление директорией «{$this->ipsclass->input['id']}»" );

		if( $this->ipsclass->input['id'] == 'default' )
		{
			$this->ipsclass->admin->page_detail = "В этой секции вы можете добавлять, изменять и  удалять смайлики.<br />";
		}

		$this->ipsclass->admin->page_detail .= "Атрибут &laquo;Нажимаемый&raquo; означает, что смайлик будет выведен в таблице смайликов при создании темы/сообщения";

		if( $this->ipsclass->input['id'] == 'default' )
		{
			$this->ipsclass->admin->page_detail = "<br /><strong>В названиях смайликов нельзя использовать символ &quot;.";
		}
		...
		$this->ipsclass->html .= "<div class='tableborder'>
							 <div class='tableheaderalt'>Список установленных смайликов в «{$this->ipsclass->input['id']}»</div>
		...
		foreach( $emo_db as $image => $data )
		{
			...
			if( $this->ipsclass->input['id'] == 'default' )
			{
				$smilies .= "<a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=emo_remove&eid={$data['id']}&id={$this->ipsclass->input['id']}' title='Удалить'><img src='{$this->ipsclass->skin_acp_url}/images/emo_delete.gif' border='0' alt='Удалить' /></a>
						  	<br />
			...
			$smilies .= "<br /><br />Нажимаемый? <input type='checkbox'  name='emo_click_{$data['id']}' value='1' {$click} />
						  </fieldset>
						 </td>";
		...
		$this->ipsclass->html .= "</table>
							<div class='tablesubheader' align='center'><input type='submit' class='realbutton' value='Обновить' /></form></div></div><br />";
		...
		{
			$this->ipsclass->html .= "<div class='tableborder'>
								<div class='tableheaderalt'>Список неустановленных смайликов в «{$this->ipsclass->input['id']}»</div>
			...
			foreach( $emo_file as $image )
			{
				...
				$smilies .= "<td width='{$td_width}%' align='center' class='tablerow1'>
							  <fieldset>
								<legend><strong>{$image}</strong></legend>
								<img src='style_emoticons/{$this->ipsclass->input['id']}/{$image}' border='0' />&nbsp;&nbsp;<b>Добавить</b> <input name='emo_add_{$master_count}' type='checkbox' value='1' />
								<br />
								Вызов: <input type='textinput' class='realbutton' size='10' name='emo_type_{$master_count}' value='$poss_name' />
								<br /><br />Нажимаемый? <input type='checkbox' name='emo_click_{$master_count}' value='1' />
			...
			$this->ipsclass->html .= "</table>
								<div class='tablesubheader' align='center'><input type='submit' class='realbutton' value='Добавить выбранные' />&nbsp;&nbsp;<input type='submit' name='addall' class='realbutton' value='Добавить все' /></form></div></div>";
		}
