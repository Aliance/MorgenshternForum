...
class ad_acppermissions
{
	...
	*/
	...
	function auto_run()
	{
		...
		{
			$this->ipsclass->admin->error("Извините, но эти функции доступны только для главных администраторов");
		}
	...
	*/
	function accperms_member_remove()
	{
		...
		if ( ! $member_id )
		{
			$this->ipsclass->main_msg = "Вы не указали ID пользователя";
		}
		...
		$this->ipsclass->main_msg = "Ограничения доступа в админцентр удалены";
		$this->acpperms_list();
	}
	...
	*/
	function acpperms_xml_show_global()
	{
		...
		$lang_map         = array( 'add'     => 'Разрешить ДОБАВЛЕНИЕ',
								   'edit'    => 'Разрешить РЕДАКТИРОВАНИЕ',
								   'remove'  => 'Разрешить УДАЛЕНИЕ',
								   'import'  => 'Разрешить ИМПОРТИРОВАНИЕ',
								   'export'  => 'Разрешить ЭКСПОРТИРОВАНИЕ',
								   'rebuild' => 'Разрешить ОБНОВЛЕНИЕ',
								   'recount' => 'Разрешить ПЕРЕСЧЕТ',
								   'recache' => 'Разрешить КЕШИРОВАНИЕ',
								   'view'    => 'Разрешить ПРОСМОТР',
								   'search'  => 'Разрешить ПОИСК',
								   'log'     => 'Разрешить УПРАВЛЕНИЕ ЛОГАМИ',
								   'show'    => 'Разрешить ПРОСМОТР ОСНОВНОГО',
								   'upload'  => "Разрешить ЗАГРУЗКУ",
								   'do'		 => "Разрешить ПРИМЕНЕНИЕ",
								  );
	...
	*/
	function acpperms_member_add_complete()
	{
		...
		if ( ! $name )
		{
			$this->ipsclass->main_msg = "Вы не указали имя пользователя.";
			$this->acpperms_member_add();
		}
		...
		if ( ! $member['id'] )
		{
			$this->ipsclass->main_msg = "Пользователь «{$name}» не найден.";
			$this->acpperms_member_add();
		}
		...
		{
			$this->ipsclass->main_msg = "Пользователь «{$name}» уже имеет ограничения доступа в админцентр.";
			$this->acpperms_member_add();
		}
		...
		{
			$this->ipsclass->main_msg = "Пользователь «{$name}» не может быть ограничен, поскольку находится в группе главных администраторов.";
			$this->acpperms_member_add();
		}
		...
		if ( ! $isok )
		{
			$this->ipsclass->main_msg = "Пользователя «{$member['members_display_name']}» нельзя добавить, поскольку он не имеет доступ в админцентр.";
			$this->acpperms_member_add();
		}
		...
		$this->ipsclass->main_msg = "Пользователь «{$member['members_display_name']}» успешно добавлен и сейчас не имеет доступа в админцентр, но вы можете дать ему права через управление ограничениями.";
		$this->acpperms_list();
	}
	...
	*/
	...
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Управление ограничениями' );
		$this->ipsclass->admin->nav[] = array( ''                        , 'Добавить администратора' );
		...
		$this->ipsclass->admin->page_title  = "Ограничение администратора";
		$this->ipsclass->admin->page_detail = "Эта секция позволит вам управлять ограничением прав доступа в админцентр для администраторов.";
	...
	*/
	function acpperms_xml_import( $noreturn=0 )
	{
		...
		if ( $_FILES['FILE_UPLOAD']['name'] == "" or ! $_FILES['FILE_UPLOAD']['name'] or ($_FILES['FILE_UPLOAD']['name'] == "none") )
		{
			...
			{
				$this->ipsclass->main_msg = "Указанный файл не найден.";
				$this->acpperms_list();
			}
			
			if ( ! file_exists( ROOT_PATH . $this->ipsclass->input['file_location'] ) )
			{
				$this->ipsclass->main_msg = "Невозможно найти файл: " . ROOT_PATH . $this->ipsclass->input['file_location'];
				$this->acpperms_list();
			}
		...
		$this->ipsclass->main_msg = "Добавлено ограничений: $inserted";
	...
	*/
	...
	{
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Права доступа в админцентр: инструменты разработчика' );
		...
		$this->ipsclass->html .= "<textarea style='width:100%;height:400px'><"."?php\n#Не заполняйте пустые поля, они заполнятся автоматически\n\$lang = array(\n{$content}\n);\n?"."></textarea>";
		
		$this->ipsclass->admin->page_title  = "Права доступа администраторов";
		$this->ipsclass->admin->page_detail = "Эта секция позволит вам управлять правами доступа администраторов.";
		$this->ipsclass->admin->output();
	}
	...
	*/
	function acpperms_dev_report_missing()
	{
		...
		foreach( $master as $_name => $data )
		{
			...
			{
				$content .= "<span style='color:red;font-weight:bold'>{$_name}.php нет информации о правах доступа....</span><br />";
			}
			else
			{
				$content .= "<span style='color:green'>{$_name}.php есть информация о правах доступа....</span><br />";
			}
		}
	...
	*/
	function acpperms_list()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление ограничениями";
		$this->ipsclass->admin->page_detail = "Эта секция позволит вам управлять ограничением прав администраторов.";
		$this->ipsclass->admin->output();
	}
