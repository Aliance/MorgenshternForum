...
class ad_attachments {
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->nav[] = array( $this->ipsclass->form_code, 'Управление прикрепляемыми файлами' );

		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам управлять прикрепленными пользователями файлами и устанавливать разрешения для различных типов файлов.";
		$this->ipsclass->admin->page_title  = "Управление прикрепляемыми файлами";
	...
	function attach_search_complete()
	{
		...
		$this->ipsclass->adskin->td_header[] = array( "Файл", "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Размер"      , "10%" );
		$this->ipsclass->adskin->td_header[] = array( "Автор"    , "15%" );
		$this->ipsclass->adskin->td_header[] = array( "Тема"     , "25%" );
		$this->ipsclass->adskin->td_header[] = array( "Дата    ", "25%" );
		$this->ipsclass->adskin->td_header[] = array( "&nbsp;"    , "1%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Прикрепляемые файлы: результаты поиска" );
		...
		$removebutton = "<input type='submit' value='Удалить выбранные файлы' class='realdarkbutton'></form>";
	...
	function attach_search_start()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Поиск прикрепленных файлов" );
		...
		$gt_array = array( 0 => array( 'gt', 'Больше чем' ), 1 => array( 'lt', 'Меньше чем' ) );

		//-----------------------------------------
		// FORM
		//-----------------------------------------

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Расширение файла</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
												 				 $this->ipsclass->adskin->form_simple_input( 'extension', isset($_POST['extension']) ? $_POST['extension'] : '', 10 ),
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Размер файла (в килобайтах)</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Добавлен <em>n</em> дней</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
																 $this->ipsclass->adskin->form_dropdown( 'days_gt', $gt_array, isset($_POST['days_gt']) ? $_POST['days_gt'] : '' ).' '.
												 				 $this->ipsclass->adskin->form_simple_input( 'days', isset($_POST['days']) ? $_POST['days'] : '', 10 ).' дней назад',
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Просмотрен <em>n</em> дней</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
																 $this->ipsclass->adskin->form_dropdown( 'hits_gt', $gt_array, isset($_POST['hits_gt']) ? $_POST['hits_gt'] : '' ).' '.
												 				 $this->ipsclass->adskin->form_simple_input( 'hits', isset($_POST['hits']) ? $_POST['hits'] : '', 10 ).' раз',
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Имя файла</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
												 				 $this->ipsclass->adskin->form_simple_input( 'filename', isset($_POST['filename']) ? $_POST['filename'] : '', 30 ),
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Автор</b><div style='color:gray'>Оставьте пустым, чтобы пропустить при поиске</div>",
												 				 $this->ipsclass->adskin->form_input( 'authorname', isset($_POST['authorname']) ? $_POST['authorname'] : '', 'text', "id='authorname'" ),
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Искать только изображения?</b><div style='color:gray'>Если выбрано «Да», в результатах поиска будут только изображения.</div>",
												 				 $this->ipsclass->adskin->form_yes_no( 'onlyimage', isset($_POST['onlyimage']) ? $_POST['onlyimage'] : '', 30 ),
														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Сортировать результаты по</b>",
																 $this->ipsclass->adskin->form_dropdown( 'orderby', array( 0 => array( 'date'    , 'дате прикрепления'      ),
																 										             1 => array( 'hits'    , 'количеству просмотров'     ),
																 										             2 => array( 'filesize', 'размеру файла' ),
																 										             3 => array( 'file'    , 'имени файла' ),
																 										           ), isset($_POST['orderby']) ? $_POST['orderby'] : '' ).' '.
																 $this->ipsclass->adskin->form_dropdown( 'sort'   , array( 0 => array( 'desc'   , 'по убыванию [9-0]'  ),
																 													 1 => array( 'asc'    , 'по возрастанию [0-9]'   ),
																 										           ), isset($_POST['sort']) ? $_POST['sort'] : '' )


														)      );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Показать <em>n</em> результатов</b><div style='color:gray'>Не более 100 за раз</div>",
	...
	function attach_bulk_remove()
	{
		...
		$this->ipsclass->main_msg = "Прикрепленные файлы удалены";
	...
	function attach_stats_start()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Прикрепленные файлы: обзор" );

		$stats = $this->ipsclass->DB->simple_exec_query( array( 'select' => 'count(*) as count, sum(attach_filesize) as sum',
 																'from'   => 'attachments' ) );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Количество файлов</b>" , $this->ipsclass->do_number_format($stats['count']),
																 			 "<b>Занимают места на диске</b>", $this->ipsclass->size_format($stats['sum']),
													  				 )      );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Прикрепленные файлы: 5 последних" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Прикрепленные файлы: 5 самых больших" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Прикрепленные файлы: 5 самых просматриваемых" );
	...
	function attach_type_import()
	{
		...
		if ( ! $content )
		{
			$this->ipsclass->main_msg = "Импортирование не выполнено, поскольку файл «ipb_attachtypes.xml» содержит ошибку, либо он пустой.";
			$this->attach_type_start();
		}
		...
		$this->ipsclass->main_msg = "Импортирование типов прикрепляемых файлов успешно завершено.";
	...
	function attach_type_delete()
	{
		...
		$this->ipsclass->main_msg = "Тип прикрепляемого файла удален";
	...
	function attach_type_save( $type='add' )
	{
		...
		{
			$this->ipsclass->main_msg = "Вы не указали расширение и MIME-тип файла.";
		...
		if ( $type == 'add' )
		{
			...
			if ( $attach['atype_id'] )
			{
				$this->ipsclass->main_msg = "Расширение файла «{$save_array['atype_extension']}» уже существует. Пожалуйста, выберите другое расширение файла.";
				$this->attach_type_form($type);
			}

			$this->ipsclass->DB->do_insert( 'attachments_type', $save_array );

			$this->ipsclass->main_msg = "Тип прикрепляемого файла добавлен.";
		...
		else
		{
			...
			$this->ipsclass->main_msg = "Тип прикрепляемого файла изменен.";
		}
	...
	function attach_type_form( $type='add' )
	{
		...
		$this->ipsclass->admin->nav[] = array( '', 'Добавление/изменение типов файлов' );
		...
		if ( $type == 'add' )
		{
			...
			while( $r = $this->ipsclass->DB->fetch_row() )
			{
				$dd .= "<option value='{$r['atype_id']}'>Основан на типе: {$r['atype_extension']}</option>\n";
			}
			...
			$title = "
					  ...
					  <select name='baseon' class='realbutton'>{$dd}</select> &nbsp;<input type='submit' value='Применить' class='realdarkbutton' />
					  </form>
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Расширение прикрепляемого файла</b><div style='color:gray'>Обычно состоит из трех символов, является суффиксом файла.<br />Не требуется вводить точку («.») до расширения файла.</div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>MIME-тип файла</b><div style='color:gray'>Не уверены какой MIME-тип у выбранного файла?<br /><a href='http://www.utoronto.ca/webdocs/HTMLdocs/Book/Book-3ed/appb/mimetype.html' target='_blank'>Попробуйте найти здесь</a></div>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Разрешить прикреплять этот тип файла к сообщениям?</b>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Разрешить использовать этот тип файла как аватар или фотографию?</b>",
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Иконка этого типа файлов</b><div style='color:gray'>Маленькая иконка, указывающая тип файла в сообщении.</div>",
	...
	function attach_type_start()
	{
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Импортирование списка прикрепляемых файлов" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array(
													 		    "<b>Загрузка XML-архива со списком прикрепляемых файлов</b><div style='color:gray'>Выберите файл для загрузки с вашего компьютера 'ipb_attachtypes.xml' или 'ipb_attachtypes.xml.gz'. Двойные записи импортированы не будут.</div>",
