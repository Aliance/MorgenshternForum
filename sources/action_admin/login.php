...
class ad_login
{
	...
	function login_complete()
	{
		...
		{
			$this->login_form("Вы должны ввести имя пользователя перед тем, как продолжить");
		}
		
		if ( empty($this->ipsclass->input['password']) )
		{
			$this->login_form("Вы должны ввести пароль перед тем, как продолжить");
		}
		...
		if ( ( ! $mem['id'] ) or ( $this->han_login->return_code == 'NO_USER' ) )
		{
			...
			$this->login_form( "Имя пользователя или пароль не верны");
		}
		...
		if ( $this->han_login->return_code != 'SUCCESS' )
		{
			...
			{
				$this->write_to_log( $this->ipsclass->input['username'], 'fail' );
				$this->login_form( "Ваша учетная запись заблокирована" );
			}
			else
			{
				$this->write_to_log( $this->ipsclass->input['username'], 'fail' );
		...
		{
			$this->write_to_log( $this->ipsclass->input['username'], 'fail' );
			$this->login_form("У вас нет прав доступа в АЦ");
		}
		else
		{
			...
			$this->ipsclass->admin->redirect( $this->ipsclass->vars['board_url'].'/'.IPB_ACP_DIRECTORY."/index.".$this->ipsclass->vars['php_ext']."?adsess=".$this->ipsclass->input['adsess']."&".$extra_query, 'Вход осуществлен' );
	...
	*/
	...
	function login_form($message='')
	{
		...
		if ( $mid > 0 )
		{
			$this->ipsclass->DB->build_query( array( 'select' 	=> 'm.id, m.name, m.mgroup, g.g_access_cp',
		...
		$this->ipsclass->html_title = "IPB: Админцентр: Вход";
