...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-27 17:07:12 -0600 (Mon, 27 Nov 2006) $
|   > $Revision: 741 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_member_tools
{
	...
	*/
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->page_title  = "Инструменты IP";
		$this->ipsclass->admin->page_detail = 'Эта секция позволяет вам быстро и просто оперировать с IP-адресами пользователей.';
	...
	function learn_ip()
	{
		...
		{
			$this->show_index("Вы должны ввести IP-адрес для поиска");
		}

		$ip = trim($this->ipsclass->input['ip']);

		$resolved = 'Недоступно (частичный IP-адрес)';
		$exact    = 0;
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Хост {$this->ipsclass->input['ip']}" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>IP-адрес определяется как</b>" ,
																 $resolved
													    )      );
		...
		$this->ipsclass->adskin->td_header[] = array( "E-mail"      , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Сообщений"      , "10%" );
		$this->ipsclass->adskin->td_header[] = array( "IP-адрес"         , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Зарегистрирован" , "20%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Пользователи, использовавшие этот IP при РЕГИСТРАЦИИ" );
		...
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_basic( "Записей не найдено", "center");
		}
		else
		{
		...
		$this->ipsclass->adskin->td_header[] = array( "Пользователь"       , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "E-mail"      , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "IP-адрес"         , "15%" );
		$this->ipsclass->adskin->td_header[] = array( "Впервые использован"  , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Сообщение"  , "15%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Пользователи использовавшие этот IP при НАПИСАНИИ СООБЩЕНИЯ" );
		...
		else
		{
			while ( $m = $this->ipsclass->DB->fetch_row() )
			{
				$m['name'] = $m['name'] ? $m['name'] : "Гость";
				$m['email'] = $m['email'] ? $m['email'] : "<i>Недоступно</i>";

				$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( $m['name'] ,
																		 $m['email'],
																		 $m['ip_address'],
																		 $this->ipsclass->get_date( $m['post_date'], 'SHORT' ),
																		 "<center><a href='{$this->ipsclass->vars['board_url']}/index.php?showtopic={$m['topic_id']}&view=findpost&p={$m['pid']}' target='_blank'>Просмотр сообщений</a></center>",
																)      );
			}
		}
		...
		$this->ipsclass->adskin->td_header[] = array( "IP-адрес"         , "15%" );
		$this->ipsclass->adskin->td_header[] = array( "Впервые использован" , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Голосование" , "15%" );

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Пользователи использовавшие этот IP при ГОЛОСОВАНИИ" );
		...
		else
		{
			while ( $m = $this->ipsclass->DB->fetch_row() )
			{
				$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( $m['name'] ,
																		 ...
																		 "<center><a href='{$this->ipsclass->vars['board_url']}/index.php?showtopic={$m['tid']}' target='_blank'>Просмотр опросов</a></center>",
																)      );
			}
		}
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Пользователи использовавшие этот IP при ОТПРАВКЕ EMAIL" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Пользователи использовавшие этот IP при АКТИВАЦИИ УЧЕТНОЙ ЗАПИСИ" );
	...
	function show_ips()
	{
		...
		{
			$this->show_index("Вы должны ввести имя пользователя для поиска");
		}
		...
		if ( isset($this->ipsclass->input['member_id']) AND $this->ipsclass->input['member_id'] )
		{
			...
			{
				$this->show_index("Невозможно найти пользователя с ID «{$id}».");
			}
		}
		else
		{
			...
			if ( ! $member = $this->ipsclass->DB->fetch_row() )
			{
				$this->show_index( "Невозможно найти пользователя с таким именем, для дополнительных возможностей используйте форму ниже", $name );
			}
		}
		...
		$this->ipsclass->adskin->td_header[] = array( "IP-адрес"          , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Входов с этого IP"          , "10%" );
		$this->ipsclass->adskin->td_header[] = array( "Дата использования"           , "25%" );
		$this->ipsclass->adskin->td_header[] = array( "Использовано при других регистрациях" , "20%" );
		$this->ipsclass->adskin->td_header[] = array( "Инструмент IP"             , "25%" );

		//-----------------------------------------

		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "У пользователя {$member['name']} найдено ({$count['cnt']}) уникальных IP-адресов" );
		...
		foreach( $master as $r )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( $r['ip_address'] ,
																	 ...
																	 "<center><a href='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=learnip&ip={$r['ip_address']}'>Информация о IP-адресе</a></center>"
															)      );
		}
	...
	function show_index($msg="", $membername="")
	{
		if ($msg != "")
		{
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Сообщение" );
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Показать все IP-адреса, которые были у этого пользователя в момент добавления сообщений" );

		if ( $membername == "" )
		{
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Введите имя пользователя:</b>" ,
		...
		else
		{
			...
			if ( ! $this->ipsclass->DB->get_num_rows() )
			{
				$this->show_index("Не найдено пользователей, у которых имя начинается с '$membername'.");
			}
			...
			$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Выберите пользователя:</b>" ,
													  $this->ipsclass->adskin->form_dropdown( "member_id", $mem_array )
											 )      );
		}

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Найти IP-адреса этого пользователя");
		...
		$this->ipsclass->html .= $this->ipsclass->adskin->start_table( "Информация об IP-адресе" );

		$this->ipsclass->html .= $this->ipsclass->adskin->add_td_row( array( "<b>Вывести всю информацию об IP:</b>" ,
												   $this->ipsclass->adskin->form_input( "ip", isset($_POST['ip']) ? $this->ipsclass->txt_stripslashes($_POST['ip']) : '' )
										  )      );

		$this->ipsclass->html .= $this->ipsclass->adskin->end_form("Вывести!");
	...
	function member_do_moderation_queue()
	{
		...
		if ($this->ipsclass->input['type'] == 'approve')
		{
			...
			while( $row = $this->ipsclass->DB->fetch_row( $main ) )
			{
				...
				$email->subject = "Активация учетной записи «{$row['name']}» на «{$this->ipsclass->vars['board_name']}»";
				$email->to      = $row['email'];
			...
			$this->ipsclass->admin->save_log( count($ids)." учетных записей активировано: ".implode( ", ", $approved ) );
			...
			$this->ipsclass->main_msg = count($ids)." учетных записей активировано.";
			$this->member_view_moderation_queue();
		}
		...
		else if ($this->ipsclass->input['type'] == 'resend')
		{
			...
			while( $row = $this->ipsclass->DB->fetch_row( $main ) )
			{
				...
				if ( $row['lost_pass'] == 1 )
				{
					...
					$email->subject = "Информация о восстановление пароля на «{$this->ipsclass->vars['board_name']}»";
					$email->to      = $row['email'];
				...
				else if ( $row['new_reg'] == 1 )
				{
					...
					$email->subject = "Регистрация на ".$this->ipsclass->vars['board_name'];
					$email->to      = $row['email'];
				...
				else if ( $row['email_chg'] == 1 )
				{
					...
					$email->subject = "Запрос на изменение E-mail на ".$this->ipsclass->vars['board_name'];
					$email->to      = $row['email'];
			...
			if( count($resent) )
			{
				$this->ipsclass->admin->save_log( count($ids)." писем активации отправлено: ".implode( ", ", $resent) );
				$main_msgs[] = count($ids)." писем активации отправлено: ".implode( ", ", $resent);
			}

			if( count($cant) )
			{
				$main_msgs[] = "Невозможно отправить письмо активации пользователям, которые ожидают активацию администратором: ".implode( ", ", $cant);
			}
		...
		else
		{
			...
			$this->ipsclass->admin->save_log( count($ids)." регистраций пользователей не было одобрено: ".implode( ", ", $denied) );

			$this->ipsclass->main_msg = count($ids)." пользователей удалено.";
			$this->member_view_moderation_queue();
		}
	}
	...
	function member_do_email_unapprove()
	{
		...
		{
			$this->ipsclass->admin->error("Невозможно найти пользователя с указанным ID в неактивированных пользователях.");
		}
		...
		$this->ipsclass->admin->save_log( "Изменение e-mail адреса пользователем {$id} не одобрено.");


		$this->ipsclass->main_msg = "Изменение e-mail адреса пользователем {$id} не одобрено.";
		$this->member_view_moderation_queue();
	}
	...
	function member_view_locked_queue()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление заблокированными учетными записями";
		$this->ipsclass->admin->page_detail = "В этой секции вы можете разблокировать пользователей, которые исчерпали количество допустимых попыток авторизации на форуме.<br />Если необходимо, вы так же можете окончательно заблокировать учетные записи пользователей, выбрав соответсвующую опцию в выпадающем меню.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Заблокированные учетные записи' );
		...
		{
			$content = $this->html->member_locked_no_rows( "У вас выключена защита от автоматического перебора паролей." );
		}
		else
		{
			...
			if ($cnt > 0)
			{
				...
				while ( $r = $this->ipsclass->DB->fetch_row() )
				{
					...
					{
						$r['name'] = "<em>Пользователь удален</em>";
					}
			...
			{
				$content = $this->html->member_locked_no_rows( "Заблокированных учетных записей не найдено." );
			}
		}
	...
	function member_do_unlock()
	{
		...
		if ( count($ids) < 1 )
		{
			$this->ipsclass->admin->error("Вы не указали ID пользователя для разблокировки или блокироки.");
		}
		...
		if ($this->ipsclass->input['type'] == 'unlock')
		{
			...
			$this->ipsclass->admin->save_log( count($ids)." пользователей разблокировано");

			$this->ipsclass->main_msg = count($ids)." пользователей разблокировано";
			$this->member_view_locked_queue();
		}
		...
		else if ($this->ipsclass->input['type'] == 'ban')
		{
			if( $this->ipsclass->member['mgroup'] != $this->ipsclass->vars['admin_group'] )
			{
				...
				while( $mem = $this->ipsclass->DB->fetch_row() )
				{
					...
					{
						$this->ipsclass->admin->error( "Вы не можете блокировать главных администраторов." );
					}
				}
			}
			...
			if ( ! $banned_gid )
			{
				$this->ipsclass->admin->error( "У вас нет группы пользователей, которая не может просматривать форум — не удалось найти группу «Заблокированные»." );
			}

			$this->ipsclass->DB->do_update( "members", array( 'failed_logins' => '', 'failed_login_count' => 0, 'mgroup' => $banned_gid  ), "id IN(".implode( ',', $ids ).')' );

			$this->ipsclass->admin->save_log( count($ids)." пользователей заблокировано");

			$this->ipsclass->main_msg = count($ids)." пользователей заблокировано";
			$this->member_view_locked_queue();
		}
	}
	...
	function member_view_moderation_queue()
	{
		...
		$this->ipsclass->admin->page_title  = "Управление неактивированными или изменившими e-mail пользователями";
		$this->ipsclass->admin->page_detail = "В этой секции вы сможете просмотреть список всех пользователей, которые ещё не активировали свою учетную запись после регистрации или изменения e-mail адреса.<br />Ниже вы сможете выбрать учетные записи для активации или удаления, а так же повторной отправки письма с инструкциями.";
		$this->ipsclass->admin->nav[] 		= array( '', 'Неактивированные пользователи' );
		...
		if ($cnt > 0)
		{
			...
			while ( $r = $this->ipsclass->DB->fetch_row() )
			{
				...
				$r['_where'] = ( $r['lost_pass'] ? 'Восстановление пароля' : ( $r['new_reg'] ? "Регистрация <strong>(активация пользователем)</strong>" : ( $r['email_chg'] ? "Изменение e-mail" : 'Неизвестно' ) ) );
				...
				{
					$r['_where'] = "Регистрация: <strong>(активация администратором)</strong>";
				}
				...
				if ( !isset($r['name']) OR $r['name'] == "" )
				{
					$r['name'] = "<em>Удаленный пользователь</em>";
				}
		...
		{
			$content = $this->html->member_locked_no_rows( "Нет неактивированных пользователей." );
		}
