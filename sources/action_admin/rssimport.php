...
+---------------------------------------------------------------------------
|   > $Date: 2006-12-18 18:02:43 -0500 (Mon, 18 Dec 2006) $
|   > $Revision: 805 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class ad_rssimport
{
	...
	function auto_run()
	{
		...
		$this->ipsclass->admin->nav[] 		= array( $this->ipsclass->form_code, 'RSS Импорт' );
	...
	function rssimport_delete()
	{
		...
		$this->ipsclass->main_msg = "Испорт RSS строки удален";
		$this->rssimport_overview();
	}
	...
	function rssimport_remove_complete( $return=0, $rss_import_id=0 )
	{
		...
		if ( ! count( $remove_tids ) )
		{
			if ( $return )
			{
				$this->ipsclass->main_msg = "Нельзя определить место расположение тем для удаления";
				$this->rssimport_overview();
				return;
			}
			else
		...
		$this->ipsclass->main_msg = intval(count($remove_tids))." тем удалено.";
		$this->rssimport_overview();
	}
	...
	function rssimport_remove_dialogue()
	{
		...
		{
			$this->ipsclass->main_msg = "Импорт RSS строки '{$rssstream['rss_import_title']}' не был произведен.";
			$this->rssimport_overview();
			return;
		}
		...
		$this->ipsclass->admin->page_title  = "Управление импортом RSS";
		$this->ipsclass->admin->page_detail = "В этой секции вы можете удалять уже созданные статьи.";

		$this->ipsclass->admin->nav[]       = array( '', "Удаление RSS строки" );
	...
	function rssimport_rebuild_cache( $rss_import_id='', $return=1, $id_is_array=0 )
	{
		...
		while( $row = $this->ipsclass->DB->fetch_row( $outer ) )
		{
			if ( $rss_import_id == 'all' OR $row['rss_import_id'] == $rss_import_id OR ( $id_is_array == 1 AND in_array( $row['rss_import_id'], $rss_import_ids ) ) )
			{
				...
				{
					$rss_error[] = "Нельзя открыть {$row['rss_import_url']} &mdash; не найдено каналов.";
					continue;
				}
				...
				{
					$rss_error[] = "{$row['rss_import_url']} &mdash; нет данных в канале для импорта";
					continue;
				}
				...
				foreach ( $this->class_rss->rss_channels as $channel_id => $channel_data )
				{
					if ( is_array( $this->class_rss->rss_items[ $channel_id ] ) and count ($this->class_rss->rss_items[ $channel_id ] ) )
					{
						foreach( $this->class_rss->rss_items[ $channel_id ] as $item_data )
						{
							...
							{
							 	$rss_error[] = "Пропущено: '{$item_data['title']}' &mdash; нет названия или содержания";
								continue;
							}
				...
				{
					$rss_error[] = "Нет RSS записей для импортирования!";
					continue;
				}
				...
				foreach( $final_items as $topic_item )
				{
					...
					{
						$the_link = "";
						$the_link = str_replace( '{url}', $topic_item['link'], $row['rss_import_showlink'] );
						if ( $row['rss_import_allow_html'] )
						{
							$the_link = $this->parser->pre_display_parse( $this->parser->pre_db_parse( $this->editor->_rte_html_to_bbcode( stripslashes($the_link) ) ) );
						}
		...
		if ( $return )
		{
			$this->ipsclass->main_msg = "Импорт RSS, кеш пересчитан";
	...
	function rssimport_save($type='add')
	{
		...
		if( isset($this->ipsclass->input['rssimport_validate']) AND $this->ipsclass->input['rssimport_validate'] )
		{
			...
			{
				$this->ipsclass->main_msg = "<b>Результаты проверки для <span class='rss-feed-url'>".$this->ipsclass->txt_stripslashes( trim( $_POST['rss_import_url'] ) )."</span></b><br />&nbsp;&middot;".implode( "<br />&nbsp;&middot;", $this->validate_msg );
		...
		$rss_import_auth_user  = trim($this->ipsclass->input['rss_import_auth_user']) ? trim($this->ipsclass->input['rss_import_auth_user']) : 'Не требуется';
		$rss_import_auth_pass  = trim($this->ipsclass->input['rss_import_auth_pass']) ? trim($this->ipsclass->input['rss_import_auth_pass']) : 'Не требуется';
		...
		if( strtolower($rss_import_charset) != $this->class_rss->doc_type )
		{
			...
			$this->class_rss->destination_charset = $this->ipsclass->vars['gb_char_set'];
		}
		else
		{
			$this->class_rss->convert_charset = 0;
		...
		if ( ! is_array( $this->class_rss->rss_channels ) or ! count( $this->class_rss->rss_channels ) )
		{
			$rss_error[] = "Нельзя открыть $rss_import_url для локальных каналов.";
		}
		...
		{
			$this->ipsclass->main_msg = "Нельзя найти пользователя: '{$rss_import_mid}'";
			$this->rssimport_form( $type );
			return;
		}
		else
		...
		{
			$this->ipsclass->main_msg = "Выбранный форум для импорта не существует";
			$this->rssimport_form( $type );
			return;
		}
		...
		{
			$this->ipsclass->main_msg = "Выбранный форум для импорта не может принять темы.";
			$this->rssimport_form( $type );
			return;
		}
	...
	function rssimport_form( $type='add' )
	{
		...
		if ( $type == 'add' )
		{
			...
			$title    = "Создать строку RSS импорта";
			$button   = "Создать";
			$rssstream = array( 'rss_import_topic_open' => 1,
							    'rss_import_enabled' 	=> 1,
							    'rss_import_showlink' 	=> '[url={url}]Читать дальше[/url]',
							    'rss_import_title'		=> '',
							    'rss_import_url'		=> '',
		...
		else
		{
			...
			$title    = "Редактирование: ".$rssstream['rss_import_title'];
		...
		$this->ipsclass->admin->page_title  = "Управление импортом RSS";
		$this->ipsclass->admin->page_detail = "Эта секция позволяет вам управлять потоками импорта RSS.";

		$this->ipsclass->admin->nav[]       = array( "", "Управление RSS импортом" );
		$this->ipsclass->admin->output();
	}
	...
	function rssimport_validate( $standalone=0 )
	{
		...
		if( !$rss_import_url )
		{
			$this->validate_msg[] = $this->html->rss_validate_msg( array( 'msg' => "URL адрес для проверки не введен" ) );
		}
		else
		{
			//--------------------------------------------
			...
			else
			{
				if( $this->class_rss->orig_doc_type )
				{
					if( !$standalone AND $rss_import_charset )
					{
						...
						{
							$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																						  'msg' => "Кодировка RSS &mdash; <i>{$this->class_rss->orig_doc_type}</i>, но вы указали &mdash; <i>{$rss_import_charset}</i>" ) );
						}
						else
						{
							$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-valid',
																						  'msg' => "Кодировка RSS &mdash; <i>{$this->class_rss->orig_doc_type}</i> (корректная)" ) );
						}
					}
					else
					{
						$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-valid',
																					  'msg' => "Кодировка RSS &mdash; <i>{$this->class_rss->orig_doc_type}</i>" ) );
					}
				}
				else
				{
					$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																				  'msg' => "Невозможно определить кодировку RSS" ) );
				}
				...
				{
					$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																				  'msg' => "Невозможно найти какие-либо каналы в указанной RSS (нечего импортировать)." ) );
				}
				else
				{
					$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-valid',
																				  'msg' => "Найдено ".count($this->class_rss->rss_channels)." каналов в RSS." ) );
					...
					{
						$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																					  'msg' => "Не найдено настоящего контента (темы, новости и т.д.) (нечего импортировать)" ) );
					}
					else
					{
						foreach ( $this->class_rss->rss_channels as $channel_id => $channel_data )
						{
							...
							{
								$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-valid',
																						  	  'msg' => "Найдено ".count($this->class_rss->rss_items[ $channel_id ])." статей/тем в RSS." ) );
								...
								foreach( $this->class_rss->rss_items[ $channel_id ] as $item_data )
								{
									if( !$item_data['unixdate'] )
									{
										$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-msg',
																									  'msg' => "Дата не найдена в не менее, чем одной RSS статье - будет использоваться текущее время." ) );
									}

									if ( $item_data['unixdate'] < 1 )
									{
										$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-msg',
																									  'msg' => "Найдена некорректная дата в не менее, чем одной RSS статье - будет использоваться текущее время." ) );
									}
									else if ( $item_data['unixdate'] > time() )
									{
										$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-msg',
									...
									{
										$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																									  'msg' => "Не менее, чем одна статья имеет некорректный заголовок или содержание. Если все статьи имеют оба некорректных поля, ничего не импортируется. IPB пропустит эти данные." ) );
									}
			...
			if( !$standalone )
			{
				if( $rss_import_mid )
				{
					...
					{
						$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																					  'msg' => "Указанный пользователь '{$rss_import_mid}' не найден." ) );
					}
				}
				else
				{
					$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																				  'msg' => "Мы не можем разрешить пользователя, используемого для импорта статей." ) );
				}
			}
			...
			if( !$standalone AND $rss_import_forum_id )
			{
				...
				else
				{
					...
					{
						$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																					  'msg' => "Указанный форум &mdash; форум переадресации или категория, или же невозможно открывать новые темы в нем." ) );
					}

					if( $rss_import_inc_pcount AND !$this->ipsclass->forums->forum_by_id[ $rss_import_forum_id ]['inc_postcount'] )
					{
						$this->validate_msg[] = $this->html->rss_validate_msg( array( 'class' => 'rss-feed-invalid',
																					  'msg' => "В указанном форуме не включен счетчик сообщений пользователей &mdash; счетчик сообщений пользователя <i>не</i> будет увеличен." ) );
					}
				}
			}
			...
			if ( !$return )
			{
				if( count($this->validate_msg) )
				{
					$this->ipsclass->main_msg = "<b>Результаты проверки для <span class='rss-feed-url'>".$rss_import_url."</span></b><br />&nbsp;&middot;".implode( "<br />&nbsp;&middot;", $this->validate_msg );
					$this->rssimport_overview();
					return;
				}
			}
