...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-15 17:05:32 -0600 (Wed, 15 Nov 2006) $
|   > $Revision: 731 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class usercp
{
 	...
 	function display_name_complete()
	{
		...
		$this->ipsclass->DB->do_update( 'members'       , array( 'members_display_name' => $display_name ), "id="            .$this->ipsclass->member['id'] );
		$this->ipsclass->DB->do_update( 'members'       , array( 'members_l_display_name' => strtolower( $display_name ) ), "id=" .$this->ipsclass->member['id'] );
 	...
 	function mod_announce_save()
	{
		...
		if ( strstr( $this->ipsclass->input['announce_start'], '.' ) )
		{
			...
			{
				if ( ! checkdate( $start_array[1], $start_array[0], $start_array[2] ) )
				{
			...
			$start_date = $this->ipsclass->date_gmmktime( 0, 0, 1, $start_array[1], $start_array[0], $start_array[2] );
		}
		...
		if ( strstr( $this->ipsclass->input['announce_end'], '.' ) )
		{
			...
			{
				if ( ! checkdate( $end_array[1], $end_array[0], $end_array[2] ) )
				{
					$this->mod_announce_form( $type, $this->ipsclass->lang['announce_error_date'] );
					return;
				}
			...
			$end_date = $this->ipsclass->date_gmmktime( 23, 59, 59, $end_array[1], $end_array[0], $end_array[2] );
		}
		...
		$save_array = array( 'announce_title'        => $this->ipsclass->input['announce_title'],
							 'announce_post'         => $this->ipsclass->input['announce_post'],
 	...
 	function mod_announce_form($type='add', $msg="")
	{
		...
		else
		{
			...
			$announce['announce_start'] = $announce['announce_start'] ? gmdate( 'd.m.Y', $announce['announce_start'] ) : '';
			$announce['announce_end']   = $announce['announce_end']   ? gmdate( 'd.m.Y', $announce['announce_end'] ) : '';
		}
 	...
 	function mod_announce_start()
	{
		...
		while ( $r = $this->ipsclass->DB->fetch_row() )
		{
			if ( $r['announce_start'] )
			{
				$r['announce_starts_converted'] = gmdate( 'd.m.Y', $r['announce_start'] );
			}
			else
			{
			...
			if ( $r['announce_end'] )
			{
				$r['announce_end_converted'] = gmdate( 'd.m.Y', $r['announce_end'] );
			}
			else
			{
	...
	function mod_find_user_complete()
	{
 	...
 	function signature()
 	{
		...
		$this->ipsclass->member['signature'] = $this->parser->pre_display_parse( $this->parser->pre_db_parse( $this->ipsclass->member['signature'] ) );
 	...
 	function splash()
 	{
			...
			if ( count( $topic_array ) )
			{
				...
				while ( $row = $this->ipsclass->DB->fetch_row() )
				{
					if ( isset($this->ipsclass->forums->forum_by_id[ $row['forum_id'] ]) AND $this->ipsclass->forums->forum_by_id[ $row['forum_id'] ] )
					{
						...
						$final_array[ $row['tid'] ] = $this->ipsclass->compiled_templates['skin_forum']->render_forum_row( $topic );
					}
				}
		...
		if ( $this->ipsclass->DB->get_num_rows() )
		{
			...
			while ( $row = $this->ipsclass->DB->fetch_row() )
			{
				...
				$info['attach_html'] .= $this->ipsclass->compiled_templates['skin_topic']->Show_attachments( array (
