...
class xmlout
{
	...
	*/
    ...
    function profile_save_settings()
    {
		...
		$value         = $this->class_ajax->convert_and_make_safe( $_REQUEST['value'] );
		...
		switch( $command )
		{
			...
			case 'birthdate':
				...
				if ( ! @checkdate( $pp_b_month, $pp_b_day, $pp_b_year ) )
				{
					$return_string = 'dateerror';
				}
				else
				{
					...
					$return_string = $_pp_b_month . '-' . $pp_b_day . '-' . $pp_b_year;
				}
		}
    ...
    function save_member_rate()
    {
		...
		@header( "Content-type: text/plain; charset={$this->ipsclass->vars['gb_char_set']}" );
		$this->class_ajax->print_nocache_headers();
		print $member['pp_rating_value'].','.$member['pp_rating_hits'].','.$member['pp_rating_real'].','.$type;
		exit();
    }
	...
	{
		$img = trim( $this->ipsclass->txt_alphanumerical_clean( $_REQUEST['img'] ) );
    ...
    function save_topic_rate()
    {
		...
		@header( "Content-type: text/plain; charset={$this->ipsclass->vars['gb_char_set']}" );
		$this->class_ajax->print_nocache_headers();
		print $topic['topic_rating_total'].','.$topic['topic_rating_hits'].','.$topic['_rate_int'].','.$type;
		exit();
    }    
    ...
    function check_email_address()
    {
    	...
    	$email = $this->ipsclass->parse_clean_value( rawurldecode( $_REQUEST['email'] ) );
    ...
    function check_display_name( $field='members_display_name' )
    {
    	...
    	//$name = strtolower( $this->ipsclass->parse_clean_value( trim( rawurldecode( $_REQUEST['name'] ) ) ) );
    	$name = strtolower( $this->class_ajax->convert_and_make_safe( $_REQUEST['name'] ) );
    ...
    function post_edit_save()
    {
		...
		$_POST['Post'] = $this->class_ajax->convert_html_entities( $_POST['Post'] );
   		...
   		$this->ipsclass->input['post_edit_reason'] = $this->class_ajax->convert_unicode( rawurldecode( $_REQUEST['post_edit_reason'] ) );
		$this->ipsclass->input['post_edit_reason'] = $this->ipsclass->parse_clean_value( $this->class_ajax->convert_html_entities( $this->ipsclass->input['post_edit_reason'] ) );		
    ...
    function post_edit_show( $error_msg = "" )
    {
		...
		// Fix IE bugs
		$raw_post = str_replace( "&sect", 	"&amp;sect", 	$raw_post );
    ...
    function save_topic()
    {
    	...
    	$name      = $this->ipsclass->parse_clean_value( $this->class_ajax->convert_unicode($_REQUEST['name']) );
    	$type      = $this->ipsclass->parse_clean_value( rawurldecode( $_REQUEST['type'] ) );
    ...
    function get_member_names()
    {
    	...
    	$name = $this->class_ajax->convert_and_make_safe( $_REQUEST['name'] );
    ...
    function get_new_messages( $return=0 )
    {
		...
		if ( ! $return )
		{
			@header( "Content-type: text/plain;charset={$this->ipsclass->vars['gb_char_set']}" );
			$this->class_ajax->print_nocache_headers();
			print $this->ipsclass->compiled_templates['skin_msg']->xmlout_msgs_wrapper( $html );
			exit();
		}
		else
		{
    ...
    function mark_forum_as_read()
    {
        ...
        if ( $this->ipsclass->vars['db_topic_read_cutoff'] and $this->ipsclass->member['id'] )
        {
        	if ( count( $delete ) )
        	{
        		$this->ipsclass->DB->do_delete( 'topic_markers', 'marker_member_id='.$this->ipsclass->member['id'].' AND marker_forum_id IN ('.implode(',',$delete ).')' );
    ...
    function get_new_posts( $return=0 )
    {
		...
		if ( ! $return )
		{
			@header( "Content-type: text/plain;charset={$this->ipsclass->vars['gb_char_set']}" );
			$this->class_ajax->print_nocache_headers();
			print $this->ipsclass->compiled_templates['skin_search']->xmlout_gnp_wrapper( $html, $previous, $next, $st );
			exit();
		}
		else
		{
