...
class moderate
{
	...
	function edit_member()
	{
		...
		{
			$this->ipsclass->input['signature'] = $parser->pre_db_parse( $this->ipsclass->input['signature'] );
	...
	function multi_split_topic()
	{
		...
		else
		{
			...
			$this->ipsclass->DB->do_update( 'posts', array( 'topic_id' => $new_topic_id, 'new_topic' => 0 ), "pid IN($pid_string)" ); 
	...
	function prune_move()
	{
		...
		if ( ! $num_rows )
		{
			...
			$this->ipsclass->input['dateline'] = round((time()-$this->ipsclass->input['dateline'])/24/60/60);
	...
	function prune_start( $complete_html="" )
	{
		...
		if ( isset($this->ipsclass->input['check']) AND $this->ipsclass->input['check'] == 1 )
		{
			...
			$tcount = $this->ipsclass->DB->simple_exec_query( array( 'select' => 'COUNT(*) as tcount', 'from' => 'topics', 'where' => "approved=1 and forum_id={$this->forum['id']}" ) );
