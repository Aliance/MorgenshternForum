...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-09 15:32:37 -0600 (Thu, 09 Nov 2006) $
|   > $Revision: 722 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class register {
	...
	function create_account()
	{
		...
		if ( count( $fields->error_fields['empty'] ) )
		{
			...
			{
				$form_errors['cfield_'.$cfield['pf_id']][] = $this->ipsclass->lang['err_complete_form'];
			}
		}
		...
		if ( count( $fields->error_fields['invalid'] ) )
		{
			...
			{
				$form_errors['cfield_'.$cfield['pf_id']][] = $this->ipsclass->lang['err_invalid'];
			}
		}
		...
		if ( count( $fields->error_fields['toobig'] ) )
		{
			...
			{
				$form_errors['cfield_'.$cfield['pf_id']][] = $this->ipsclass->lang['err_cf_to_long'];
			}
		}
		...
		if( $this->ipsclass->vars['username_characters'] )
		{
			...
			{
				$form_errors['username'][] = str_replace( '{chars}', $this->ipsclass->vars['username_characters'], $this->ipsclass->vars['username_errormsg'] );
			}
			
			if( !preg_match( "/^[".$check_against."]+$/i", $_POST['members_display_name'] ) && $this->ipsclass->vars['auth_allow_dnames'] )
			{
				$form_errors['dname'][] = str_replace( '{chars}', $this->ipsclass->vars['username_characters'], $this->ipsclass->vars['username_errormsg'] );
			}
		}
		...
		{
			$form_errors['password'][] = $this->ipsclass->lang['reg_error_pass_nm'];
		}
		...
		if ( is_array( $banfilters['email'] ) and count( $banfilters['email'] ) )
		{
			foreach ( $banfilters['email'] as $email )
			{
				...
				if ( preg_match( "/^{$email}$/i", $in_email ) )
				{
					$form_errors['email'][] = $this->ipsclass->lang['reg_error_email_ban'];
					break;
				}
			}
		}
		...
		/**
		 * Aliance's plugin: check existing char in combats
		 */
		$dname = $this->ipsclass->vars['auth_allow_dnames'] ? $members_display_name : $in_username;
		$check_login_codes = $this->ipsclass->morg->check_login( $dname );
		$need_insert_morgenshtern_member = false;
		switch ( intval( $check_login_codes ) )
		{
			case 0:
				// Сервер БК заблокировал наш IP
				$this->ipsclass->Error( array( 'LEVEL' => 1, 'MSG' => 'dname_check_ip' ) );
			break;
			case 1:
				// Сервер БК недоступен ( Service Unavailable )
				$this->ipsclass->Error( array( 'LEVEL' => 1, 'MSG' => 'dname_check_unavailable' ) );
			break;
			case 2:
				// Чар не найден
				$form_errors['username'][] = 'Вы должны ввести Ваш логин в БК, чтобы зарегистрироваться.';
			break;
			case 3:
				// Ошибка cUrl
				$this->ipsclass->Error( array( 'LEVEL' => 1, 'MSG' => 'dname_check_curl_error' ) );
			break;
			case 4:
				// Все в порядке
				$need_insert_morgenshtern_member = true;
			break;
			default:
				$form_errors['username'][] = 'Вы должны ввести Ваш логин в БК, чтобы зарегистрироваться.';
			break;
		...
		$this->ipsclass->DB->force_data_type = array( 'name' => 'string', 'members_display_name' => 'string' );
		...
		//-----------------------------------------
		// Insert: MORGENSHTERN MEMBER
		//-----------------------------------------

		if ( $need_insert_morgenshtern_member )
		{
			$data = array(
				'id'	=>	$member_id,
				'nick'	=>	$dname
			);
			$this->ipsclass->DB->do_insert( 'morgenshtern_members', $data );
		}
	...
	function validate_user()
	{
		...
		else
		{
			...
			else if ($in_type == 'lostpass')
			{
				...
				if ( $this->ipsclass->vars['bot_antispam'] != 0 AND $this->ipsclass->input['regid'] != "" )
				{
					$this->ipsclass->DB->simple_construct( array( 'select' => '*',
																  'from'   => 'reg_antispam',
																  'where'  => "regid='".trim($this->ipsclass->txt_alphanumerical_clean($this->ipsclass->input['regid']))."'"
				...
				if ( $this->ipsclass->vars['lp_method'] == 'random' )
				{
					...
					$this->ipsclass->DB->do_update( 'members_converge', $save_array                        , 'converge_id='.intval( $member['id'] ) );
