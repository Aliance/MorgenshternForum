...
class calendar
{
    ...
    function cal_event_form( $type='add' )
    {
		...
		if ( $event['event_content'] )
		{
			if ( $this->post->han_editor->method == 'rte' )
			{
				...
				$event['event_content'] = $this->post->parser->pre_db_parse( $event['event_content'] );
				$event['event_content'] = $this->post->parser->convert_ipb_html_to_html( $event['event_content'] );	
			}
			else
			{
				$event['event_content'] = $this->ipsclass->my_br2nl($event['event_content']);
			}
		}
    ...
    function make_event_html($event)
    {
		...
		$this->post->parser->parse_smilies = $event['event_smilies'];
		$this->post->parser->parse_html    = 0;
		$this->post->parser->parse_bbcode  = 1;
		
        $event['event_content'] = $this->post->parser->pre_display_parse( $this->post->parser->pre_db_parse( $event['event_content'] ) );
        ...
        else
        {
	        ...
	        $member = $this->topiclib->parse_member( $member );
