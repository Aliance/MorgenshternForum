...
+---------------------------------------------------------------------------
|   > $Date: 2006-09-28 17:05:45 -0500 (Thu, 28 Sep 2006) $
|   > $Revision: 587 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class forums {
	...
	function render_forum()
	{
		...
		if ( is_array( $this->ipsclass->cache['announcements'] ) and count( $this->ipsclass->cache['announcements'] ) )
		{
			...
			if ( count( $announcements ) )
			{
				...
				foreach( $announcements as $announce )
				{
					...
					{
						$announce['announce_start'] = gmdate( 'd.m.Y', $announce['announce_start'] );
					}
					else
					{
		...
		if ( $this->ipsclass->vars['db_topic_read_cutoff'] AND $this->ipsclass->member['id'] AND count($topic_ids) )
		{
			 ...
			 OR  ( $this->forum['forum_last_deletion'] > $this->db_row['marker_last_update'] ) )
			{
				...
				{
					// Better safe than sorry - been seeing a lot of SQL errors popping up about the dup unique key
					
					$check = $this->ipsclass->DB->build_and_exec_query( array( 'select' => 'marker_forum_id', 'from' => 'topic_markers', 'where' => 'marker_member_id='.$this->ipsclass->member['id'].' AND marker_forum_id='.$this->forum['id'] ) );
					
					if( !$check['marker_forum_id'] )
					{
				...
				else
				{
					if (empty( $cached[ $result['member_id'] ] ) )
					{
						...
						if ( strstr( $result['location'], 'post' ) )
						{
							...
							$new_p_start = " class='activeuserposting'";
							$new_p_end = "</span>";
						}
						...
						if ($result['login_type'] == 1)
						{
							...
							{
								//-----------------------------------------
								// Отображение БКшных ников пользователях online
								//-----------------------------------------
								/*
								$this->ipsclass->DB->simple_select( '*', 'morgenshtern_members', 'member_id = ' . $result['member_id'] );
								$this->ipsclass->DB->simple_limit( 1 );
								$this->ipsclass->DB->exec_query();
								$data = $this->ipsclass->DB->fetch_row();
								$num_rows = $this->ipsclass->DB->get_num_rows();
								if ( $num_rows == 1 ) {
									$result['member_name'] = preg_replace_callback( "/" . $data['nick'] . "/m", array('self', 'char_online'), $result['member_name'] );
									$result['member_name'] = preg_replace( "/ title=''/m", $p_title, $result['member_name'] );
									$result['member_name'] = preg_replace( "/ class=''/m", $new_p_start, $result['member_name'] );
									$result['member_name'] = preg_replace( "#</span> #m", $new_p_end, $result['member_name'] );
	...
	function render_entry($topic)
	{
		...
		if ($topic['pinned'] == 1)
		{
			...
			//-----------------------------------------
			// Отображение БКшных ников
			//-----------------------------------------
			/*
			$this->ipsclass->DB->simple_select( '*', 'morgenshtern_members', 'nick = "' . $topic['starter_name'] . '"' );
			$this->ipsclass->DB->simple_limit( 1 );
			$this->ipsclass->DB->exec_query();
			$data = $this->ipsclass->DB->fetch_row();
			$num_rows = $this->ipsclass->DB->get_num_rows();
			if ( $num_rows == 1 ) {
				$topic['starter'] = preg_replace_callback( "/" . $data['nick'] . "/m", array(&$this->ipsclass, 'bk_char'), $topic['starter'] );
			}
			*/
			//$topic['starter'] = preg_replace_callback( "/" . $topic['starter_name'] . "/m", array(&$this->ipsclass, 'bk_char'), $topic['starter'] );
			//$topic['starter'] = $this->ipsclass->bk_char( array( $topic['starter_name'] ) );
			$char = array(
				'id'   => $topic['starter_id'],
				'name' => $topic['starter_name']
			);
			$topic['starter'] = $this->ipsclass->morg->formatChar( $char );

			/*$this->ipsclass->DB->simple_select( '*', 'morgenshtern_members', 'nick = "' . $topic['last_poster_name'] . '"' );
			$this->ipsclass->DB->simple_limit( 1 );
			$this->ipsclass->DB->exec_query();
			$data = $this->ipsclass->DB->fetch_row();
			$num_rows = $this->ipsclass->DB->get_num_rows();
			if ( $num_rows == 1 ) {
				$topic['last_poster'] = preg_replace_callback( "/" . $data['nick'] . "/m", array(&$this->ipsclass, 'bk_char'), $topic['last_poster'] );
			}*/
			//$topic['last_poster'] = preg_replace_callback( "/" . $data['last_poster_name'] . "/m", array(&$this->ipsclass, 'bk_char'), $topic['last_poster'] );
			$topic['last_poster'] = $this->ipsclass->bk_char( array( $topic['last_poster_name'] ) );
		...
		else
		{
			...
			//-----------------------------------------
			// Отображение БКшных ников в имени автора топика
			//-----------------------------------------
			$char = array(
				'id'   => $topic['starter_id'],
				'name' => $topic['starter_name']
			...
			//-----------------------------------------
			// Отображение БКшных ников в имени автора посл. сообщения
			//-----------------------------------------
			$char = array(
				'id'   => $topic['last_poster_id'],
				'name' => $topic['last_poster_name']
			);
			$topic['last_poster'] = $this->ipsclass->morg->formatChar( $char );
			//$topic['last_poster'] = $this->ipsclass->bk_char( array( $topic['last_poster_name'] ) );
	...
	/*-------------------------------------------------------------------------*/
	//
	// Функция для отображения ника пользователя
	//
	/*-------------------------------------------------------------------------*/

	function char_online( $matches )
	{
		try {
			$this->ipsclass->DB->simple_select( '*', 'morgenshtern_members', 'nick = "' . $matches[0] . '"' );
			$this->ipsclass->DB->simple_limit( 1 );
			$this->ipsclass->DB->exec_query();
			...
			if ( $this->ipsclass->DB->get_num_rows() == 1 ) {
				...
				$char = $clan . '<span onclick="window.open(\'' . $this->ipsclass->base_url . 'showuser=' . $result['id'] . '\');" class=\'\' style="cursor: pointer;" title=\'\'>' . $result['nick'] . '</span> [' . $result['level'] . ']' . $inf;
			} else {
				$inf = '<a href="http://emeraldscity.combats.com/inf.pl?login=' . $matches[0] . '" target="_blank"><img src="http://img.combats.com/i/inf.gif" alt="Информация о ' . $matches[0] . '" /></a>';
				$char = $matches[0] . ' ' . $inf;
			}
		} catch ( Exception $e ) {
			$inf = '<a href="http://emeraldscity.combats.com/inf.pl?login=' . $matches[0] . '" target="_blank"><img src="http://img.combats.com/i/inf.gif" alt="Информация о ' . $matches[0] . '" /></a>';
