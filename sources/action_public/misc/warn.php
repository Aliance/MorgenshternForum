...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-21 13:35:57 -0600 (Tue, 21 Nov 2006) $
|   > $Revision: 737 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class warn {
	...
	function do_warn()
	{
		...
		if ( $test_content != "" )
		{
			...
			$save['wlog_contact_content'] = $this->parser->pre_db_parse( $save['wlog_contact_content'] );
			...
			if ( $this->ipsclass->input['contactmethod'] == 'email' )
			{
				...
				$this->parser->parse_bbcode    = 1;
				$this->ipsclass->input['contact']        = $this->parser->pre_db_parse( $this->ipsclass->input['contact'] );
			...
			else
			{
				...
				$this->lib->to_by_id    = $this->warn_member['id'];
 				...
 				$this->lib->msg_post    = $this->ipsclass->remove_tags($this->ipsclass->input['contact']);
				$this->lib->force_pm    = 1;
