...
+---------------------------------------------------------------------------
|   > $Date: 2006-11-20 13:46:59 -0600 (Mon, 20 Nov 2006) $
|   > $Revision: 736 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class contactmember
{
	...
	function chat_display()
	{
		...
		$lang   = $this->ipsclass->vars['chat_language'] ? $this->ipsclass->vars['chat_language'] : 'ru';
	...
	function send_report()
	{
		...
		$report = $this->ipsclass->my_nl2br( htmlspecialchars($report, ENT_QUOTES) );
		...
		foreach( $mods as $data )
		{
			...
			else
			{
				$this->lib->to_by_id    = $data['id'];
 				...
 				$this->lib->msg_post    = htmlspecialchars( $this->email->message, ENT_QUOTES );
				$this->lib->force_pm    = 1;
	...
	function _allow_to_email($member_id, $email_limit)
	{
		...
		return TRUE; //936d62a249c0dc8fd81438cdbc911b98 If we get here...
