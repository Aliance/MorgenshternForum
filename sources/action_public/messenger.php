        ...
        $this->parser->bypass_badwords = intval($this->ipsclass->member['g_bypass_badwords']);
 		...
 		if ( $this->ipsclass->DB->get_num_rows() )
 		{
 			while ( $r = $this->ipsclass->DB->fetch_row() )
 			{
				...
				$info['msg_content'] = $this->parser->pre_display_parse( $this->parser->pre_db_parse( $r['msg_post'] ) );
 		...
 		foreach ($this->ipsclass->input as $key => $value)
 		{
 			if ( preg_match( "/^dir_(\d+)$/", $key, $match ) )
 			{
 				if ($this->ipsclass->input[$match[0]])
 				{
 					...
 					$v_dir .= '|'.$match[0].':'.trim(str_replace( '|', '', str_replace( ";", "", $this->ipsclass->input[$match[0]] ) ) ).';'.$count;
 				}
 			}
 		}
	...
	/*-------------------------------------------------------------------------*/
 	...
 	function multi_act()
 	{
		//-----------------------------------------
 		...
 		if ( $affected_ids > 0 )
 		{
 			...
 			{
 				$this->ipsclass->DB->simple_construct( array( 'update' => 'message_topics', 'set' => "mt_vid_folder='{$this->msglib->vid}', mt_to_id={$this->ipsclass->member['id']}", 'where' => "mt_vid_folder != '{$this->msglib->vid}' AND mt_owner_id=".$this->ipsclass->member['id']." AND mt_id IN ($id_string)" ) );
 				$this->ipsclass->DB->simple_exec();
 	...
 	function view_msg()
 	{
		//-----------------------------------------
 		...
 		require ( ROOT_PATH.'sources/action_public/topics.php' );
 		
 		$this->topiclib           =  new topics();
 		$this->topiclib->ipsclass =& $this->ipsclass;
 		
 		$this->topiclib->topic_init();
 		$this->topiclib->topic['forum_id'] = 0;
 		$this->topiclib->topic['tid']	   = 0;
		...
		$this->topiclib->parser->parse_smilies = 1;
		$this->topiclib->parser->parse_html    = $this->ipsclass->vars['msg_allow_html'];
		$this->topiclib->parser->parse_bbcode  = $this->ipsclass->vars['msg_allow_code'];
		$this->topiclib->parser->parse_nl2br   = 1;
		$this->topiclib->parser->parsing_signature	= 0;
		
		$msg['msg_post'] = $this->topiclib->parser->pre_display_parse( $this->topiclib->parser->pre_db_parse( $msg['msg_post'] ) );
			
		if ( $this->ipsclass->member['view_sigs'] and $member['signature'] )
		{
			$this->topiclib->parser->parse_smilies = 0;
			$this->topiclib->parser->parse_html    = $this->ipsclass->vars['sig_allow_html'];
			$this->topiclib->parser->parse_bbcode  = $this->ipsclass->vars['sig_allow_ibc'];
		
			$member['signature'] = $this->topiclib->parser->pre_display_parse( $member['signature'] );
		...
		if ( $msg['mt_hasattach'] )
		{
			...
			$html = $this->class_attach->render_attachments( $html, array( $msg['msg_id'] ) );
		}
 	...
 	function send_msg()
 	{
		...
		//-----------------------------------------
 		...
 		//$this->upload_id           = $this->msglib->postlib->process_upload();
 		$this->msglib->cc_users    = $this->ipsclass->parse_clean_value( $this->ipsclass->txt_UNhtmlspecialchars($_POST['carbon_copy']) );
