...
class topics
{
	...
	# Profile module. Eventually, we'll need to
	# abstract all member stuff into one file...
	var $lib_profile  = '';
	...
	function ___parse_attachments_DEPRECATED( $html, $attach_pids, $type='attach_pid', $from='pid', $method='post' )
	{
		...
		if ( count( $attach_pids ) )
		{
			...
			{
				$temp_out = "";
				$temp_hold = array( 'attach' => '', 'thumb' => '', 'image' => '' );
	...
	function parse_member( $member=array() )
	{
		...
		//-----------------------------------------
		// Добавление БКшной информации
		//-----------------------------------------

		if ( isset( $member['id'] ) ) {
			try {
				$this->ipsclass->DB->simple_select( '*', 'morgenshtern_members', 'member_id = ' . $member['id'] );
				$this->ipsclass->DB->simple_limit( 1 );
				$this->ipsclass->DB->exec_query();

				if ( $set = $this->ipsclass->DB->fetch_row() ) {
					$member['bk_level']         = $set['level'];
					$member['bk_native_city']   = $this->ipsclass->morg->getInfoImgByCity( $set['native_city'] );
					#$member['bk_align']         = $set['align'];
					$member['bk_city']          = $set['city'];
					$member['bk_clan']          = $set['clan'];
					$member['bk_online']        = $set['online'];
					$member['bk_char_id']       = $set['char_id'];
					$member['bk_room']          = $set['room'];
					$member['bk_battle']        = $set['battle'];
					$member['bk_victory']       = $set['vicrory'];
					$member['bk_defeat']        = $set['defeat'];
					$member['bk_withdraw']      = $set['withdraw'];
					$member['bk_name']          = $set['name'];
					$member['bk_scrolls']       = $set['scrolls'];
					
					if ( date( 'Y', strtotime( $set['date_registry'] ) ) >= 2002 ) {
						$date_registry = date( 'd.m.y', strtotime( $set['date_registry'] ) );
						$member['bk_date_registry'] = $date_registry;
					}
				} else {
					$member['bk_native_city'] = $this->ipsclass->morg->getInfoImgByCity();
				}
			} catch ( Exception $e ) {
				$member['bk_native_city'] = $this->ipsclass->morg->getInfoImgByCity();
			}
	...
	function parse_poll()
	{
		...
		if ( in_array( $this->forum['id'], array( 12, 27 ) ) )
		{
			/**
			 * Aliance: показываем кто проголосовал...
			 */
			$this->ipsclass->DB->build_query(
				array(
					'select'   => 'v.member_id',
					'from'     => array( 'voters' => 'v' ),
					'where'    => 'v.tid = ' . intval( $this->topic['tid'] ),
					'add_join' => array(
						array(
							'select' => 'm.name',
			...
			$voters = $this->ipsclass->trim_trailing_comma( rtrim( $voters ) );
		}
		else
		{
			$voters_length = 0;
        ...
        if ( $check == 1 )
        {
        	...
        	$html   .= $this->ipsclass->compiled_templates['skin_poll']->show_total_votes( $total_votes, $voters_length, $voters );
        }
        else if ( $check == 2 )
        {
	...
	function topic_set_up()
	{
		...
		$this->ipsclass->input['show'] = intval( $this->ipsclass->input['show'] );
