...
+---------------------------------------------------------------------------
|   > $Date: 2006-10-12 11:58:40 -0500 (Thu, 12 Oct 2006) $
|   > $Revision: 633 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class func_msg
{
	...
	function ucp_generate_menu()
	{
    	...
    	if ( $this->ipsclass->member['g_use_pm'] )
        {
			...
			{
				$this->ipsclass->member['vdirs'] = "in:Входящие|sent:Отправленные";
			}
	...
	function send_form($preview=0, $errors="")
 	{
		...
		else
		{
 		...
 		else if ( $this->form_orig_id )
 		{
 			...
 			if ( $old_msg['mt_title'] )
 			{
 				if ( $this->class->edit_saved ) 
				{
					...
					$old_message           = $this->postlib->parser->bad_words( $this->ipsclass->my_br2nl( $old_msg['msg_post'] ) );
					$_msg_id               = $old_msg['mt_msg_id'];
					
					if ( $this->postlib->han_editor->method == 'rte' )
					{
						$_POST['tmp_for_msg_edit'] = $old_message;
						
						$old_message = $this->postlib->han_editor->process_raw_post( 'tmp_for_msg_edit' );
						$old_message = $this->postlib->parser->pre_display_parse( $this->postlib->parser->pre_db_parse( $old_message ) );
						
						unset($_POST['tmp_for_msg_edit']);
					}
 		...
 		{
 			$old_message = $this->ipsclass->my_nl2br( $old_message );
 		}
 	...
 	function build_contact_list()
	{
 	...
 	function delete_messages($ids, $owner_id, $extra="")
 	{
		//-----------------------------------------
 		...
 		$this->ipsclass->DB->simple_construct( array( 'select' => 'mt_id, mt_msg_id, mt_read, mt_to_id', 'from' => 'message_topics', 'where' => "$extra AND mt_id $id_string" ) );
 		$this->ipsclass->DB->simple_exec();
 		...
 		while ( $i = $this->ipsclass->DB->fetch_row() )
 		{
 			...
 			if( $i['mt_read'] == 0 )
 			{
	 			$unread[ $i['mt_to_id'] ] = intval($unread[ $i['mt_to_id'] ]) + 1;
 			}
 		}
	...
	function send_pm( $opts=array() )
 	{
		...
		$this->ipsclass->DB->simple_exec();
 		...
 		$testing_parse = $this->postlib->parser->pre_db_parse( $this->msg_post );
 		
 		if( $this->postlib->parser->error != "" )
 		{
	 		$this->error = $this->ipsclass->lang[$this->postlib->parser->error];
	 		return;
 		}
		...
		if ( $this->orig_id )
		{
 		...
 		foreach ($cc_array as $to_member)
 		{
			...
			{
				$to_member['language'] = $to_member['language'] == "" ? 'ru' : $to_member['language'];
				...
				$this->ipsclass->DB->do_insert( 'mail_queue', array( 'mail_to' => $to_member['email'], 'mail_date' => time(), 'mail_subject' => $this->ipsclass->lang['pm_email_subject'], 'mail_content' => $this->postlib->email->message ) );
