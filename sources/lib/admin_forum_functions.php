	...
	function render_forum($r, $depth_guide="")
	{
		...
		if ( $this->type == 'manage' )
		{
			...
			if ( ! $this->show_all )
			{
				...
				if ( count( $sub ) )
				{
					$subforums = '<fieldset style="margin-top:4px"><legend>Подфорумы:</legend>'.implode( ", ", $sub ).'</fieldset>';
				}
			...
			foreach( $this->moderators as $data )
			{
				if ( $data['forum_id'] == $r['id'] )
				{
					if ($data['is_group'] == 1)
					{
						$data['_fullname'] = 'Группа: '.$data['group_name'];
					}
					else
					{
						$data['_fullname'] = $data['member_name'];
					}
