...
class func_mod
{
	...
	function rebuild_topic($tid, $doforum=1)
	{
		...
		$this->ipsclass->DB->do_update( 'topics', array( 'last_post'         => $last_post['post_date'] ? $last_post['post_date'] : $first_post['post_date'],
		...
		if ( ($this->ipsclass->forums->forums_by_id[ $last_post['forum_id'] ]['last_id'] == $tid) AND ($doforum == 1) )
		{
			...
			if ( $this->ipsclass->forums->forums_by_id[ $this->forum['id'] ]['newest_id'] == $tid )
			{
				$sort_key = $this->ipsclass->forums->forums_by_id[ $this->forum['id'] ]['sort_key'];
	...
	{
		$this->ipsclass->DB->build_query( array( 'select' => 'tid', 'from' => 'topics', 'where' => 'approved=1 and forum_id='.intval($forum_id) ) );
