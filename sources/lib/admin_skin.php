...
class admin_skin
{
	...
	function js_help_link($help="", $text="Быстрая помощь")
	{
	...
	{
		return "<a href='#' title=\"Перейти к странице...\" onclick=\"multi_page_jump('$ub',$tp,$pp);\">Pages:</a>";
	}
	...
	function start_table( $title="", $desc="") {
		...
		if (isset($this->td_header[0]))
		{
			...
			foreach ($this->td_header as $td)
			{
				...
				if ($td[0] != '{none}')
				{
					$html .= "<td class='tablesubheader'".$width."align='center'>{$td[0]}</td>\n";
				}
	...
	function skin_jump_menu_wrap()
	{
		return "<br /><div align='center'>
			   <div style='padding:3px 0px 3px 0px;border:0px;'>
