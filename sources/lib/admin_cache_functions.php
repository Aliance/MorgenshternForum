...
+---------------------------------------------------------------------------
|   > $Date: 2007-01-18 18:02:56 -0500 (Thu, 18 Jan 2007) $
|   > $Revision: 831 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class admin_cache_functions
{
	...
	function _recache_wrapper($id, $parent="", $root='1')
	{
		...
		$this->messages[] = "Обновление общего шаблона выполнено... (id: ".implode(',',$theids).")";
	}
	...
	function _recache_templates( $id, $parent="", $group_only="", $root=1, $master_only=0 )
	{
		...
		foreach ( $ids as $i => $tid )
		{
			...
			$this->messages[] = "HTML шаблоны успешно обновлены... (ID: $id)";
		}
	}
	...
	function _write_template_to_file($id, $group_name, $content)
	{
		...
		if ( ! SAFE_MODE_ON )
		{
			...
			if ( is_writeable( CACHE_PATH.'cache/skin_cache' ) )
			{
				...
				else
				{
					if ( file_exists( CACHE_PATH.'cache/skin_cache/cacheid_'.$id.'/'.$group_name.'.php' ) )
					{
						...
						{
							$this->messages[] = "Невозможно произвести запись в файл кеша <span class='rss-feed-invalid'>cache/skin_cache/cacheid_{$id}/{$group_name}.php</span>.";
							$good_to_go = 0;
						}
						else
						{
			...
			{
				$this->messages[] = "Невозможно произвести запись в файл кеша <span class='rss-feed-invalid'>cache/skin_cache/cacheid_{$id}</span>";
			}
		}
		...
		if ( $good_to_go )
		{
			if ( $FH = @fopen( CACHE_PATH.'cache/skin_cache/cacheid_'.$id.'/'.$group_name.'.php', 'w' ) )
			{
				...
				$this->messages[] = "Файл skin_cache/cacheid_{$id}/{$group_name}.php — успешно записан";
			}
		}
	...
	function _rebuild_templates_from_php($id)
	{
		...
		{
			$this->messages[] = "Этот стиль установлен в безопасном режиме (safe mode) и php-файлы кеша отсутствуют, соответствено, нет необходимости запускать эту утилиту для этого стиля.";
			return 0;
		}
		...
		{
			$this->messages[] = "ОШИБКА: невозможна запись в директорию /$skin_dir/, пожалуйста, проверьте права доступа (CHMOD) — они должны быть 0777!";
			return 0;
		}
		...
		if ( is_dir($skin_dir) )
		{
				...
				while (($filename = readdir($handle)) !== false)
				{
					if (($filename != ".") && ($filename != ".."))
					{
						if ( preg_match( "/\.php$/", $filename ) )
						{
							...
							{
								$this->messages[] = "Пропущен skin_bugtracker.php...";
								continue;
							}
							...
							{
								$this->messages[] = "Невозможно открыть для чтения файл $filename. Этот файл пропущен...";
								continue;
							}
							...
							if ( ! preg_match( "/\n/", $fdata ) )
							{
								$this->messages[] = "Файл $filename пропущен, так как он пустой...";
								continue;
							}
			...
			{
				$this->messages[] = "Невозможно открыть для чтения директорию /$skin_dir/.";
				return 0;
			}
		}
		else
		{
			$this->messages[] = "Директория $skin_dir не найдена — пожалуйста, проверьте переменную \$root_path в файле admin.php.";
			return 0;
		}
	
		$this->messages[] = "Обновление php-файлов кеша успешно завершено.";
		return 1;
	}
	...
	function _write_css_to_cache($id, $parent="", $root='1')
	{
		...
		{
			$css = str_replace( "url(style_images/", "url({$this->ipsclass->vars['ipb_img_url']}style_images/", $css );
		}
			...
			if ( is_writeable( CACHE_PATH."style_images" ) )
			{
				foreach( $ids as $id )
				{
					...
					if ( $FH = @fopen( CACHE_PATH."style_images/css_".$id['set_skin_set_id'].".css", 'w' ) )
					{
						...
						$this->messages[] = "Обновление файла кеша CSS css_{$id['set_skin_set_id']}...";
					}
					else
					{
						$this->messages[] = "<br /><b>Файл с кешем css_{$id['set_skin_set_id']} не обновлен! Проверьте права доступа (CHMOD) у /style_images/ и /style_images/css_{$id['set_skin_set_id']}.css!</b>";
					}
				}
			}
			else
			{
				$this->messages[] = "<b>Файл(ы) кеша не обновлен(ы). Проверьте права доступа (CHMOD) у /style_images/ и /style_images/css_{$id['set_skin_set_id']}.css.</b>";
			}
		}
		else
		{
			$this->messages[] = "<b>Файл(ы) кеша не обновлен(ы). Директория /style_images/ не существует.</b>";
		}
	}
	...
	function _recache_macros($id, $parent="", $root=1)
	{
		...
		foreach( $ids as $i )
		{
			...
			$this->messages[] = "Выполнено... (Макрос: {$i['id']})...";
	...
	function _rebuild_all_caches( $affected )
	{
		...
		foreach( $affected as $aid )
		{
			...
			$this->messages[] = "Обновление макросов для стиля &laquo;{$skins[$aid]['set_name']}&raquo;...";
			...
			$this->messages[] = "Обновление CSS для стиля &laquo;{$skins[$aid]['set_name']}&raquo;...";
			...
			$this->messages[] = "Обновление кеша HTML шаблонов для стиля &laquo;{$skins[$aid]['set_name']}&raquo;...";
			...
			$this->messages[] = "Обновление кеша общего шаблона для стиля &laquo;{$skins[$aid]['set_name']}&raquo;...";
		...
		$this->messages[] = "Обновление настроек стиля...";
