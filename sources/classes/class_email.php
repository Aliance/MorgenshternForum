...
class emailer {
	...
	function build_headers()
	{
		...
		$this->mail_headers['X-Unique-ID'] 	= "936d62a2_9c0dc8fd_1438cdbc_11b98";
        ...
        foreach( $enc_headers as $header => $value)
        {
			...
			if (preg_match('/(\w*[\x80-\xFF]+\w*)/', $value))
			{
				if ( function_exists('mb_encode_mimeheader') )
				{
					...
					$value = mb_encode_mimeheader($value, $this->char_set, 'Q');
				}
				else
				{
					$replacement = preg_replace('/(\?|=|\s|[\x80-\xFF])/e', '"=" . strtoupper(dechex(ord("\1")))',$value);
					$value = '=?' .$this->char_set . '?Q?';
					if ( strlen( $value . $replacement . '?=') > 76 )
					{
						$tokens = explode('=', $replacement);
						$encoded_text_parts = array();

						for ($i=1; $i < count( $tokens ); $i++)
						{
							$token_len = strlen( $tokens[$i] ) + 1;
							if ( (strlen( $value ) + $token_len + 2) < 76  )
							{
								$value .= '=' .  $tokens[$i];
							}
							else
							{
								$encoded_text_parts[] = $value . '?=';
								$value = '=?' . $this->char_set . '?Q?=' . $tokens[$i];
							}
						}
	...
	function send_mail()
	{
			...
			else
			{
				...
				if ($this->mail_method != 'smtp')
				{
					if ( ! @mail( $this->to, $this->subject, $this->message, $this->rfc_headers, $this->extra_opts ) )
					{
						...
						{
							$this->fatal_error("Не возможно отправить сообщение", "Проблема при вызове 'mail' функции");
						}
					}
				}
				//-----------------------------------------
		...
		{
			$this->fatal_error("Поля От: или Тема: пустые");
			return FALSE;
		}
	...
	function get_template($name="", $language="")
	{
		...
		{
			$this->error++;
			$this->fatal_error("Обработчик не получил уникальный идентификатор шаблона письма", "");
		}
		...
		if ( ! isset($lang[ $name ]) )
		{
			...
			{
				$this->fatal_error("Шаблон письма с указанным идентификатором ($name) не найден в системе", "");
			}
			else
			{
				require_once( $this->root_path."cache/lang_cache/".$this->ipsclass->vars['default_language']."/lang_email_content.php" );
	...
	function build_message( $words )
	{
		...
		{
			$this->error++;
			$this->fatal_error("Не возможно создать сообщение, так как не указан шаблон письма", "Проверьте что шаблон письма указан");
		}
	...
	function smtp_send_mail()
	{
		...
		if ( ! $this->smtp_fp )
		{
			$this->smtp_error("Не возможно установить связь с SMTP сервером");
			return;
		}
		...
		if ( $this->smtp_code == 220 )
		{
			...
			if ($this->smtp_user and $this->smtp_pass)
			{
				...
				if ( $this->smtp_code == 334 )
				{
					...
					if ( $this->smtp_code != 334  )
					{
						$this->smtp_error("Сервер сообщает об отсутсвии пользователя с указанным именем");
						return;
					}
					...
					{
						$this->smtp_error("Сервер сообщает о несовпадении указанного пароля с паролем пользователя");
						return;
					}
				}
				else
				{
					$this->smtp_error("Сервер не поддерживает аутентификации");
					return;
				}
			}
			...
			foreach( $to_arry as $to_email )
			{
				...
				if ( $this->smtp_code != 250 )
				{
					$this->smtp_error("Недопустимый e-mail адрес: $to_email");
					return;
					break;
				}
			}
			...
			{
				$this->smtp_error("Ошибка при отправке комманды SMTP серверу");
				return;
			}
