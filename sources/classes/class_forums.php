		...
		if (isset($this->mod_cache[ $forum_id ] ) )
		{
			...
			if (is_array($this->mod_cache[ $forum_id ]) )
			{
				foreach ($this->mod_cache[ $forum_id ] as $moderator)
				{
					if ($moderator['isg'] == 1)
					{
						$mod_string .= 'клан <a href="http://capitalcity.combats.com/clans_inf.pl?' . $moderator['gname'] . '" target="_blank"><img src="http://img.combats.com/i/klan/' . $moderator['gname'] . '.gif" alt="" title="' . $moderator['gname'] . '" /></a>';
					...
					else
					{
						...
						//-----------------------------------------
						// Отображение БКшных ников в списке модераторов подфорума
						//-----------------------------------------
						$char = array(
							'id'   => $moderator['memid'],
							'name' => $moderator['name']
						);
						$moderator['name'] = $this->ipsclass->morg->formatChar( $char );
						
#						$mod_string .= "<a href='{$this->ipsclass->base_url}showuser={$moderator['memid']}'>{$moderator['name']}</a>, ";
						$mod_string .= $moderator['name'] . ", ";
					}
				}
...
#					$mod_string .= "<a href='{$this->ipsclass->base_url}showuser={$this->mods[$forum_id]['memid']}'>{$this->mods[$forum_id]['name']}</a>";
					$mod_string .= $this->mods[$forum_id]['name'];
				}
			}
		}
	...
	*/
	...
	function forums_format_lastinfo($forum_data)
	{
		...
		{
			//-----------------------------------------
			// Отображение БКшных ников в последнем ответевшем пользователе
			//-----------------------------------------
			$char = array(
				'id'   => $forum_data['last_poster_id'],
				'name' => $forum_data['last_poster_name']
			);
			$forum_data['last_poster'] = $this->ipsclass->morg->formatChar( $char );
		}
		else
		{
