...
class class_bbcode extends class_bbcode_core
{
		...
		if ( $this->parse_smilies )
		{
			...
			if ( count( $this->ipsclass->cache['emoticons'] ) > 0 )
			{
				...
				foreach( $this->ipsclass->cache['emoticons'] as $row)
				{
					...
					$this->_code = preg_quote( str_replace( "&#39;", '&apos;', str_replace( '<', '&lt;', str_replace( '>', '&gt;', $code ) ) ), "/" );	
					$this->_image = $row['image'];
					//$this->test .= " ".$txt." ".$this->_code."\n";
					$txt = preg_replace_callback( "!(?<=[^\w&;/\"])".$this->_code."(?=.\W|\"|\W.|\W$)!i", array( &$this, 'convert_emoticon' ), $txt );
				}
			}
	...
	*/
	function pre_edit_parse($txt="")
	{
		...
		if ( $this->parse_bbcode )
		{
			...
			while ( preg_match( "#<span style=['\"]font-size:?\s+?(.+?)pt;?\s+?line-height:?\s+?100%['\"]>(.+?)</span>#is", $txt ) )
			{
				$txt = preg_replace_callback( "#<span style=['\"]font-size:?\s+?(.+?)pt;?\s+?line-height:?\s+?100%['\"]>(.+?)</span>#is" , array( &$this, 'unconvert_size' ), $txt );
			}
			
			while ( preg_match( "#<span style=['\"]color:?\s+?(.+?)['\"]>(.+?)</span>#is", $txt ) )
			{
				$txt = preg_replace( "#<span style=['\"]color:?\s+?(.+?)['\"]>(.+?)</span>#is"    , "\[color=\\1\]\\2\[/color\]", $txt );
			}
			
			while ( preg_match( "#<span style=['\"]font-family:?\s+?(.+?)['\"]>(.+?)</span>#is", $txt ) )
			{
				$txt = preg_replace( "#<span style=['\"]font-family:?\s+?(.+?)['\"]>(.+?)</span>#is", "\[font=\\1\]\\2\[/font\]", $txt );
			}
	...
	*/
	function convert_emoticon( $matches=array() )
	{
		...
		//return "<img src=\"{$this->ipsclass->vars['EMOTICONS_URL']}/$image\" style=\"vertical-align: middle; border: 0;\" emoid=\"".trim($code)."\" alt=\"$image\" />";
		return "<img src=\"{$this->ipsclass->vars['EMOTICONS_URL']}/$image\" style=\"vertical-align: middle;\" border=\"0\" emoid=\"".trim($code)."\" alt=\"$image\" />";
	}
