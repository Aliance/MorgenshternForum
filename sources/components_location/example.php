...
//-----------------------------------------
// This must always be 'components_location_{filename}'
//-----------------------------------------

class components_location_example
{
	var $ipsclass;
	...
	{
		return array( '1_type' => 'viewing',
					  '1_id'   => intval($this->ipsclass->input['id']) );
	}
	...
	function parse_online_entries( $array=array() )
	{
		...
		if ( is_array( $array ) and count( $array ) )
		{
			...
			{
				if ( $session_array['member_id'] AND $session_array['location_1_type'] == 'viewing' )
				{
					$location = $this->ipsclass->base_url.'automodule=example&amp;id='.$session_array['location_1_id'];
					$text     = 'In example, ID '.$session_array['location_1_id'];
				}
				else
				{
