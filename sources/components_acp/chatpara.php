...
class ad_chatpara
{
	...
	function chat_splash()
	{
		...
		else
		{
			...
			$this->ipsclass->admin->page_detail = "Если вы уже купили IP Chat Parachat, просто введите сюда ваш Site ID в поле ниже.";
			...
			$this->ipsclass->html .= "<form action='{$this->ipsclass->base_url}&{$this->ipsclass->form_code}&code=chatsave' method='POST'>
									  ...
									  <tr>
									   <td valign='middle' align='left'><b style='color:white'>Купили IP Chat?</b></td>
									   <td valign='middle' align='left'><input type='text' size=35 name='account_no' value='введите ваш Site ID здесь...' onclick=\"this.value='';\"></td>
									   <td valign='middle' align='left'><input type='submit' class='realdarkbutton' value='Продолжить...'></td>
									  </tr>
									  </table>
									  </form>";
	...
	function chat_save()
	{
		...
		if ( $acc_number == "" )
		{
			$this->ipsclass->admin->error("Извините, но указанный вами номер некорректный");
		}
	...
	{
		$this->ipsclass->admin->page_detail = "Вы можете изменять настройки чата";
		$this->ipsclass->admin->page_title  = "Настройка Invision Power Chat";
