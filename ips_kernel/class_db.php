	...
	*/
    ...
    function fatal_error($the_error="")
    {
     	...
     	else if ( $this->obj['use_error_log'] AND $this->obj['error_log'] )
		{
			$_error_string  = "\n===================================================";
			$_error_string .= "\n Время: ". date( 'r' );
			$_error_string .= "\n Код ошибки: " . $this->error_no;
			$_error_string .= "\n Сообщение сервера: " . $this->error;
			...
			print "<html><head>
                    <meta http-equiv=\"content-type\" content=\"text/html; charset=" . $this->error_codepage . "\" />
		...
		else
		{
	    	...
	    	$the_error .= "Время: ".date("l dS \o\f F Y h:i:s A");
	    	...
	    	$out = "<html><head>
	    		   ...
	    		   <blockquote><h1>Ошибка при работе с базой данных</h1><b>Возникла проблема при работе с базой данных.</b><br>
	    		   Вы можете попробовать обновить эту страницу, нажав <a href=\"javascript:window.location=window.location;\">сюда</a>.
	    		   <br><br><b>Отладочная информация:</b><br>
	    		   <form name='mysql'><textarea rows=\"15\" cols=\"60\">".htmlspecialchars($the_error)."</textarea></form><br>Приносим свои извинения за возможные неудобства.</blockquote></body></html>";
