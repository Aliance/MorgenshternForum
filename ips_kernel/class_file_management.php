...
class class_file_management
{
	...
	*/
	function get_file_contents( $file_location, $http_user='', $http_pass='' )
	{
		...
		if ( ! stristr( $file_location, 'http://' ) AND ! stristr( $file_location, 'https://' ) )
		{
			...
			{
				$this->errors[] = "Файл '$file_location' не существует, проверьте путь.";
				return;
			}
	...
	*/
	function get_contents_with_socket( $file_location )
	{
		...
		if ( ! $url_parts['host'] )
		{
			$this->errors[] = "Не найден хост в заданном URL '$file_location'!";
			return FALSE;
		}
		...
		if ( !empty( $url_parts["query"] ) )
		{
      	...
      	{
			$this->errors[] = "Соединение прервано удаленным сервером $host";
			return FALSE;
		...
		else
		{
			...
			{
				$this->errors[] = "Не возможно отправить запрос удаленному серверу $host!";
				return FALSE;
			}
			...
			if ( $this->auth_req )
			{
				if ( $this->auth_user && $this->auth_pass )
				{
					...
					if ( ! fputs( $fp, $header ) )
					{
						$this->errors[] = "Не удалось авторизоваться!";
						return FALSE;
					}
				}
			}
	...
	*/
	...
	function dir_copy_dir($from_path, $to_path, $mode = 0777)
	{
		...
		if ( ! is_dir( $from_path ) )
		{
			$this->errors[] = "Не возможно найти директорию '$from_path'";
			return FALSE;
		}
		...
		if ( ! is_dir( $to_path ) )
		{
			...
			{
				$this->errors[] = "Не возможно создать директорию '$to_path' проверьте права доступа (CHMOD) и попробуйте снова";
				return FALSE;
			}
			else
			{
