...
<div class='tableborder'>
 <div class='tableheaderalt'>Список групп пользователей</div>
 <table cellpadding='4' cellspacing='0' width='100%'>
 <tr>
  <td class='tablesubheader' width='40%'>Название группы</td>
  <td class='tablesubheader' width='10%' align='center'>Доступ в АЦ</td>
  <td class='tablesubheader' width='10%' align='center'>Супермодератор</td>
  <td class='tablesubheader' width='10%' align='center'>Пользователей</td>
...
<div class='tableborder'>
 <div class='tableheaderalt'>Создать группу пользователей</div>
 <table cellpadding='4' cellspacing='0' width='100%'>
 <tr>
  <td class='tablerow2' width='40%'><strong>Основанную на</strong></td>
  <td class='tablerow1' width='60%'>{$form['_new_dd']}</td>
 </tr>
 <tr>
  <td colspan='2' class='tablesubheader' align='center'><input type='submit' value='Создать' class='realbutton' /></td>
 </tr>
 </table>
</div>
</form>
...
{
$IPBHTML .= <<<EOF
	<a href='{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?act=Members&max_results=30&showall=1&filter={$r['g_id']}&sort_order=asc&sort_key=members_display_name&st=0' target='_blank' title='Список пользователей'>{$r['_title']}</a>
EOF;
}
else
{
...
$IPBHTML .= <<<EOF
  ...
  <td class='tablerow1' align='center'><img id="menu{$r['g_id']}" src='{$this->ipsclass->skin_acp_url}/images/filebrowser_action.gif' border='0' alt='Опции' class='ipd' /></td>
</tr>
<script type="text/javascript">
  menu_build_menu(
  "menu{$r['g_id']}",
  new Array( img_edit   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=edit&amp;id={$r['g_id']}'>Изменить</a>",
EOF;
...
{
$IPBHTML .= <<<EOF
              img_delete   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=delete&amp;_admin_auth_key={$this->ipsclass->_admin_auth_key}&amp;id={$r['g_id']}'>Удалить</a>"
EOF;
}
else
{
$IPBHTML .= <<<EOF
              img_delete   + " <em>Невозможно удалить</em>"
EOF;
}
$IPBHTML .= <<<EOF
		    ) );
...
<div class='tableborder'>
 ...
 <div class='tablerow1'>
  <b>Используется группами</b> — указывает на группы пользователей, использующие данную маску доступа.
  <br /><b>Используется пользователями</b> — указывает количество пользователей, использующих данную маску доступа.
  <br /><b>Просмотр</b> дает возможность протестировать данную маску доступа.
 </div>
</div>
<br />					    
<div class='tableborder'>
 <div class='tableheaderalt'>Маски доступа к форумам</div>
 <table cellpadding='4' cellspacing='0' width='100%'>
 <tr>
  <td class='tablesubheader' width='20%'>Название маски</td>
  <td class='tablesubheader' width='15%'>Используется группами</td>
  <td class='tablesubheader' width='20%' align='center'>Используется пользователями</td>
  <td class='tablesubheader' width='1%'>&nbsp;</td>
 </tr>
 {$content}
 </table>
...
<input type='hidden' name='_admin_auth_key' value='{$this->ipsclass->_admin_auth_key}' />
 <div class='tableborder'>
  <div class='tableheaderalt'>Создание новой маски доступа</div>
  <table cellpadding='4' cellspacing='0' width='100%'>
  <tr>
   <td class='tablerow1'><strong>Название</strong></td>
   <td class='tablerow2'><input type='text' class='input' size='30' name='new_perm_name' /></td>
  </tr>
  <tr>
   <td class='tablerow1'><strong>Наследовать права из маски:</strong></td>
   <td class='tablerow2'><select name='new_perm_copy' class='dropdown'>{$dlist}</select></td>
  </tr>
 </table>
 <div class='tablefooter' align='center'><input type='submit' value='Создать' class='realbutton' /></div>
</div>
</form>
EOF;
...
{
$IPBHTML .= <<<EOF
{$r['mems']} (<a href='javascript:pop_win("&amp;{$this->ipsclass->form_code}&amp;code=view_perm_users&amp;id={$r['id']}", "User", "500","350");' title='Открыть новое окно, в котором вывести всех пользователей, использующих данную маску доступа!'>Просмотреть</a>)
EOF;
}
else
{
...
$IPBHTML .= <<<EOF
  ...
  <td class='tablerow1' align='center'><img id="menu{$r['id']}" src='{$this->ipsclass->skin_acp_url}/images/filebrowser_action.gif' border='0' alt='Опции' class='ipd' /></td>
</tr>
<script type="text/javascript">
  menu_build_menu(
  "menu{$r['id']}",
  new Array( img_view   + " <a href='javascript:pop_win(\"&amp;{$this->ipsclass->form_code}&amp;code=preview_forums&amp;id={$r['id']}&amp;t=read\", \"Preview\", \"400\",\"350\");' title='Просмотреть, что могут делать группы'>Просмотр</a>",
  			 img_edit   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=fedit&amp;id={$r['id']}'>Изменить</a>",
EOF;
if ( ! $r['isactive'] )
{
$IPBHTML .= <<<EOF
              img_delete   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=pdelete&amp;id={$r['id']}'>Удалить</a>"
EOF;
}
else
{
$IPBHTML .= <<<EOF
              img_delete   + " <em>Используется</em>"
EOF;
}
$IPBHTML .= <<<EOF
		    ) );
