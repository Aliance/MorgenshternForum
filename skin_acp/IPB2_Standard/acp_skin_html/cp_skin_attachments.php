...
<tr>
 ...
 <td class='tablerow2'><img id="menu{$row['atype_id']}" src='{$this->ipsclass->skin_acp_url}/images/filebrowser_action.gif' border='0' alt='Опции' class='ipd' /></td>
</tr>
<script type="text/javascript">
  menu_build_menu(
  "menu{$row['atype_id']}",
  new Array( img_edit   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=attach_edit&amp;id={$row['atype_id']}'>Изменить</a>",
  			 img_delete   + " <a href='{$this->ipsclass->base_url}&amp;{$this->ipsclass->form_code}&amp;code=attach_delete&amp;id={$row['atype_id']}'>Удалить</a>"
		    ) );
 </script>
EOF;
