...
class Converge_Server
{
    ...
    */
	...
	function onEmailChange( $auth_key, $product_id, $old_email_address, $new_email_address )
	{
		...
		if ( $this->__authenticate( $auth_key, $product_id ) !== FALSE )
		{
			if ( $old_email_address AND $new_email_address )
			{
				...
				{
					$this->ipsclass->DB->do_update( 'members', array( 'username' => $new_email_address ), "id='" . $member['id'] . "'" );
				}
	 ...
	 * and the last ID entered into the local applicationвЂ™s member table.
	 *
	 * Expected repsonse:
	...
	function convergeLogIn( $auth_key, $product_id, $email_address='', $md5_once_password='', $ip_address='', $unix_join_date='', $timezone=0, $dst_autocorrect=0 )
	{
		...
		if ( $this->__authenticate( $auth_key, $product_id ) !== FALSE )
		{
			...
			#$this->class_api_server->xmlrpc->_add_debug( "Found $email_address - ".$this->ipsclass->member['id'] );
