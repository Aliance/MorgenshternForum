...
class handshake_server
{
    ...
    */
	...
	function handshakeEnd( $reg_id='', $reg_code='', $reg_date='', $reg_product_id='', $converge_url='', $handshake_completed='' )
	{
		...
		{
			$this->ipsclass->DB->do_update( 'converge_local', array( 'converge_active' => 0 ), "converge_url = '".$converge_url."'" );
			...
			$this->ipsclass->DB->do_update( "login_methods", array( "login_enabled"      => 1,
																	"login_login_url"    => $_full_url,
			 														"login_maintain_url" => $_full_url . '&redirect=options',
																	"login_logout_url"	 => $_full_url . '&redirect=logout',
																	"login_register_url" => $_full_url . '&act=register' ), "login_folder_name='ipconverge'" );
