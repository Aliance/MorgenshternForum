...
/* CACHE FILE: Generated: Fri, 28 May 2010 08:05:06 GMT */
...
<table class='ipbtable' cellspacing=\"1\">
	<tr>
		<td valign=\"middle\" class=\"row2 nicknamerow topic-left-row\" colspan='2' id='nickname-row-{$post['pid']}'>
			<a name=\"entry{$post['pid']}\"></a>
			" . (($author['id']) ? ("
				<img src=\"http://img.combats.com/i/lock.gif\" width=\"20\" height=\"15\" alt=\"Добавить в приват\" title=\"Добавить в приват\" border='0' onclick='addPrivate( \"{$author['members_display_name']}\" );' style='cursor: pointer;' /> " . (($author['bk_clan']) ? ("<a href=\"http://capitalcity.combats.com/clans_inf.pl?{$author['bk_clan']}\"><img src=\"http://img.combats.com/i/klan/{$author['bk_clan']}.gif\" alt=\"Клан {$author['bk_clan']}\" title=\"Клан {$author['bk_clan']}\" width=\"24\" height=\"15\" /></a>") : ("")) . "<span id=\"post-member-{$post['pid']}\" class='popmenubutton-new-out'>{$author['members_display_name']}</span>" . (($author['bk_level']) ? ("[{$author['bk_level']}]") : ("")) . "" . (($author['bk_char_id']) ? ("<a href=\"http://devilscity.combats.com/inf.pl?{$author['bk_char_id']}\" target='_blank'>") : ("<a href=\"http://devilscity.combats.com/inf.pl?login={$author['members_display_name']}\" target='_blank'>")) . "" . (($author['bk_native_city']) ? ("<img src=\"{$author['bk_native_city']}\" alt=\"Информация о {$author['members_display_name']}\" width=\"12\" height=\"11\" />") : ("")) . "</a>
				<!-- member menu -->
				<div class='popupmenu-new' id='post-member-{$post['pid']}_menu' style='display:none;width:230px'>
					<div class='popupmenu-item-last'>
						...
						" . (($this->ipsclass->member['id']) ? ("
							" . (($this->ipsclass->member['id'] == 1) ? ("
							<div class='popupmenu-item'>
								{$author['_pp_gender_image']} <a href='#' onclick='updateChar( \"{$author['members_display_name']}\" ); return false;'>Обновить персонажа</a>
							</div>
							") : ("")) . "
							<div class='popupmenu-item'>
								" . ((in_array( $author['id'], array_keys( $this->ipsclass->member['_cache']['friends'] ) )) ? ("
		...
		<td class=\"row2\" valign=\"middle\">
			...
			<div align=\"right\">
				<span class=\"postdetails\">{$post['ip_address']}
		...
		<td valign=\"middle\" class=\"row2 topic-right-row\" colspan='2' id='post-count-{$post['pid']}'>
			{$this->ipsclass->lang['posted_on']} <a title=\"{$this->ipsclass->lang['tt_link']}\" href=\"{$this->ipsclass->vars['board_url']}/index.{$this->ipsclass->vars['php_ext']}?showtopic={$post['topic_id']}&view=findpost&p={$post['pid']}\" onclick=\"link_to_post({$post['pid']}); return false;\">#{$post['post_count']}</a>
		</td>
	</tr>
	<tr>
		<td valign=\"top\" class=\"{$post['post_css']} topic-left-row\" id='topic-left-row-{$post['pid']}'>
        		<span class=\"postdetails\">
        			{$author['_pp_online_image']} {$author['title']}<br />
				" . (($author['id'] AND $author['avatar']) ? ("
					<a href=\"{$this->ipsclass->base_url}showuser={$author['id']}\" title=\"{$this->ipsclass->lang['dd_view_profile']}\">{$author['avatar']}</a><br />
				") : ("
	        			{$author['avatar']}<br />
				")) . "
				<br />
        			{$author['member_rank_img']} {$author['member_group']}<br />
	        		{$author['member_posts']}<br />
        			{$author['member_number']}<br />
        			{$author['member_joined']}<br />
        			{$author['member_location']}
	        	</span>
        		<img src=\"{$this->ipsclass->vars['img_url']}/spacer.gif\" alt=\"\" width=\"180\" height=\"1\" />
		</td>
		<td class='{$post['post_css']} topic-switch-left-row'>
			<div class='switcher' id='switcher-left-{$post['pid']}' onclick='switchRow( \"left\", {$post['pid']} )'></div>
		</td>
		<td valign=\"top\" class=\"{$post['post_css']}\" id='post-main-{$post['pid']}'>
			<!-- THE POST {$post['pid']} -->
			<div class=\"postcolor\" id='post-{$post['pid']}'>
				{$post['post']}
				{$post['edit_by']}
		...
		<td class='{$post['post_css']} topic-switch-right-row'>
			<div class='switcher' id='switcher-right-{$post['pid']}' onclick='switchRow( \"right\", {$post['pid']} )'></div>
		</td>
		<td valign=\"top\" class=\"{$post['post_css']} topic-right-row\" id='topic-right-row-{$post['pid']}'>
			<div align=\"right\">
				" . (($author['bk_date_registry']) ? ("
					Регистрация в БК: {$author['bk_date_registry']}<br />
				") : ("")) . "
				" . (($author['bk_city']) ? ("
					Город в БК: {$author['bk_city']}<br />
				") : ("")) . "
				" . (($author['bk_victory']) ? ("
					Побед: {$author['bk_victory']}<br />
				") : ("")) . "
				" . (($author['bk_defeat']) ? ("
					Поражений: {$author['bk_defeat']}<br />
				") : ("")) . "
				" . (($author['bk_withdraw']) ? ("
					Ничьих: {$author['bk_withdraw']}<br />
				") : ("")) . "
				" . (($author['icq_number']) ? ("
					<img src=\"http://web.icq.com/whitepages/online?icq={$author['icq_number']}&img=5\" border='0' width='14' height='14'>{$author['icq_number']}<br />
				") : ("")) . "
	...
	<tr>
		<td class=\"formbuttonrow topic-left-row\" nowrap=\"nowrap\" colspan='2' id='up_button-row-{$post['pid']}'>
			<div style='text-align:left'>
				<a href=\"javascript:scrollToTop();\"><{P_UP}></a>{$post['report_link']}
			</div>
		</td>
		<td class=\"formbuttonrow\" nowrap=\"nowrap\" colspan=\"3\" id='buttons-row-{$post['pid']}'>
			<div class=\"warn-block\">
				{$author['warn_minus']}{$author['warn_img']}{$author['warn_add']}{$author['warn_text']}
			</div>
			<!-- REPORT / UP -->
         		<div align=\"right\">
...
$IPBHTML = "";
//--starthtml--//
$IPBHTML .= "<a href=\"javascript:PopUp('{$this->ipsclass->base_url}act=warn&amp;mid={$id}&amp;CODE=view','Pager','500','450','0','1','1','1')\" class='warn_level'>{$percent}</a>%";
//--endhtml--//
return $IPBHTML;
}
...
/*--------------------------------------------------*/
/*<changed bits>
member_group,RenderRow,warn_level_warn
</changed bits>*/
/* END OF FILE                                      */
/*--------------------------------------------------*/
